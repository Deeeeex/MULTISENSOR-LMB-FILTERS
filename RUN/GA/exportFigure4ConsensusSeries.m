...
% Export fixed-vs-structure-aware consensus time series for paper Figure 4.
...
[~, summary] = runMultisensorFilters_formation_4plus4_TieredLinkAblation( ...
    5, 1, true, struct(), false, 'structureAware', struct(), [1 4]);
...
if size(series.ospa, 2) ~= 2
    error('Expected two arms for Figure 4 export, got %d.', size(series.ospa, 2));
end

data = [ ...
    series.time(:), ...
    series.ospa(:, 1), series.ospa(:, 2), ...
    series.pos(:, 1), series.pos(:, 2), ...
    series.card(:, 1), series.card(:, 2)];

fid = fopen(outputPath, 'w');
if fid < 0
    error('Unable to open output CSV: %s', outputPath);
end
fprintf(fid, 'time,ospa_fixed,ospa_adaptive,rmse_fixed,rmse_adaptive,card_fixed,card_adaptive\n');
fclose(fid);
