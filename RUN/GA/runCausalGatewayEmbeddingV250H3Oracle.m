...
for windowIdx = 1:numel(protocol.anchorTimes)
    ...
    if isempty(result.windows{windowIdx})
        window = initializeWindow(bank, anchorTime, endTime);
    else
    ...
    for candidateIdx = 1:bank.candidateCount
        ...
        timerId = tic;
        fprintf('V250 H=3 t=%d candidate %02d/%02d %s ...\n', ...
...
fprintf(['V250 H=3 oracle complete: joint-positive anchors %d/%d; ', ...
...
function window = initializeWindow(bank, anchorTime, endTime)
window = struct();
...
function config = attachContinuation( ...
        config, posteriors, history, anchorTime)
...
fprintf(fid, '# V250 causal sensor-gateway H=3 oracle\n\n');
