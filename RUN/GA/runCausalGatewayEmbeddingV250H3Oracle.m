...
for windowIdx = 1:numel(protocol.anchorTimes)
    ...
    if isBudgetedLocalGatewayTeacherProtocol(protocol)
        bank = buildCausalGatewayEmbeddingCandidateBankV250( ...
            bankContext, protocol.maximumRestrictedCandidateCount, ...
            struct('singleArcOnly', true, ...
    ...
    for candidateIdx = 1:bank.candidateCount
        ...
        arm = scoreWindow(estimates, diagnostics, ...
            ...
            anchorTime, endTime, candidate, toc(timerId), protocol);
...
window.candidateBankMode = getField(bank, ...
    'candidateBankMode', 'v250-full-bounded-bank');
window.sourceCandidateIndices = getField(bank, ...
    'sourceCandidateIndices', 1:bank.candidateCount);
window.candidateCountByDirectedSlot = getField(bank, ...
    'candidateCountByDirectedSlot', zeros(1, 0));
window.changedDirectedArcCountByCandidate = getField(bank, ...
    'changedDirectedArcCountByCandidate', zeros(1, 0));
...
if valid && isBudgetedLocalGatewayTeacherProtocol(protocol)
    valid = isfield(window, 'candidateBankMode') && ...
        strcmp(window.candidateBankMode, bank.candidateBankMode) && ...
        isfield(window, 'sourceCandidateIndices') && ...
        isequal(window.sourceCandidateIndices, ...
            bank.sourceCandidateIndices) && ...
        isfield(window, 'candidateCountByDirectedSlot') && ...
        isequal(window.candidateCountByDirectedSlot, ...
            bank.candidateCountByDirectedSlot) && ...
        isfield(window, 'changedDirectedArcCountByCandidate') && ...
        isequal(window.changedDirectedArcCountByCandidate, ...
            bank.changedDirectedArcCountByCandidate);
end
if valid
    for candidateIdx = 1:bank.candidateCount
        valid = valid && isequal( ...
...
function arm = scoreWindow(estimates, diagnostics, truth, config, ...
        anchorTime, endTime, candidate, elapsedSeconds, protocol)
...
arm.posteriorAttemptedPayloadBytes = sum(attemptedBytes(:));
arm.controlAttemptedBytes = controlAttemptedBytes( ...
    candidate, config, protocol);
arm.attemptedPayloadBytes = arm.posteriorAttemptedPayloadBytes + ...
    arm.controlAttemptedBytes;
...
if getField(protocol, ...
        'includeControlAttemptedBytesInOracleComparison', false)
    expectedControlBytes = controlAttemptedBytesFromConfig( ...
        config, protocol, candidateIdx > 1);
    passed = passed && ...
        strcmp(arm.candidateType, conditionalValue(candidateIdx > 1, ...
            'single-directed-arc', 'v242-reference')) && ...
        arm.controlAttemptedBytes == expectedControlBytes && ...
        arm.attemptedPayloadBytes == ...
            arm.posteriorAttemptedPayloadBytes + expectedControlBytes;
end
...
aggregate.posteriorAttemptedPayloadBytes = sum(cellfun( ...
    @(value) getField(value, 'posteriorAttemptedPayloadBytes', ...
        value.attemptedPayloadBytes - ...
            getField(value, 'controlAttemptedBytes', 0)), arms));
aggregate.controlAttemptedBytes = sum(cellfun( ...
    @(value) getField(value, 'controlAttemptedBytes', 0), arms));
...
fprintf(fid, '| Selected control bytes | `%.0f B` |\n', ...
    getField(result.aggregateCandidate, 'controlAttemptedBytes', 0));
...
fprintf(fid, ['At each anchor, the oracle maximizes the minimum of the ', ...
    ...
    'attempted-byte saving is at least %+.1f%% and neither formation-level ', ...
    'E-OSPA nor RMSE regresses by more than %.1f%%. The requested gateway ', ...
    'assignment persists for H=3 when physical and falls back to V242 ', ...
    'when it is not.\n\n'], ...
    result.protocol.minimumAggregateByteSavingPercent, ...
    result.protocol.maximumFormationRegressionPercent);
if getField(result.protocol, ...
        'includeControlAttemptedBytesInOracleComparison', false)
    fprintf(fid, ['Every non-reference arm includes %.0f B of controller ', ...
        'telemetry once per H=3 hold; the V242 reference has no action ', ...
        'control charge.\n\n'], ...
        registeredControlAttemptedBytes(result));
end
...
function bytes = controlAttemptedBytes(candidate, config, protocol)
charge = getField(protocol, ...
    'includeControlAttemptedBytesInOracleComparison', false) && ...
    ~strcmp(candidate.candidateType, 'v242-reference');
bytes = controlAttemptedBytesFromConfig(config, protocol, charge);
end

function bytes = controlAttemptedBytesFromConfig(config, protocol, charge)
if ~charge
    bytes = 0;
    return;
end
required = {'sensorsPerFormation', 'formationCount'};
...
if ~valid
    error('BudgetedLocalGatewayTeacherV255:ControlChargeConfig', ...
        'The local control charge requires a uniform formation size.');
end
bytes = controlAttemptedBytesFromProtocol(protocol, ...
    config.sensorsPerFormation);
end

function bytes = controlAttemptedBytesFromProtocol(protocol, ...
        sensorsPerFormation)
if nargin < 2 || isempty(sensorsPerFormation)
    error('BudgetedLocalGatewayTeacherV255:ControlFormationSize', ...
        'The control charge requires the registered formation size.');
end
required = {'compactNodeBytesPerSensor', ...
    'routeCommandHeaderBytes', 'routeCommandBytesPerChangedArc', ...
    'controlCollectionCountPerHold', ...
    'maximumChangedDirectedGatewayArcs'};
if ~isstruct(protocol) || ~isscalar(protocol) || ...
        ~all(isfield(protocol, required))
    error('BudgetedLocalGatewayTeacherV255:ControlChargeProtocol', ...
        'The V255 protocol lacks its local control-byte contract.');
end
bytes = protocol.controlCollectionCountPerHold * ...
    (2 * sensorsPerFormation * protocol.compactNodeBytesPerSensor) + ...
    protocol.routeCommandHeaderBytes + ...
    protocol.maximumChangedDirectedGatewayArcs * ...
        protocol.routeCommandBytesPerChangedArc;
end

function bytes = registeredControlAttemptedBytes(result)
bytes = 0;
for windowIdx = 1:numel(result.windows)
    window = result.windows{windowIdx};
    for armIdx = 1:numel(window.arms)
        bytes = max(bytes, getField( ...
            window.arms{armIdx}, 'controlAttemptedBytes', 0));
    end
end
if bytes <= 0
    error('BudgetedLocalGatewayTeacherV255:MissingControlCharge', ...
        'The completed V255 result contains no charged action arm.');
end
end

function value = conditionalValue(condition, whenTrue, whenFalse)
if condition
    value = whenTrue;
else
    value = whenFalse;
