function [reportPath, summaryPath, summary, config] = ...
    runTopologyDistanceBalancedReliabilitySeed( ...
        seed, simulationLength, runExperiment, includeStaticBaseline)
% RUNTOPOLOGYDISTANCEBALANCEDRELIABILITYSEED
% Focused seed-level runner for distance-balanced reliability bridge repair
% under targeted bridge-link failures.
...
config.bridgeMinScoreFraction = 0.50;
...
config.experimentOverrides = struct( ...
    ...
    'distanceBalancedBridgeMinScoreFraction', ...
        config.bridgeMinScoreFraction);
...
summaryPath = fullfile(scriptDir, sprintf( ...
    'topology_distance_balanced_reliability_seed%d_len%d_%s.mat', ...
    seed, simulationLength, timestamp));

fprintf('\nDistance-balanced reliability-repair topology seed run\n');
...
fprintf('\nDistance-balanced reliability-repair topology result\n');
fprintf('Report: %s\n', reportPath);
fprintf('Summary: %s\n', summaryPath);
fprintf('Arm names:\n');
