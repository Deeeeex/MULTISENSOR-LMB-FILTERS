function [reportPath, result] = ...
        runRiskTriggeredFormationShortcutV261Continuation(options)
% RUNRISKTRIGGEREDFORMATIONSHORTCUTV261CONTINUATION One causal M24 arm.
...
if isempty(tracePath) || exist(tracePath, 'file') ~= 2 || ...
    ...
    error('RiskTriggeredShortcutV261:InvalidRunRequest', ...
        'V261 requires the completed V259 trace and a scalar resume flag.');
end
gitState = resolveResearchGitState();
if gitState.trackedWorktreeDirty || ...
        ~isempty(gitState.untrackedSourceFiles)
    error('RiskTriggeredShortcutV261:DirtySource', ...
        'Official V261 execution requires clean source.');
end

loaded = load(tracePath, 'result', 'behaviorBundle');
if ~isfield(loaded, 'result') || ~isfield(loaded, 'behaviorBundle')
    error('RiskTriggeredShortcutV261:MalformedTrace', ...
...
matPath = fullfile(outputRoot, ...
    'RISK_TRIGGERED_FORMATION_SHORTCUT_V261_CONTINUATION.mat');
reportPath = fullfile(outputRoot, ...
    'RISK_TRIGGERED_FORMATION_SHORTCUT_V261_CONTINUATION.md');
if resume && exist(matPath, 'file') == 2
    ...
    if isfield(checkpoint, 'result') && ...
        ...
        fprintf('Reused completed V261 continuation: %s\n', reportPath);
        return;
    end
end
...
config = buildRiskTriggeredFormationShortcutV261Config(inputs.config);
config = attachContinuation(config, initialPosteriors, initialHistory, ...
    protocol.continuationStartTime, tracePath);
execution = buildRiskTriggeredFormationShortcutV261ExecutionContext( ...
...
fprintf('V261 continuation, t=%d--%d ...\n', ...
...
result.contractVersion = ...
    'risk-triggered-formation-shortcut-v261-result-v1';
...
result.nextMethodDecision = conditionalValue( ...
    ...
    'close-formation-hop-shortcut-and-revisit-relay-gateway-alignment');
...
writeReport(reportPath, result);
fprintf(['V261 complete: E/R/C %+.3f%%/%+.3f%%/%+.3f%%; ', ...
...
valid = isstruct(trace) && isscalar(trace) && ...
    ...
    numel(bundle.preDecisionTopologyHistoryByTime) >= ...
        protocol.continuationEndTime;
if ~valid
    error('RiskTriggeredShortcutV261:TraceContract', ...
        'The V259 trace does not satisfy the V261 source contract.');
end
end
...
arm = scoreMatrices(eospa, rmse, cardinality, config);
arm.name = 'V261 risk-triggered formation shortcut';
...
if ~passed
    error('RiskTriggeredShortcutV261:RuntimeContract', ...
        'The V261 continuation violated its route or evidence contract.');
end
end
...
fid = fopen(path, 'w');
if fid < 0
    error('RiskTriggeredShortcutV261:ReportOpen', ...
        'Unable to write the V261 report.');
end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
base = result.v242;
candidate = result.candidate;
comparison = result.comparison;
fprintf(fid, '# V261 risk-triggered formation shortcut\n\n');
...
if result.actionMechanismPassed
    fprintf(fid, ['The same-budget formation shortcut repairs the ', ...
        'diagnosed F4 event under the registered network and formation ', ...
        'guards. Run one complete M24 candidate without changing the ', ...
        'risk or donor thresholds.\n\n']);
else
    fprintf(fid, ['The same-budget formation shortcut does not repair ', ...
        'the diagnosed F4 event. Close pure hop shortening and test ', ...
        'whether temporal gateway alignment, rather than the formation ', ...
        'tree alone, is the missing relay mechanism.\n\n']);
end
