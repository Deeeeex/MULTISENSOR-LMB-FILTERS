...
if resume && exist(matPath, 'file') == 2
    ...
    if isfield(checkpoint, 'result') && ...
        ...
        fprintf('Reused completed V265 continuation: %s\n', reportPath);
        return;
    end
end
...
fprintf('V265 continuation, t=%d--%d ...\n', ...
...
writeReport(reportPath, result);
fprintf(['V265 complete: E/R/C %+.3f%%/%+.3f%%/%+.3f%%; ', ...
...
arm = scoreMatrices(eospa, rmse, cardinality, config);
arm.name = 'V265 label-selective risk shortcut';
    ...
    if isstruct(pageDetails) && isscalar(pageDetails) && ...
        ...
        if isstruct(evaluation) && isscalar(evaluation) && ...
            ...
            arm.etaReferenceExistenceByTime(localTime) = ...
                getField(evaluation, 'referenceExistence', NaN);
            arm.etaCandidateExistenceByTime(localTime) = ...
                getField(evaluation, 'candidateExistence', NaN);
            projection = getField(evaluation, 'projection', struct());
...
fprintf(fid, '# V265 label-selective risk shortcut\n\n');
...
fprintf(fid, ['| t | Source to receiver | Source r / risk | ', ...
...
for localTime = find(candidate.labelShortcutActiveByTime)
    ...
    fprintf(fid, ['| %d | S%d to S%d | %.3f / %.3f | %.3f / %.3f | ', ...
        '%.3f | %+.3f | %.3f | %s |\n'], ...
        times(localTime), ...
...
if result.actionMechanismPassed
    fprintf(fid, ['The label-only shortcut repairs the diagnosed event ', ...
        'without changing the full-posterior backbone or violating the ', ...
        'registered guards. Run one complete M24 arm with this frozen ', ...
        'risk, gateway and eta-projection rule.\n\n']);
else
    fprintf(fid, ['The causal label-only shortcut does not repair the ', ...
        'diagnosed event under the registered guards. Inspect whether ', ...
        'the failure is source selection, one-hop latency or insufficient ', ...
        'label weight before introducing a learned ranker.\n\n']);
end
