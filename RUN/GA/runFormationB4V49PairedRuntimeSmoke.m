function result = runFormationB4V49PairedRuntimeSmoke()
% RUNFORMATIONB4V49PAIREDRUNTIMESMOKE Minimal real-filter sanity check.
...
for armIdx = 1:2
    ...
    if ~strcmpi(triggerConfig.eventPolicy, 'alwaysHeavy') || ...
        ...
        error('FormationB4V49Smoke:InvalidComparison', ...
...
phaseOneRoutesDiffer = all(arrayfun(@(time) ...
...
passed = ...
    ...
    phaseOneRoutesDiffer && nonburstRoutesMatch && ...
    all(candidate.cycleSelectedByTime(phaseOneTimes)) && ...
    ~any(candidate.cycleSelectedByTime(nonburstTimes));
if ~passed
    error('FormationB4V49Smoke:PairedComparisonFailed', ...
        'The controlled V46/V49 runtime comparison failed.');
end
...
result.phaseOneRoutesDiffer = phaseOneRoutesDiffer;
...
fprintf(['V49 runtime smoke: attempts=%d/%d common=%d ', ...
    'cycles=%d/%d delivered-strong(v46/v49)=%d/%d\n'], ...
...
if ~isequal(delivered, expectedDelivered)
    error('FormationB4V49Smoke:DeliveryMismatch', ...
...
if ~all(attemptedStrong)
    error('FormationB4V49Smoke:AttemptedRouteDisconnected', ...
