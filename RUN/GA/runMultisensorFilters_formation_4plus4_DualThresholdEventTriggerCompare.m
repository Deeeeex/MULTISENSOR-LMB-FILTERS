...
storeDebugArtifacts = getField( ...
    experimentOverrides, 'storeDebugArtifacts', false);
...
attemptCount = zeros(numberOfTrials, numberOfArms);
deliveryCount = zeros(numberOfTrials, numberOfArms);
...
topologyEdgeRisk = zeros(numberOfTrials, numberOfArms);
topologyStaticEdgeRisk = zeros(numberOfTrials, numberOfArms);
topologyNonStaticEdgeRisk = zeros(numberOfTrials, numberOfArms);
topologyNewEdgeRisk = zeros(numberOfTrials, numberOfArms);
topologyLabelMismatch = zeros(numberOfTrials, numberOfArms);
topologyStaticLabelMismatch = zeros(numberOfTrials, numberOfArms);
topologyNonStaticLabelMismatch = zeros(numberOfTrials, numberOfArms);
topologyNewEdgeLabelMismatch = zeros(numberOfTrials, numberOfArms);
fusionStaticWeightMass = zeros(numberOfTrials, numberOfArms);
fusionNonStaticWeightMass = zeros(numberOfTrials, numberOfArms);
fusionNewEdgeWeightMass = zeros(numberOfTrials, numberOfArms);
fusionRiskWeightedEdgeRisk = zeros(numberOfTrials, numberOfArms);
fusionRiskWeightedLabelMismatch = zeros(numberOfTrials, numberOfArms);
pDropBySensorTrials = zeros(numberOfTrials, numberOfSensors);
trialSeeds = NaN(1, numberOfTrials);
debugArtifacts = [];
if storeDebugArtifacts
    debugArtifacts = cell(numberOfTrials, numberOfArms);
end
...
for trialIdx = 1:numberOfTrials
    ...
    for armIdx = 1:numberOfArms
        ...
        if storeDebugArtifacts
            debugArtifacts{trialIdx, armIdx} = struct( ...
                'stateEstimatesBySensor', {stateEstimatesBySensor}, ...
                'communicationDiagnostics', communicationDiagnostics, ...
                'consensusPositionSeries', positionSeries, ...
                'consensusCardinalitySeries', cardinalitySeries, ...
                'consensusOspaSeries', ospaSeries, ...
                'triggerConfig', arms(armIdx).triggerConfig);
        end
        ...
        attemptCount(trialIdx, armIdx) = getField( ...
            communicationSummary, 'attemptCount', 0);
        deliveryCount(trialIdx, armIdx) = getField( ...
            communicationSummary, 'deliveryCount', 0);
        ...
        topologyEdgeRisk(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanTopologyEdgeRisk', 0);
        topologyStaticEdgeRisk(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanTopologyStaticEdgeRisk', 0);
        topologyNonStaticEdgeRisk(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanTopologyNonStaticEdgeRisk', 0);
        topologyNewEdgeRisk(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanTopologyNewEdgeRisk', 0);
        topologyLabelMismatch(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanTopologyLabelMismatch', 0);
        topologyStaticLabelMismatch(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanTopologyStaticLabelMismatch', 0);
        topologyNonStaticLabelMismatch(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanTopologyNonStaticLabelMismatch', 0);
        topologyNewEdgeLabelMismatch(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanTopologyNewEdgeLabelMismatch', 0);
        fusionStaticWeightMass(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanFusionStaticWeightMass', 0);
        fusionNonStaticWeightMass(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanFusionNonStaticWeightMass', 0);
        fusionNewEdgeWeightMass(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanFusionNewEdgeWeightMass', 0);
        fusionRiskWeightedEdgeRisk(trialIdx, armIdx) = getField( ...
            communicationSummary, 'meanFusionRiskWeightedEdgeRisk', 0);
        fusionRiskWeightedLabelMismatch(trialIdx, armIdx) = getField( ...
            communicationSummary, ...
            'meanFusionRiskWeightedLabelMismatch', 0);
...
summary.communication.attemptCount = mean(attemptCount, 1);
summary.communication.deliveryCount = mean(deliveryCount, 1);
...
summary.topologyRisk.edgeRisk = mean(topologyEdgeRisk, 1);
summary.topologyRisk.staticEdgeRisk = mean(topologyStaticEdgeRisk, 1);
summary.topologyRisk.nonStaticEdgeRisk = ...
    mean(topologyNonStaticEdgeRisk, 1);
summary.topologyRisk.newEdgeRisk = mean(topologyNewEdgeRisk, 1);
summary.topologyRisk.labelMismatch = mean(topologyLabelMismatch, 1);
summary.topologyRisk.staticLabelMismatch = ...
    mean(topologyStaticLabelMismatch, 1);
summary.topologyRisk.nonStaticLabelMismatch = ...
    mean(topologyNonStaticLabelMismatch, 1);
summary.topologyRisk.newEdgeLabelMismatch = ...
    mean(topologyNewEdgeLabelMismatch, 1);
summary.topologyRisk.fusionStaticWeightMass = ...
    mean(fusionStaticWeightMass, 1);
summary.topologyRisk.fusionNonStaticWeightMass = ...
    mean(fusionNonStaticWeightMass, 1);
summary.topologyRisk.fusionNewEdgeWeightMass = ...
    mean(fusionNewEdgeWeightMass, 1);
summary.topologyRisk.fusionRiskWeightedEdgeRisk = ...
    mean(fusionRiskWeightedEdgeRisk, 1);
summary.topologyRisk.fusionRiskWeightedLabelMismatch = ...
    mean(fusionRiskWeightedLabelMismatch, 1);
...
summary.trials.attemptCount = attemptCount;
summary.trials.deliveryCount = deliveryCount;
...
summary.trials.topologyEdgeRisk = topologyEdgeRisk;
summary.trials.topologyStaticEdgeRisk = topologyStaticEdgeRisk;
summary.trials.topologyNonStaticEdgeRisk = topologyNonStaticEdgeRisk;
summary.trials.topologyNewEdgeRisk = topologyNewEdgeRisk;
summary.trials.topologyLabelMismatch = topologyLabelMismatch;
summary.trials.topologyStaticLabelMismatch = topologyStaticLabelMismatch;
summary.trials.topologyNonStaticLabelMismatch = ...
    topologyNonStaticLabelMismatch;
summary.trials.topologyNewEdgeLabelMismatch = ...
    topologyNewEdgeLabelMismatch;
summary.trials.fusionStaticWeightMass = fusionStaticWeightMass;
summary.trials.fusionNonStaticWeightMass = fusionNonStaticWeightMass;
summary.trials.fusionNewEdgeWeightMass = fusionNewEdgeWeightMass;
summary.trials.fusionRiskWeightedEdgeRisk = fusionRiskWeightedEdgeRisk;
summary.trials.fusionRiskWeightedLabelMismatch = ...
    fusionRiskWeightedLabelMismatch;
if storeDebugArtifacts
    summary.debugArtifacts = debugArtifacts;
end
...
if getField(experimentOverrides, 'includeTopologyRecoveryStressVariants', false)
    staticEdgeBudget = countUndirectedEdgesFromNeighborMap( ...
        buildNeighborMap4Plus4(8));
    ...
    arms(end+1) = makeArm( ...
        ...
        'Static 4+4 light-posterior graph under targeted bridge-edge failures.', ...
        cfg);

    cfg = buildPeriodicLightVariantConfig( ...
        base, staticEdgeBudget, experimentOverrides, true);
    cfg.topologyReliabilityWeight = getField(experimentOverrides, ...
        'recoveryTopologyReliabilityWeight', 0.55);
    cfg.topologyOverlapWeight = getField(experimentOverrides, ...
        'recoveryTopologyOverlapWeight', 0.35);
    cfg.topologyComplementarityWeight = getField(experimentOverrides, ...
        'recoveryTopologyComplementarityWeight', 0.10);
    cfg.topologyStaticEdgeBonus = getField(experimentOverrides, ...
        'recoveryTopologyStaticEdgeBonus', 0.35);
    cfg.topologyFallbackToBaseOnConnectivityFailure = getField( ...
        experimentOverrides, 'recoveryTopologyFallbackToBase', true);
    cfg.modeAwareFusionWeights = getField(experimentOverrides, ...
    ...
    cfg.heavyFusionWeightFactor = getField(experimentOverrides, ...
        'recoveryHeavyFusionWeightFactor', 1.0);
    ...
    arms(end+1) = makeArm( ...
        ...
        'Failure-aware dynamic topology that preserves edge budget while avoiding failed bridge edges.', ...
        cfg);
end
end
...
fprintf(fid, '## 通信与性能结果\n\n');
fprintf(fid, '| Arm | Scalars | Bytes | Attempts | Deliveries | Trigger | Light | Heavy | Delivery | Downgrades | Edges | Alg. conn. | Runtime (s) | Local E-OSPA | Local RMSE | Consensus OSPA | Position disagreement | Card. dispersion |\n');
fprintf(fid, '|:--|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|\n');
for armIdx = 1:numel(summary.armNames)
    fprintf(fid, '| %s | %.0f | %.0f | %.0f | %.0f | %.3f | %.3f | %.3f | %.3f | %.1f | %.1f | %.3f | %.3f | %.4f | %.4f | %.4f | %.4f | %.4f |\n', ...
        summary.armNames{armIdx}, ...
        summary.communication.payloadScalars(armIdx), ...
        summary.communication.payloadBytes(armIdx), ...
        summary.communication.attemptCount(armIdx), ...
        summary.communication.deliveryCount(armIdx), ...
        summary.communication.triggerRate(armIdx), ...
...
if isfield(summary, 'topologyRisk')
fprintf(fid, '\n## Topology risk attribution diagnostics\n\n');
fprintf(fid, '| Arm | Edge risk | Static risk | Non-static risk | New-edge risk | Label mismatch | Non-static mismatch | Fusion non-static weight | Fusion new-edge weight | Fusion risk-weighted edge risk | Fusion risk-weighted mismatch |\n');
fprintf(fid, '|:--|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|\n');
for armIdx = 1:numel(summary.armNames)
    fprintf(fid, '| %s | %.3f | %.3f | %.3f | %.3f | %.3f | %.3f | %.3f | %.3f | %.3f | %.3f |\n', ...
        summary.armNames{armIdx}, ...
        summary.topologyRisk.edgeRisk(armIdx), ...
        summary.topologyRisk.staticEdgeRisk(armIdx), ...
        summary.topologyRisk.nonStaticEdgeRisk(armIdx), ...
        summary.topologyRisk.newEdgeRisk(armIdx), ...
        summary.topologyRisk.labelMismatch(armIdx), ...
        summary.topologyRisk.nonStaticLabelMismatch(armIdx), ...
        summary.topologyRisk.fusionNonStaticWeightMass(armIdx), ...
        summary.topologyRisk.fusionNewEdgeWeightMass(armIdx), ...
        summary.topologyRisk.fusionRiskWeightedEdgeRisk(armIdx), ...
        summary.topologyRisk.fusionRiskWeightedLabelMismatch(armIdx));
end
end
