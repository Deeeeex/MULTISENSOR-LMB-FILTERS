...
attemptedPayloadScalars = zeros(numberOfTrials, numberOfArms);
deliveredPayloadBytes = zeros(numberOfTrials, numberOfArms);
deliveredPayloadScalars = zeros(numberOfTrials, numberOfArms);
...
for trialIdx = 1:numberOfTrials
    ...
    for armIdx = 1:numberOfArms
        ...
        attemptedPayloadBytes(trialIdx, armIdx) = getField( ...
            communicationSummary, 'attemptedPayloadBytes', ...
            communicationSummary.payloadBytes);
        attemptedPayloadScalars(trialIdx, armIdx) = getField( ...
            communicationSummary, 'attemptedPayloadScalars', ...
            communicationSummary.payloadScalars);
        deliveredPayloadBytes(trialIdx, armIdx) = getField( ...
            communicationSummary, 'deliveredPayloadBytes', ...
            communicationSummary.payloadBytes);
        deliveredPayloadScalars(trialIdx, armIdx) = getField( ...
            communicationSummary, 'deliveredPayloadScalars', ...
            communicationSummary.payloadScalars);
...
summary.communication.attemptedPayloadScalars = ...
    mean(attemptedPayloadScalars, 1);
summary.communication.deliveredPayloadBytes = ...
    mean(deliveredPayloadBytes, 1);
summary.communication.deliveredPayloadScalars = ...
    mean(deliveredPayloadScalars, 1);
summary.communication.payloadDeliveryRatio = ...
    summary.communication.deliveredPayloadBytes ./ ...
    max(summary.communication.attemptedPayloadBytes, 1);
...
summary.communication.effectiveKlaGraphQuality = ...
    summary.effectiveGraph.effectiveWeightConnectivity;
...
if getField(experimentOverrides, 'includeTopologyRecoveryStressVariants', false)
    ...
    cfg = buildPeriodicLightVariantConfig( ...
        base, staticEdgeBudget, experimentOverrides, true);
    cfg.topologyReliabilityWeight = 1.0;
    cfg.topologyOverlapWeight = 0.0;
    cfg.topologyComplementarityWeight = 0.0;
    cfg.topologyStaticEdgeBonus = 0.0;
    cfg.topologyFallbackToBaseOnConnectivityFailure = false;
    cfg.nonStaticFusionWeightFactor = getField(experimentOverrides, ...
        'stableReliabilityNonStaticFusionWeightFactor', 1.0);
    cfg.maxNonStaticFusionWeight = getField(experimentOverrides, ...
        'stableReliabilityMaxNonStaticFusionWeight', inf);
    arms(end+1) = makeArm( ...
        'Recovery stress: Stable reliability-only dynamic topology', ...
        'Failure-aware dynamic topology selected only by link reliability to avoid posterior-driven topology churn.', ...
        cfg);
    ...
    cfg.topologyOverlapWeight = 0.0;
    cfg.topologyComplementarityWeight = 0.0;
    cfg.topologyStructurePreservingEnabled = true;
    cfg.topologyBackboneGroups = getField(experimentOverrides, ...
        'recoveryTopologyBackboneGroups', {1:4, 5:8});
    cfg.topologyStructureBridgeBudget = getField(experimentOverrides, ...
        'stableReliabilityBridgeBudget', 4);
    cfg.topologyMinAlgebraicConnectivity = 0;
    cfg.topologyFallbackToBaseOnConnectivityFailure = false;
    cfg.topologyPreviousEdgeBonus = getField(experimentOverrides, ...
        'stableReliabilityPreviousEdgeBonus', 0);
    arms(end+1) = makeArm( ...
        'Recovery stress: Static-preserving reliability repair topology', ...
        'Failure-aware reliability-only topology that preserves the static 4+4 backbone and rewires only bridge slots.', ...
        cfg);
    ...
    cfg.topologyStructureBridgeBudget = getField(experimentOverrides, ...
        'stableReliabilityBridgeBudget', 4);
    cfg.topologyMaxStructureBridgeDegree = getField(experimentOverrides, ...
        'stableReliabilityMaxBridgeDegree', 1);
    cfg.topologyStructureBridgePairs = getField(experimentOverrides, ...
        'stableReliabilityBridgePairs', []);
    cfg.topologyMinAlgebraicConnectivity = 0;
    cfg.topologyFallbackToBaseOnConnectivityFailure = false;
    arms(end+1) = makeArm( ...
        'Recovery stress: Balanced reliability repair topology', ...
        'Failure-aware reliability-only topology that preserves the static backbone and balances bridge repairs across nodes.', ...
        cfg);
    ...
    cfg.topologyMaxStructureBridgeDegree = getField(experimentOverrides, ...
        'stableReliabilityMaxBridgeDegree', 1);
    cfg.topologyStructureBridgePairs = getField(experimentOverrides, ...
        'matchedReliabilityBridgePairs', [1 6; 2 8; 3 5; 4 7]);
    cfg.topologyMinAlgebraicConnectivity = 0;
    cfg.topologyFallbackToBaseOnConnectivityFailure = false;
    arms(end+1) = makeArm( ...
        'Recovery stress: Role-matched reliability repair topology', ...
        'Failure-aware reliability-only bridge repair that preserves the static backbone and uses a role-matched one-to-one bridge policy.', ...
        cfg);
    ...
    cfg.topologyMaxStructureBridgeDegree = getField(experimentOverrides, ...
        'stableReliabilityMaxBridgeDegree', 1);
    cfg.topologyStructureBridgeSelectionMode = 'distanceBalanced';
    cfg.topologyStructureBridgeMinScoreFraction = getField( ...
        experimentOverrides, 'distanceBalancedBridgeMinScoreFraction', 0.50);
    cfg.topologyMinAlgebraicConnectivity = 0;
    cfg.topologyFallbackToBaseOnConnectivityFailure = false;
    arms(end+1) = makeArm( ...
        'Recovery stress: Distance-balanced reliability repair topology', ...
        'Failure-aware reliability-only bridge repair that selects a one-to-one cross-group matching with balanced bridge lengths.', ...
        cfg);

    cfg = buildPeriodicLightVariantConfig( ...
        base, staticEdgeBudget, experimentOverrides, true);
    cfg.topologyScoreMode = 'compatible';
    cfg.topologyReliabilityWeight = 1.0;
    cfg.topologyOverlapWeight = 0.0;
    cfg.topologyComplementarityWeight = getField(experimentOverrides, ...
        'compatibleRepairComplementarityWeight', 0.10);
    cfg.topologyCompatibilityRiskWeight = getField(experimentOverrides, ...
        'compatibleRepairRiskWeight', 0.50);
    cfg.topologyCompatibilityCoverageWeight = getField(experimentOverrides, ...
        'compatibleRepairCoverageWeight', 0.10);
    cfg.topologyCompatibilityOverlapWeight = getField(experimentOverrides, ...
        'compatibleRepairOverlapWeight', 0.10);
    cfg.topologyStructurePreservingEnabled = true;
    cfg.topologyBackboneGroups = getField(experimentOverrides, ...
        'recoveryTopologyBackboneGroups', {1:4, 5:8});
    cfg.topologyStructureBridgeBudget = getField(experimentOverrides, ...
    ...
    cfg.topologyFallbackToBaseOnConnectivityFailure = false;
    cfg.topologyPreviousEdgeBonus = getField(experimentOverrides, ...
        'compatibleRepairPreviousEdgeBonus', 0.75);
    arms(end+1) = makeArm( ...
        'Recovery stress: Compatibility-balanced repair topology', ...
        'Failure-aware bridge repair that preserves the static backbone and prefers KLA-compatible cross-group pairs.', ...
        cfg);
    ...
    cfg.topologyMaxEdgeChangeFraction = getField(experimentOverrides, ...
        'recoveryTopologyMaxEdgeChangeFraction', 0.15);
    arms(end+1) = makeArm( ...
        'Recovery stress: Churn-aware dynamic topology', ...
        'Failure-aware dynamic topology with previous-edge hysteresis and a per-step rewire cap.', ...
        cfg);
    ...
    arms(end+1) = makeArm( ...
        ...
        'Failure-aware dynamic topology with capped KLA weight mass on non-static recovery edges.', ...
        cfg);
    ...
    arms(end+1) = makeArm( ...
        ...
        'Failure-aware dynamic topology that preserves static intra-group backbone edges and rewires only bridge slots.', ...
        cfg);
end
end
...
for armIdx = 1:numel(summary.armNames)
    fprintf('%s: attempted bytes %.0f, delivered bytes %.0f, delivery %.3f, effective lambda2 %.3f, local E-OSPA %.3f, consensus OSPA %.3f\n', ...
...
fprintf(fid, '## 通信与性能结果\n\n');
fprintf(fid, '| Arm | Delivered scalars | Attempted bytes | Delivered bytes | Attempts | Deliveries | Packet delivery | Payload delivery | Eff. lambda2 | Trigger | Light | Heavy | Downgrades | Edges | Alg. conn. | Runtime (s) | Local E-OSPA | Local RMSE | Consensus OSPA | Position disagreement | Card. dispersion |\n');
fprintf(fid, '|:--|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|\n');
for armIdx = 1:numel(summary.armNames)
    fprintf(fid, '| %s | %.0f | %.0f | %.0f | %.0f | %.0f | %.3f | %.3f | %.3f | %.3f | %.3f | %.3f | %.1f | %.1f | %.3f | %.3f | %.4f | %.4f | %.4f | %.4f | %.4f |\n', ...
        summary.armNames{armIdx}, ...
        summary.communication.deliveredPayloadScalars(armIdx), ...
        summary.communication.attemptedPayloadBytes(armIdx), ...
        summary.communication.deliveredPayloadBytes(armIdx), ...
