...
options.collectInvalidRecords = getField( ...
    options, 'collectInvalidRecords', false);
if ~islogical(options.collectInvalidRecords) || ...
        ~isscalar(options.collectInvalidRecords)
    error('DynamicTopologyScenarioAudit:InvalidOption', ...
        'collectInvalidRecords must be a scalar logical value.');
end
...
for presetIdx = 1:numel(presetNames)
    for seedIdx = 1:numel(seedList)
        ...
        validation = validateDynamicTopologyScenario( ...
            config, sensors, targets, graphs, struct( ...
                'throwOnInvalid', ~options.collectInvalidRecords));
