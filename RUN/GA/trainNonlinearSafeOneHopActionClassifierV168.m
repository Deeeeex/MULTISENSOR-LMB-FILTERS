    ...
    for weightDecay = weightDecayGrid
        ...
        for seedIdx = 1:numel(ensembleSeeds)
            ...
            members{seedIdx} = fitPositiveUtilityHead( ...
                ...
                cellIds(trainMask), utilityRidgeLambda, rankingMode);
        ...
        for uncertaintyPenalty = uncertaintyPenaltyGrid
            ...
            if ismember(rankingMode, { ...
                    'positive-utility', 'rmse-utility'})
...
if ismember(rankingMode, {'positive-utility', 'rmse-utility'})
...
elseif strcmp(dataset.contractVersion, ...
        'second-rollout-aggregated-label-action-value-dataset-v181-v1')
    boundary = [ ...
        'V181 is the second opened seed-211 DAgger-style learnability ', ...
        'gate. The MLP trains on V166 early/static cells, V176 t=78 and ', ...
        'the previously opened V176 t=79 rollout cells. The original V166 ', ...
        'F3 calibration boundary remains unchanged; duplicate V180 t=78 ', ...
        'states are excluded. Width, decay, uncertainty and threshold are ', ...
        'frozen before the new V180 F5 t=79 rollout cells are read. ', ...
        'Inference features are truth-free and current truth supplies ', ...
        'immediate action signs. A pass authorizes one recursive ', ...
        'development probe, not validation or generalization.'];
elseif strcmp(dataset.contractVersion, ...
        'rollout-aggregated-label-action-value-dataset-v178-v1')
    boundary = [ ...
        'V178 is an opened seed-211 policy-iteration learnability gate. ', ...
        'The MLP trains on grouped V166 early/static cells plus V176 t=78 ', ...
        'states actually visited by recursive V169. Width, weight decay, ', ...
        'uncertainty penalty and threshold are frozen using only the V166 ', ...
        'F3 t=79 calibration cells before the V176 F5 t=79 rollout cells ', ...
        'are opened. Inference features are truth-free; current truth ', ...
        'supplies immediate action signs. A pass authorizes one recursive ', ...
        'development probe, not validation or generalization.'];
else
    boundary = [ ...
...
function model = fitPositiveUtilityHead( ...
        model, Z, gain, gainScale, cellIds, lambda, rankingMode)
...
hidden = tanh(bsxfun(@plus, ...
    Z * model.parameters{1}, model.parameters{2}));
if strcmp(rankingMode, 'rmse-utility')
    utility = gain(positive, 2) / gainScale(2);
else
    utility = min(bsxfun(@rdivide, ...
        gain(positive, :), gainScale), [], 2);
end
