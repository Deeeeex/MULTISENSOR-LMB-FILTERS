function [reportPath, summaryPath, summary, config] = ...
    runTopologyStableReliabilitySeed( ...
        seed, simulationLength, runExperiment, includeStaticBaseline, ...
        nonStaticFusionWeightFactor, maxNonStaticFusionWeight)
% RUNTOPOLOGYSTABLERELIABILITYSEED
% Focused seed-level runner for the stable reliability-only topology
% recovery candidate under targeted bridge-link failures.
...
if nargin < 5
    nonStaticFusionWeightFactor = [];
end
if nargin < 6
    maxNonStaticFusionWeight = [];
end
...
config.nonStaticFusionWeightFactor = nonStaticFusionWeightFactor;
config.maxNonStaticFusionWeight = maxNonStaticFusionWeight;
...
if ~isempty(nonStaticFusionWeightFactor)
    experimentOverrides.stableReliabilityNonStaticFusionWeightFactor = ...
        nonStaticFusionWeightFactor;
end
if ~isempty(maxNonStaticFusionWeight)
    experimentOverrides.stableReliabilityMaxNonStaticFusionWeight = ...
        maxNonStaticFusionWeight;
end
...
summaryPath = fullfile(scriptDir, sprintf( ...
    'topology_stable_reliability_seed%d_len%d_%s.mat', ...
    seed, simulationLength, timestamp));

fprintf('\nStable reliability-only topology recovery seed run\n');
...
if ~isempty(nonStaticFusionWeightFactor) || ~isempty(maxNonStaticFusionWeight)
    fprintf('Non-static fusion factor: %s, cap: %s\n', ...
        formatOptionalScalar(nonStaticFusionWeightFactor), ...
        formatOptionalScalar(maxNonStaticFusionWeight));
end
...
fprintf('\nStable reliability-only topology recovery result\n');
fprintf('Report: %s\n', reportPath);
fprintf('Summary: %s\n', summaryPath);
fprintf('Arm names:\n');
...
function text = formatOptionalScalar(value)
if isempty(value)
    text = 'default';
