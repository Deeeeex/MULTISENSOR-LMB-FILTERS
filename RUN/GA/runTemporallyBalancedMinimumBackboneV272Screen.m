function [reportPath, result] = ...
        runTemporallyBalancedMinimumBackboneV272Screen(options)
% RUNTEMPORALLYBALANCEDMINIMUMBACKBONEV272SCREEN One causal M24 arm.

if nargin < 1 || isempty(options)
    options = struct();
end
protocol = getTemporallyBalancedMinimumBackboneV272ScreenProtocol();
...
if isempty(tracePath) || exist(tracePath, 'file') ~= 2 || ...
    ...
    error('TemporallyBalancedBackboneV272Screen:InvalidRunRequest', ...
        'V272 requires the completed V259 trace and a scalar resume flag.');
end
gitState = resolveResearchGitState();
if gitState.trackedWorktreeDirty || ...
        ~isempty(gitState.untrackedSourceFiles)
    error('TemporallyBalancedBackboneV272Screen:DirtySource', ...
        'Official V272 execution requires clean source.');
end

loaded = load(tracePath, 'result', 'behaviorBundle');
if ~isfield(loaded, 'result') || ~isfield(loaded, 'behaviorBundle')
    error('TemporallyBalancedBackboneV272Screen:MalformedTrace', ...
...
outputRoot = char(getField( ...
    options, 'outputRoot', protocol.screenOutputRoot));
if ~isAbsolutePath(outputRoot)
    outputRoot = fullfile(repoRoot, outputRoot);
end
if exist(outputRoot, 'dir') ~= 7, mkdir(outputRoot); end
matPath = fullfile(outputRoot, ...
    'TEMPORALLY_BALANCED_MINIMUM_BACKBONE_V272_SCREEN.mat');
reportPath = fullfile(outputRoot, ...
    'TEMPORALLY_BALANCED_MINIMUM_BACKBONE_V272_SCREEN.md');
if resume && exist(matPath, 'file') == 2
    ...
    if isfield(checkpoint, 'result') && ...
        ...
        fprintf('Reused completed V272 continuation: %s\n', reportPath);
        return;
    end
end
...
config = buildTemporallyBalancedMinimumBackboneV272Config(inputs.config);
config = attachContinuation(config, initialPosteriors, initialHistory, ...
    protocol.continuationStartTime, tracePath);
execution = buildTemporallyBalancedMinimumBackboneV272ExecutionContext( ...
...
fprintf('V272 continuation, t=%d--%d ...\n', ...
...
result.contractVersion = ...
    'temporally-balanced-minimum-backbone-v272-screen-result-v1';
...
result.nextMethodDecision = conditionalValue( ...
    ...
    'run-one-full-m24-arm-then-independent-x36', ...
    'close-or-revise-temporal-balancing-without-outcome-tuning');
result.completedAt = datestr(now, 31);
result.matPath = matPath;
result.reportPath = reportPath;
result.developmentEvidenceOnly = true;
result.validationClaimAllowed = false;
result.evidenceBoundary = protocol.screenEvidenceBoundary;
save('-mat7-binary', matPath, 'result');
writeReport(reportPath, result);
fprintf(['V272 complete: E/R/C %+.3f%%/%+.3f%%/%+.3f%%; ', ...
...
valid = isstruct(trace) && isscalar(trace) && ...
    ...
    strcmp(bundle.presetName, protocol.screenPresetName) && ...
    ismember(bundle.seed, protocol.allowedSeeds) && ...
    numel(bundle.posteriorSnapshots) >= ...
        protocol.continuationEndTime && ...
    numel(bundle.preDecisionTopologyHistoryByTime) >= ...
        protocol.continuationEndTime;
if ~valid
    error('TemporallyBalancedBackboneV272Screen:TraceContract', ...
        'The V259 trace does not satisfy the V272 source contract.');
end
end
...
arm = scoreMatrices(eospa, rmse, cardinality, config);
arm.name = 'V272 temporally balanced minimum backbone';
...
routePages = logical(diagnostics.topologyActiveEdge(:, :, times));
changed = false(1, numel(times));
for localTime = 2:numel(times)
    changed(localTime) = ~isequal( ...
        routePages(:, :, localTime), routePages(:, :, localTime - 1));
end
arm.routeChangeFraction = mean(changed(2:end));
arm.routeScheduleMetadataBytesIncluded = false;
[arm.eventFormationEospa, arm.eventFormationRmse, ...
 arm.eventFormationCardinality] = eventFormationMetrics( ...
    eospa, rmse, cardinality, times, config, protocol);
arm.completed = true;
...
passed = candidate.completed && candidate.physicalFeasibilityPassed && ...
    ...
    isfinite(candidate.routeChangeFraction) && ...
    islogical(comparison.shortHorizonGatePassed);
if ~passed
    error('TemporallyBalancedBackboneV272Screen:RuntimeContract', ...
        'The V272 continuation violated its route or evidence contract.');
end
end
...
fid = fopen(path, 'w');
if fid < 0
    error('TemporallyBalancedBackboneV272Screen:ReportOpen', ...
        'Unable to write the V272 report.');
end
...
fprintf(fid, '- Candidate route-change fraction: `%.3f`\n', ...
    candidate.routeChangeFraction);
fprintf(fid, ['- Route-schedule metadata bytes included in payload ', ...
    'accounting: `%d`\n'], ...
    candidate.routeScheduleMetadataBytesIncluded);
fprintf(fid, '- Mechanism / full-M24 authorization: `%d / %d`\n\n', ...
    result.actionMechanismPassed, result.fullM24RunAuthorized);
fprintf(fid, ['| Arm | E-OSPA | RMSE | Cardinality error | Consistency | ', ...
    'F4 event E / RMSE | Window bytes | Spliced static saving |\n']);
fprintf(fid, '|:--|--:|--:|--:|--:|:--|--:|--:|\n');
writeArm(fid, base);
...
fprintf(fid, '- Cardinality-error change vs V242: `%+.3f`\n', ...
    candidate.meanAbsoluteCardinalityError - ...
        base.meanAbsoluteCardinalityError);
...
if result.actionMechanismPassed
    fprintf(fid, ['The reliability-constrained temporal embedding ', ...
        'improves the diagnosed F4 event and the registered network ', ...
        'metrics while retaining a positive spliced byte saving. Run ', ...
        'one complete M24 candidate without changing the policy.\n\n']);
else
    fprintf(fid, ['The temporal embedding does not jointly repair the ', ...
        'opened event and network metrics. Its lower conditional RMSE ', ...
        'coincides with higher cardinality error, so it cannot be ', ...
        'interpreted as a localization gain. Close the frozen policy or ', ...
        'revise its mechanism without selecting parameters on this ', ...
        'tracking outcome.\n\n']);
end
...
fprintf(fid, ['| %s | %.3f | %.3f | %.3f | %.3f | %.3f / %.3f | ', ...
    ...
    arm.positionRmse, arm.meanAbsoluteCardinalityError, ...
    arm.interFormationPositionOspa, ...
    arm.eventFormationEospa, arm.eventFormationRmse, ...
    arm.windowAttemptedPayloadBytes, ...
