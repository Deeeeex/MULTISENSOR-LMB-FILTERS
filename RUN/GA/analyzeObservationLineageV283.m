...
for first = 1:5:T
    last = min(first+4, T); a = perTime(first:last);
    weakCount = sum([a.weakBeforeSpatialCount]);
    noStrongCount = weakCount - sum([a.strongInputWeakPoolCount]);
    fprintf(fid, '| %d--%d | %d | %d | %d | %.3f%% |\n', ...
        first, last, sum([a.labelFusionCount]), weakCount, noStrongCount, ...
        100*noStrongCount/max(weakCount, 1));
end
fprintf(fid, ['\nA weak pool means weighted input log odds below zero, before spatial ', ...
    ...
    'cannot distinguish detection from missed detection. Low late-prefix ', ...
    'never-informed mass does not support ongoing untouched-prior dilution ', ...
    'as a complete explanation of the late weak pools. It does not exclude ', ...
    'a lasting cold-start effect: an early prior-handling intervention could ', ...
    'change the subsequent trajectory and has not been evaluated. Weak-history ', ...
    'attenuation and delayed fresh measurements also remain possible. Neither ', ...
    'geometry-only censoring nor a new fusion rule is validated here.\n']);
