function [reportPath, result] = ...
    generateSignedMergeSplitOpportunityV69Caches(options)
% GENERATESIGNEDMERGESPLITOPPORTUNITYV69CACHES M24/X36 source runs.
...
result.contractVersion = ...
    'signed-merge-split-opportunity-v69-cache-result-v1';
...
matPath = fullfile(outputRoot, ...
    'SIGNED_MERGE_SPLIT_OPPORTUNITY_V69_CACHE_RESULT.mat');
reportPath = fullfile(outputRoot, ...
    'SIGNED_MERGE_SPLIT_OPPORTUNITY_V69_CACHE_RESULT.md');
result.matPath = matPath;
result.reportPath = reportPath;
save('-mat7-binary', matPath, 'result');
writeReport(reportPath, result);
fprintf('V69 merge-split reference caches: %s\n', reportPath);
end
...
if fid < 0
    error('SignedMergeSplitV69:WriteFailed', ...
        'Could not create the V69 cache report.');
end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, '# V69 M24/X36 merge-split reference caches\n\n');
