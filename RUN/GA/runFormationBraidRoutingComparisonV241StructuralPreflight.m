...
observableRuntimeContextPassed = checkObservableRuntimeContext( ...
    config, graph, pDrop, identity);
...
result.observableRuntimeContextPassed = observableRuntimeContextPassed;
result.arms = arms;
result.gatePassed = validation.isValid && ...
    observableRuntimeContextPassed && ...
...
function passed = checkObservableRuntimeContext( ...
        config, graph, pDrop, identity)
% The runtime observable-context sanitizer intentionally exposes only
% formation membership, not the preset name or the planned scene.  Exercise
% that exact model boundary before a long tracking arm is allowed to start.
...
context.model = struct('dynamicTopologyScenario', struct( ...
    'config', struct('sensorGroupIds', ...
        reshape(config.sensorGroupIds, 1, []))));
[adjacency, details] = ...
    selectCausalMinimalEditFormationTreeV240Policy(context);
passed = nnz(adjacency) == 2 * config.numberOfSensors && ...
    isscalar(getField(details, 'objective', NaN)) && ...
    ~logical(getField(details, 'truthUsed', true)) && ...
    ~logical(getField(details, 'futureOutcomeUsed', true));
end
...
fprintf(fid, '- Observable runtime context passed: `%d`\n', ...
    result.observableRuntimeContextPassed);
