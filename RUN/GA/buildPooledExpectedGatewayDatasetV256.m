...
for pathIdx = 1:numel(allPaths)
    ...
    for windowIdx = 1:numel(oracle.windows)
        ...
        desiredBank = buildRestrictedBank( ...
            context, inputs, teacher, protocol);
        desiredAssignments = { ...
...
if numel(records) ~= expectedRecordCount || ...
    ...
    error('PooledExpectedGatewayV256:DatasetShape', ...
        ['Every M24 window must retain at least one physical action; ', ...
         'unavailable directed formation slots remain non-actionable.']);
end
...
function bank = buildRestrictedBank(context, inputs, teacher, protocol)
...
options = struct('singleArcOnly', true, ...
    ...
    'minimumCandidateCount', protocol.minimumActionsPerWindow + 1, ...
...
bank = restrictBudgetedLocalGatewayTeacherCandidateBankV255( ...
    bank, teacher, struct('requireEveryDirectedSlot', false, ...
        'minimumCandidateCount', protocol.minimumActionsPerWindow + 1));
end
