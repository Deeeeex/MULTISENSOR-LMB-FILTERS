% RUNMULTISENSORFILTERS_FORMATION_4PLUS4_THREEWAVES - 4+4 sensor formations + three target streams demo
close all; clc;
...
gifPath = fullfile(outputDir, 'formation_animation_4plus4_threewaves.gif');
frameSkip = 1;
frameDelay = 0.08;
fovHalfAngleDeg = 60;
fovRange = 60000;
...
gifPathBase = fullfile(outputDir, 'formation_animation_4plus4_threewaves_base.gif');
gifPathAdaptive = fullfile(outputDir, 'formation_animation_4plus4_threewaves_adaptive.gif');
...
%% Target formations (3-3-4), three streams: top-down, right-left, bottom-up
...
function ensureGifDir(pathStr)
    if nargin < 1 || isempty(pathStr)
        return;
    end
    outDir = fileparts(pathStr);
...
function targetBirthStates = buildTargetBirthStates()
    groupCenters = [0, 80, 0; 80, 0, -80]; % top-center, right-center, bottom-center
