function [reportPath, result] = ...
        runRiskEpisodeLatchedFormationShortcutV262Continuation(options)
% RUNRISKEPISODELATCHEDFORMATIONSHORTCUTV262CONTINUATION One causal M24 arm.
...
if isempty(tracePath) || exist(tracePath, 'file') ~= 2 || ...
    ...
    error('RiskEpisodeLatchV262:InvalidRunRequest', ...
        'V262 requires the completed V259 trace and a scalar resume flag.');
end
gitState = resolveResearchGitState();
if gitState.trackedWorktreeDirty || ...
        ~isempty(gitState.untrackedSourceFiles)
    error('RiskEpisodeLatchV262:DirtySource', ...
        'Official V262 execution requires clean source.');
end

loaded = load(tracePath, 'result', 'behaviorBundle');
if ~isfield(loaded, 'result') || ~isfield(loaded, 'behaviorBundle')
    error('RiskEpisodeLatchV262:MalformedTrace', ...
...
matPath = fullfile(outputRoot, ...
    'RISK_EPISODE_LATCHED_FORMATION_SHORTCUT_V262_CONTINUATION.mat');
reportPath = fullfile(outputRoot, ...
    'RISK_EPISODE_LATCHED_FORMATION_SHORTCUT_V262_CONTINUATION.md');
if resume && exist(matPath, 'file') == 2
    ...
    if isfield(checkpoint, 'result') && ...
        ...
        fprintf('Reused completed V262 continuation: %s\n', reportPath);
        return;
    end
end
...
config = buildRiskEpisodeLatchedFormationShortcutV262Config(inputs.config);
config = attachContinuation(config, initialPosteriors, initialHistory, ...
    protocol.continuationStartTime, tracePath);
execution = buildRiskEpisodeLatchedFormationShortcutV262ExecutionContext( ...
...
fprintf('V262 continuation, t=%d--%d ...\n', ...
...
runtimeContract = evaluateCandidateContract( ...
    candidate, comparison, inputs.config);

result = struct();
result.contractVersion = ...
    'risk-episode-latched-formation-shortcut-v262-result-v1';
...
result.nextMethodDecision = conditionalValue( ...
    ...
    'close-single-donor-latch-and-formulate-multi-source-risk-tree');
...
writeReport(reportPath, result);
fprintf(['V262 complete: E/R/C %+.3f%%/%+.3f%%/%+.3f%%; ', ...
...
fprintf('V262 report: %s\n', reportPath);
fprintf(['V262 runtime contract [complete physical strong truth future ', ...
    'messages initiated active retained suppressed typed] = %s\n'], ...
    mat2str(runtimeContract.flags));
if ~runtimeContract.passed
    error('RiskEpisodeLatchV262:RuntimeContract', ...
        'The V262 continuation contract failed: %s', ...
        mat2str(runtimeContract.flags));
end
end
...
valid = isstruct(trace) && isscalar(trace) && ...
    ...
    numel(bundle.preDecisionTopologyHistoryByTime) >= ...
        protocol.continuationEndTime;
if ~valid
    error('RiskEpisodeLatchV262:TraceContract', ...
        'The V259 trace does not satisfy the V262 source contract.');
end
end
...
arm.shortcutInitiationTimes = zeros(1, 0);
arm.shortcutActiveTimes = zeros(1, 0);
arm.completed = true;
end
...
arm = scoreMatrices(eospa, rmse, cardinality, config);
arm.name = 'V262 risk-episode latched formation shortcut';
...
arm.shortcutInitiatedByTime = false(1, numel(times));
arm.shortcutActiveByTime = false(1, numel(times));
arm.latchRetainedByTime = false(1, numel(times));
arm.latchReleasedByTime = false(1, numel(times));
arm.donorSwitchSuppressedByTime = false(1, numel(times));
arm.instantaneousProposedDonorIdByTime = zeros(1, numel(times));
arm.targetFormationIdByTime = zeros(1, numel(times));
arm.donorFormationIdByTime = zeros(1, numel(times));
arm.hopReductionByTime = zeros(1, numel(times));
arm.referenceHopDistanceByTime = nan(1, numel(times));
arm.physicalHopDistanceByTime = nan(1, numel(times));
for localTime = 1:numel(times)
    schedule = diagnostics.topologyPolicyScheduleCertificate{ ...
        times(localTime)};
    arm.shortcutInitiatedByTime(localTime) = logical(getField( ...
        schedule, 'shortcutInitiated', false));
    arm.shortcutActiveByTime(localTime) = logical(getField( ...
        schedule, 'shortcutActive', false));
    arm.latchRetainedByTime(localTime) = logical(getField( ...
        schedule, 'riskEpisodeLatchRetained', false));
    arm.latchReleasedByTime(localTime) = logical(getField( ...
        schedule, 'riskEpisodeLatchReleased', false));
    arm.donorSwitchSuppressedByTime(localTime) = logical(getField( ...
        schedule, 'instantaneousDonorSwitchSuppressed', false));
    arm.instantaneousProposedDonorIdByTime(localTime) = getField( ...
        schedule, 'instantaneousProposedDonorFormationId', 0);
    ...
    arm.hopReductionByTime(localTime) = getField( ...
        schedule, 'formationHopReduction', 0);
    arm.referenceHopDistanceByTime(localTime) = getField( ...
        schedule, 'referenceFormationHopDistance', NaN);
    arm.physicalHopDistanceByTime(localTime) = getField( ...
        schedule, 'physicalFormationHopDistance', NaN);
end
arm.shortcutInitiationTimes = times(arm.shortcutInitiatedByTime);
arm.shortcutActiveTimes = times(arm.shortcutActiveByTime);
arm.latchRetainedTimes = times(arm.latchRetainedByTime);
arm.latchReleasedTimes = times(arm.latchReleasedByTime);
arm.donorSwitchSuppressedTimes = ...
    times(arm.donorSwitchSuppressedByTime);
...
function contract = evaluateCandidateContract(candidate, comparison, config)
formationCount = numel(unique(config.sensorGroupIds));
minimumMessages = config.numberOfSensors + 2 * (formationCount - 1);
contract = struct();
contract.names = {'completed', 'physical', 'strong', 'truth-free', ...
    'future-free', 'message-parity', 'initiated', 'active', ...
    'retained', 'switch-suppressed', 'gate-typed'};
contract.flags = logical([candidate.completed, ...
    ...
    all(candidate.messageCountByTime == minimumMessages), ...
    ~isempty(candidate.shortcutInitiationTimes), ...
    any(candidate.shortcutActiveByTime), ...
    any(candidate.latchRetainedByTime), ...
    any(candidate.donorSwitchSuppressedByTime), ...
    islogical(comparison.shortHorizonGatePassed)]);
contract.passed = all(contract.flags);
end

function writeReport(path, result)
fid = fopen(path, 'w');
if fid < 0
    error('RiskEpisodeLatchV262:ReportOpen', ...
        'Unable to write the V262 report.');
end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
base = result.v242;
candidate = result.candidate;
comparison = result.comparison;
fprintf(fid, '# V262 risk-episode latched formation shortcut\n\n');
...
fprintf(fid, '- Shortcut initiated / active pages: `%s / %s`\n', ...
    mat2str(candidate.shortcutInitiationTimes), ...
    mat2str(candidate.shortcutActiveTimes));
fprintf(fid, '- Latch retained / released pages: `%s / %s`\n', ...
    mat2str(candidate.latchRetainedTimes), ...
    mat2str(candidate.latchReleasedTimes));
fprintf(fid, '- Donor switches suppressed at: `%s`\n', ...
    mat2str(candidate.donorSwitchSuppressedTimes));
fprintf(fid, '- Mechanism / full-M24 authorization: `%d / %d`\n\n', ...
    result.actionMechanismPassed, result.fullM24RunAuthorized);
fprintf(fid, '- Runtime contract flags: `%s` (passed `%d`)\n\n', ...
    mat2str(result.runtimeContract.flags), ...
    result.runtimeContract.passed);
...
if result.actionMechanismPassed
    fprintf(fid, ['The risk-episode latch removes V261 donor chatter ', ...
        'and repairs the diagnosed F4 event under the registered network ', ...
        'and formation guards. Run one complete M24 candidate without ', ...
        'changing the risk, donor or release rules.\n\n']);
else
    fprintf(fid, ['Suppressing single-donor chatter does not close all ', ...
        'registered guards. Close this latch mechanism and formulate a ', ...
        'multi-source risk tree before considering a learned ranker.\n\n']);
end
...
complete = isstruct(result) && isscalar(result) && ...
    ...
    isfield(result, 'runtimeContract') && result.runtimeContract.passed;
end
