function [reportPath, summary] = runMultisensorFilters_formation_4plus4_AssociationAmbiguityCompare( ...
...
% Compare three-factor adaptive weighting with and without association ambiguity.
...
baseOverrides = mergeStructFields(struct( ...
    'useAssociationAmbiguity', false, ...
    'useNIS', false, ...
    'useHistory', false, ...
    'existenceConfidenceMinScore', 0.85, ...
    'existenceConfidencePower', 2.0, ...
    'associationAmbiguityMinScore', 0.85, ...
    'associationAmbiguityPower', 1.0), adaptiveFusionOverrides);
...
if writeReport
    ...
    reportPath = fullfile(reportDir, sprintf('GA_ASSOCIATION_AMBIGUITY_COMPARE_%s.md', timestamp));
...
fprintf(fid, '# GA Association Ambiguity Comparison (%s)\n\n', datestr(now, 'yyyy-mm-dd HH:MM:SS'));
fprintf(fid, 'Comparison order: three-factor baseline -> +association ambiguity\n\n');
fprintf(fid, '## Run Config\n');
fprintf(fid, '- Trials: %d\n', numberOfTrials);
fprintf(fid, '- baseSeed: %d (fixed=%d)\n\n', baseSeed, useFixedSeed);

fprintf(fid, '## Arm Configs\n');
fprintf(fid, '### Three-factor baseline\n');
fprintf(fid, '- useAssociationAmbiguity: %d\n', getField(summary.baseOverrides, 'useAssociationAmbiguity', false));
fprintf(fid, '- associationAmbiguityMinScore: %.3f\n', getField(summary.baseOverrides, 'associationAmbiguityMinScore', 0));
fprintf(fid, '- associationAmbiguityPower: %.3f\n\n', getField(summary.baseOverrides, 'associationAmbiguityPower', 0));
fprintf(fid, '### +association ambiguity\n');
fprintf(fid, '- useAssociationAmbiguity: %d\n', getField(summary.experimentOverrides, 'useAssociationAmbiguity', false));
...
fprintf(fid, '| +association ambiguity | %.6f | %.6f | %.6f |\n\n', ...
