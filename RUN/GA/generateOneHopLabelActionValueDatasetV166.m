...
for scheduleIdx = 1:numel(repairPages)
    ...
    for receiverIdx = reshape(receivers, 1, [])
        cellId = cellId + 1;
        fprintf('V166 cell %d/30: t=%d F=%d receiver=%d\n', ...
            cellId, currentTime, groupIds(receiverIdx), receiverIdx);
        ...
        for candidate = reshape(shortlisted, 1, [])
            ...
            row = emptyRow();
            row.cellId = cellId;
            row.split = splitFor(currentTime, groupIds(receiverIdx));
            row.page = pageIdx;
        ...
        cellRecord.split = splitFor( ...
            currentTime, groupIds(receiverIdx));
...
reportPath = fullfile(outputRoot, ...
    'ONE_HOP_LABEL_ACTION_VALUE_DATASET_V166.md');
...
function value = splitFor(currentTime, formation)
if currentTime <= 78
    value = 1;
elseif formation == 3
    value = 2;
...
fprintf(fid, '# V166 one-hop label-action value dataset\n\n');
