function [reportPath, result] = ...
        runRiskRootedMultiSourceTreeV263Continuation(options)
% RUNRISKROOTEDMULTISOURCETREEV263CONTINUATION One causal M24 arm.
...
if isempty(tracePath) || exist(tracePath, 'file') ~= 2 || ...
    ...
    error('RiskRootedMultiSourceV263:InvalidRunRequest', ...
        'V263 requires the completed V259 trace and a scalar resume flag.');
end
gitState = resolveResearchGitState();
if gitState.trackedWorktreeDirty || ...
        ~isempty(gitState.untrackedSourceFiles)
    error('RiskRootedMultiSourceV263:DirtySource', ...
        'Official V263 execution requires clean source.');
end

loaded = load(tracePath, 'result', 'behaviorBundle');
if ~isfield(loaded, 'result') || ~isfield(loaded, 'behaviorBundle')
    error('RiskRootedMultiSourceV263:MalformedTrace', ...
...
matPath = fullfile(outputRoot, ...
    'RISK_ROOTED_MULTI_SOURCE_TREE_V263_CONTINUATION.mat');
reportPath = fullfile(outputRoot, ...
    'RISK_ROOTED_MULTI_SOURCE_TREE_V263_CONTINUATION.md');
if resume && exist(matPath, 'file') == 2
    ...
    if isfield(checkpoint, 'result') && ...
        ...
        fprintf('Reused completed V263 continuation: %s\n', reportPath);
        return;
    end
end
...
config = buildRiskRootedMultiSourceTreeV263Config(inputs.config);
config = attachContinuation(config, initialPosteriors, initialHistory, ...
    protocol.continuationStartTime, tracePath);
execution = buildRiskRootedMultiSourceTreeV263ExecutionContext( ...
...
fprintf('V263 continuation, t=%d--%d ...\n', ...
...
result.contractVersion = ...
    'risk-rooted-multi-source-tree-v263-result-v1';
...
result.nextMethodDecision = conditionalValue( ...
    ...
    'test-one-edge-risk-cycle-before-label-selective-edge-trust');
...
writeReport(reportPath, result);
fprintf(['V263 complete: E/R/C %+.3f%%/%+.3f%%/%+.3f%%; ', ...
...
valid = isstruct(trace) && isscalar(trace) && ...
    ...
    numel(bundle.preDecisionTopologyHistoryByTime) >= ...
        protocol.continuationEndTime;
if ~valid
    error('RiskRootedMultiSourceV263:TraceContract', ...
        'The V259 trace does not satisfy the V263 source contract.');
end
end
...
arm.rootedTreeInitiationTimes = zeros(1, 0);
arm.rootedTreeActiveTimes = zeros(1, 0);
arm.completed = true;
end
...
arm = scoreMatrices(eospa, rmse, cardinality, config);
arm.name = 'V263 risk-rooted multi-source tree';
...
arm.rootedTreeInitiatedByTime = false(1, numel(times));
arm.rootedTreeActiveByTime = false(1, numel(times));
arm.multiSourceShortestPathPassedByTime = false(1, numel(times));
arm.eligibleSourceCountByTime = zeros(1, numel(times));
arm.initiallyShortenedSourceCountByTime = zeros(1, numel(times));
arm.targetFormationIdByTime = zeros(1, numel(times));
arm.eligibleSourceFormationIdsByTime = ...
    repmat({zeros(1, 0)}, 1, numel(times));
arm.baseSourceHopDistancesByTime = ...
    repmat({zeros(1, 0)}, 1, numel(times));
arm.rootedSourceHopDistancesByTime = ...
    repmat({zeros(1, 0)}, 1, numel(times));
arm.physicalSourceHopDistancesByTime = ...
    repmat({zeros(1, 0)}, 1, numel(times));
for localTime = 1:numel(times)
    schedule = diagnostics.topologyPolicyScheduleCertificate{ ...
        times(localTime)};
    arm.rootedTreeInitiatedByTime(localTime) = logical(getField( ...
        schedule, 'riskRootedTreeInitiated', false));
    arm.rootedTreeActiveByTime(localTime) = logical(getField( ...
        schedule, 'riskRootedTreeActive', false));
    arm.multiSourceShortestPathPassedByTime(localTime) = ...
        logical(getField( ...
            schedule, 'multiSourceShortestPathPassed', false));
    arm.eligibleSourceCountByTime(localTime) = getField( ...
        schedule, 'eligibleSourceCount', 0);
    arm.initiallyShortenedSourceCountByTime(localTime) = getField( ...
        schedule, 'initiallyShortenedSourceCount', 0);
    arm.targetFormationIdByTime(localTime) = getField( ...
        schedule, 'selectedLocalizationFormationId', 0);
    arm.eligibleSourceFormationIdsByTime{localTime} = getField( ...
        schedule, 'eligibleSourceFormationIds', zeros(1, 0));
    arm.baseSourceHopDistancesByTime{localTime} = getField( ...
        schedule, 'baseSourceHopDistances', zeros(1, 0));
    arm.rootedSourceHopDistancesByTime{localTime} = getField( ...
        schedule, 'rootedSourceHopDistances', zeros(1, 0));
    arm.physicalSourceHopDistancesByTime{localTime} = getField( ...
        schedule, 'physicalSourceHopDistances', zeros(1, 0));
end
arm.rootedTreeInitiationTimes = times(arm.rootedTreeInitiatedByTime);
arm.rootedTreeActiveTimes = times(arm.rootedTreeActiveByTime);
...
passed = candidate.completed && candidate.physicalFeasibilityPassed && ...
    ...
    ~isempty(candidate.rootedTreeInitiationTimes) && ...
    any(candidate.rootedTreeActiveByTime) && ...
    all(candidate.multiSourceShortestPathPassedByTime( ...
        candidate.rootedTreeActiveByTime)) && ...
    max(candidate.eligibleSourceCountByTime) >= ...
        protocol.minimumEligibleSourceCount && ...
    max(candidate.initiallyShortenedSourceCountByTime) >= ...
        protocol.minimumInitiallyShortenedSourceCount && ...
    islogical(comparison.shortHorizonGatePassed);
if ~passed
    error('RiskRootedMultiSourceV263:RuntimeContract', ...
        'The V263 continuation violated its route or evidence contract.');
end
end
...
fid = fopen(path, 'w');
if fid < 0
    error('RiskRootedMultiSourceV263:ReportOpen', ...
        'Unable to write the V263 report.');
end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
base = result.v242;
candidate = result.candidate;
comparison = result.comparison;
fprintf(fid, '# V263 risk-rooted multi-source tree\n\n');
...
fprintf(fid, '- Rooted tree initiated / active pages: `%s / %s`\n', ...
    mat2str(candidate.rootedTreeInitiationTimes), ...
    mat2str(candidate.rootedTreeActiveTimes));
fprintf(fid, '- Maximum eligible / initially shortened sources: `%d / %d`\n', ...
    max(candidate.eligibleSourceCountByTime), ...
    max(candidate.initiallyShortenedSourceCountByTime));
fprintf(fid, '- Active pages componentwise shortest: `%d`\n', ...
    all(candidate.multiSourceShortestPathPassedByTime( ...
        candidate.rootedTreeActiveByTime)));
...
if result.actionMechanismPassed
    fprintf(fid, ['The same-budget risk-rooted tree repairs the ', ...
        'diagnosed F4 event while preserving all registered guards. ', ...
        'Run one complete M24 candidate without changing the risk or ', ...
        'multi-source thresholds.\n\n']);
else
    fprintf(fid, ['The componentwise shortest risk-rooted tree does ', ...
        'not repair the diagnosed F4 event under the registered guards. ', ...
        'Close tree-only routing and test one minimally redundant ', ...
        'risk-cycle edge before changing label-wise trust or training ', ...
        'a learned ranker.\n\n']);
end
