...
| C2 | 该帧多数被检查来源为单分量，遗漏其他模态不是主要解释。 | E2 | 462/503 个去重来源—标签—真值位置组合为单分量，不外推全部时间。 |
...
503 个去重来源—标签—真值位置组合中，仅 41 个来自多分量对象。平均
完整风险约 78,327 平方米，实际平方误差约 6,800 平方米；对数风险—误差
相关系数约 0.232。共享观测和此前融合使其并非独立校准样本，不能据此断言风险普遍
...
统计修正：初版只按来源节点与真值位置去重，误将同一来源的不同输出标签
合为一条。版本 v2 保留 `(sender, birthTime, birthLocation, truthIndex)`，
session 40149 用同一命令重新计算快照，退出 0。来源描述从 273 对更正为
503 个组合，单分量占比从 93.8% 更正为 91.8%；全部来源池、编队的选源
RMSE 表逐项不变。已明确记录修正，不改已推送提交。
...
同一符号标签在不同节点的几何匹配也可能指向不同目标；不能把“按相同
标签找到更近的均值”当成目标身份与关联问题已经解决。
