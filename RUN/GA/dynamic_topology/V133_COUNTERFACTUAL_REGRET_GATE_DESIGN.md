...
## Closed-loop boundary discovered before teacher generation

V133 remains the atomic one-formation counterfactual and static-carrier baseline. Its initial teacher design must not be promoted directly to a full-episode policy: all examples begin from static-carrier states, protection-history inputs are zero, and the deployed controller would generally create overlapping actions and visit a shifted posterior distribution. Existing M24/X36 mechanism evidence also shows that material gains arise from protecting formation sets rather than isolated receivers. The successor V134 therefore uses set-valued one-page pulses, a propagation-diameter recovery interval, a deployable-action-space oracle gate and on-policy dataset aggregation. No official V133 teacher shards should be generated after this boundary was identified.
