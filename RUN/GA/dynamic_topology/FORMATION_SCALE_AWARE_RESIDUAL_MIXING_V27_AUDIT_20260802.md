...
## Why the existing posterior-risk score is insufficient

For a Bernoulli label, the current proxy is

`min(r, (1-r) + r v)`, where `v = min(trace(P)/c^2, 1)`.

When KLA conflict suppresses `r`, the empty-set term `r` also shrinks. The score can therefore reward deleting a reference-supported label. It measures internal decision confidence, not an explicit guarantee that plausible reference labels are retained. The v27 structural bank did not use this proxy. A later post-hoc re-score does assign a small negative Bayes-risk advantage to every v27 candidate, so it would be inaccurate to say that the proxy missed the direction in this particular state. Its mathematical blind spot and the earlier cross-state v10 instability nevertheless mean it cannot serve as the required retention certificate. V27 also shows that a disagreement guard cannot replace that certificate: agreement can improve while all nodes move toward the same wrong cardinality.
