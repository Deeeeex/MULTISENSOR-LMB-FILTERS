...
expectedReceiverMode = getField( ...
    options, 'expectedMissingLabelFusionMode', '');
required = { ...
...
actualReceiverMode = 'support-renormalized';
if isfield(bundle, 'missingLabelFusionMode')
    actualReceiverMode = bundle.missingLabelFusionMode;
elseif isfield(bundle, 'behaviorConfigSnapshot') && ...
    ...
    actualReceiverMode = ...
        bundle.behaviorConfigSnapshot.missingLabelFusionMode;
end
if ~isempty(expectedReceiverMode) && ...
        (~ischar(actualReceiverMode) || ...
         ~strcmp(actualReceiverMode, expectedReceiverMode))
    error('Formation H=3 observable-state receiver semantics mismatch.');
end
...
state.missingLabelFusionMode = actualReceiverMode;
