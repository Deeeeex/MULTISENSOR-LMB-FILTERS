...
meanGainByCandidate = nan(1, numel(candidateRecords));
worstSensorGainByCandidate = nan(1, numel(candidateRecords));
minimumFormationGainByCandidate = nan(1, numel(candidateRecords));
consensusGainByCandidate = nan(1, numel(candidateRecords));
attemptedByteSavingByCandidate = nan(1, numel(candidateRecords));
structurePassByCandidate = false(1, numel(candidateRecords));
for idx = 1:numel(candidateRecords)
    ...
    meanGainByCandidate(idx) = relativeGain( ...
        reference.meanEospa, candidate.meanEospa);
    worstSensorGainByCandidate(idx) = relativeGain( ...
        reference.worstSensorEospa, candidate.worstSensorEospa);
    minimumFormationGainByCandidate(idx) = formationGain;
    consensusGainByCandidate(idx) = relativeGain( ...
        reference.consensusOspa, candidate.consensusOspa);
    attemptedByteSavingByCandidate(idx) = relativeSaving( ...
        reference.attemptedBytes, candidate.attemptedBytes);
    structurePassByCandidate(idx) = ...
...
summary.dynamicMeanGainPercent = meanGainByCandidate;
summary.dynamicWorstSensorGainPercent = worstSensorGainByCandidate;
summary.dynamicMinimumFormationGainPercent = ...
    minimumFormationGainByCandidate;
summary.dynamicConsensusGainPercent = consensusGainByCandidate;
summary.dynamicAttemptedByteSavingPercent = ...
    attemptedByteSavingByCandidate;
summary.dynamicStructurePass = structurePassByCandidate;
