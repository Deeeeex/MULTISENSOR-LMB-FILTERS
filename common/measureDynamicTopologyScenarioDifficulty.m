...
for targetIdx = 1:targetCount
    ...
    for timeIdx = 1:timeCount
        ...
        [visible, nearestDistance] = visibleFormations( ...
            config, sensorTrajectories, headingSchedule, state, timeIdx);
...
function [visible, nearestDistance] = visibleFormations( ...
    config, sensorTrajectories, headingSchedule, targetState, timeIdx)
...
for formationIdx = 1:config.formationCount
    ...
    for sensorIdx = reshape(sensors, 1, [])
        ...
        inFov = distance <= config.fovRange;
        headingVector = sensorState(3:4);
        if size(headingSchedule, 1) >= sensorIdx && ...
                size(headingSchedule, 2) >= timeIdx && ...
                isfinite(headingSchedule(sensorIdx, timeIdx))
            heading = headingSchedule(sensorIdx, timeIdx);
            headingVector = [cos(heading); sin(heading)];
        end
        headingMagnitude = norm(headingVector);
        if inFov && headingMagnitude > 1e-9 && distance > 1e-9
            cosine = relative' * headingVector / ...
                (distance * headingMagnitude);
            cosine = min(max(cosine, -1), 1);
            inFov = acosd(cosine) <= config.fovHalfAngleDeg;
        end
        visible(formationIdx) = visible(formationIdx) || inFov;
    end
end
end
