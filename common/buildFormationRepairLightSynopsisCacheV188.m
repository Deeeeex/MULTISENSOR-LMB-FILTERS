...
participatingSensorIds = reshape(getField( ...
    options, 'participatingSensorIds', 1:sensorCount), 1, []);
localParticipatingSensorIds = reshape(getField(options, ...
    'localParticipatingSensorIds', participatingSensorIds), 1, []);
fusedParticipatingSensorIds = reshape(getField(options, ...
    'fusedParticipatingSensorIds', participatingSensorIds), 1, []);
recordMode = char(getField(options, 'recordMode', 'v188-full'));
if strcmp(recordMode, 'v216-risk-only')
    bytesPerLabelRecord = 20;
    labelRecordLayout = [ ...
        'uint16 birth-time, uint16 birth-location, uint16 existence, ', ...
        'uint16 evidence, float32 x, float32 y, ', ...
        'float32 position-trace'];
elseif strcmp(recordMode, 'v188-full')
    bytesPerLabelRecord = protocol.lightSynopsisBytesPerLabel;
    labelRecordLayout = [ ...
        'uint16 birth-time, uint16 birth-location, uint16 existence, ', ...
        'uint16 evidence, uint16 support, float32 x, float32 y, ', ...
        'float32 position-trace, uint16 observation-opportunity'];
else
    error('FormationRepairSynopsisV188:InvalidRecordMode', ...
        'The light-synopsis record mode is not registered.');
end
participatingSensorIds = unique( ...
    [localParticipatingSensorIds, fusedParticipatingSensorIds], ...
    'stable');
if ~iscell(fusedPosteriorBySensor) || ...
        ...
        isempty(participatingSensorIds) || ...
        ~validSensorIds(localParticipatingSensorIds, sensorCount) || ...
        ~validSensorIds(fusedParticipatingSensorIds, sensorCount)
...
for sensorIdx = 1:sensorCount
    if ismember(sensorIdx, localParticipatingSensorIds)
        ...
        localBySensor{sensorIdx} = summarizePosterior( ...
            [], model, sensorIdx, currentTime, positionCutoff, ...
            activeExistenceThreshold);
    end
    if strcmp(recordMode, 'v216-risk-only')
        localBySensor{sensorIdx} = stripUnusedFields( ...
            localBySensor{sensorIdx});
    end
    if ismember(sensorIdx, fusedParticipatingSensorIds)
    ...
    if strcmp(recordMode, 'v216-risk-only')
        fusedBySensor{sensorIdx} = stripUnusedFields( ...
            fusedBySensor{sensorIdx});
    end
    if ismember(sensorIdx, participatingSensorIds)
...
cache.bytesPerLabelRecord = bytesPerLabelRecord;
cache.attemptedBytesBySensor = attemptedBytesBySensor;
cache.totalAttemptedBytes = sum(attemptedBytesBySensor);
cache.participatingSensorIds = participatingSensorIds;
cache.participatingSensorCount = numel(participatingSensorIds);
cache.localParticipatingSensorIds = localParticipatingSensorIds;
cache.localParticipatingSensorCount = ...
    numel(localParticipatingSensorIds);
cache.fusedParticipatingSensorIds = fusedParticipatingSensorIds;
cache.fusedParticipatingSensorCount = ...
    numel(fusedParticipatingSensorIds);
cache.scopedSynopsis = ...
    numel(participatingSensorIds) < sensorCount;
cache.recordMode = recordMode;
cache.labelRecordLayout = labelRecordLayout;
...
function summary = stripUnusedFields(summary)
summary.associationSupport(:) = 0;
summary.observationOpportunity(:) = 0;
end
...
function valid = validSensorIds(ids, sensorCount)
valid = isnumeric(ids) && isvector(ids) && ~isempty(ids) && ...
    all(isfinite(ids)) && all(ids == round(ids)) && ...
    all(ids >= 1) && all(ids <= sensorCount) && ...
