...
switch mode
    ...
    case 'quality-advantage'
        ...
        scores = reshape(features(:, :, featureIdx), ...
            nodeCount, nodeCount);
    case 'reliability-quality'
        [features, names] = ...
            computeDirectedRoutingFeatures(context);
        reliabilityIdx = requireFeature( ...
            names, 'link_reliability');
        qualityIdx = requireFeature( ...
            names, 'node_quality_advantage');
        reliability = reshape(features(:, :, reliabilityIdx), ...
            nodeCount, nodeCount);
        qualityAdvantage = reshape(features(:, :, qualityIdx), ...
            nodeCount, nodeCount);
        qualityCoefficient = getField( ...
            options, 'qualityCoefficient', 0.25);
        if ~isscalar(qualityCoefficient) || ...
                ~isfinite(qualityCoefficient)
            error('qualityCoefficient must be a finite scalar.');
        end
        scores = reliability + ...
            qualityCoefficient * qualityAdvantage;
        details.qualityCoefficient = qualityCoefficient;
        details.qualityFeature = ...
            'source_quality_minus_receiver_quality';
        details.receiverQualityCancelsWithinReceiverArgmax = true;
        details.receiverQualityAffectsBudgetTruncation = true;
