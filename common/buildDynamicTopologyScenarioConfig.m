...
switch canonicalName
    ...
    case {'m24-formation-fov-formation-braid', ...
    ...
    case {'m24-formation-fov-temporal-coupled-formation-braid', ...
            'm24-temporal-coupled-formation-braid-fov'}
        config = configureM24(config, 'formation-fov');
        config = applyFormationFovSceneStyle( ...
            config, 'temporal-coupled-formation-braid');
    ...
    case {'x36-formation-fov-braided-handover', ...
        ...
        config = configureX36(config, 'formation-fov');
        config = applyFormationFovSceneStyle(config, 'braided-handover');
    case {'x36-formation-fov-formation-braid', ...
    ...
    case {'x36-formation-fov-coupled-formation-braid', ...
        ...
        config = applyFormationFovSceneStyle( ...
            config, 'information-coupled-formation-braid');
    case {'x36-formation-fov-temporal-coupled-formation-braid', ...
            'x36-temporal-coupled-formation-braid-fov'}
        config = configureX36(config, 'formation-fov');
        config = applyFormationFovSceneStyle( ...
            config, 'temporal-coupled-formation-braid');
    ...
    case {'x48-formation-fov-formation-braid', ...
    ...
    case {'x48-formation-fov-coupled-formation-braid', ...
        ...
        config = applyFormationFovSceneStyle( ...
            config, 'information-coupled-formation-braid');
    case {'x48-formation-fov-temporal-coupled-formation-braid', ...
            'x48-temporal-coupled-formation-braid-fov'}
        config = configureX48(config, 'formation-fov');
        config = applyFormationFovSceneStyle( ...
            config, 'temporal-coupled-formation-braid');
    otherwise
...
if isfield(config, 'sceneGeometryVersion') && ...
        strncmp(config.sceneGeometryVersion, ...
            'formation-fov-multistyle-', ...
            numel('formation-fov-multistyle-')) && ...
        ~isempty(fieldnames(overrides))
    error(['Versioned multistyle formation-FoV presets do not accept ', ...
        'runtime overrides. Define a new named scene version instead.']);
end
...
if ~isfield(config, 'formationStochasticRoleUidsByFormation')
    % Freeze the legacy formation-level random-draw role as registered
    % metadata.  The role moves with a physical formation if storage is
    % reordered, preserving both legacy seeded geometry and equivariance.
    config.formationStochasticRoleUidsByFormation = ...
        1:config.formationCount;
end
physicalIdentity = buildDynamicTopologyPhysicalIdentityRegistry(config);
config.formationPhysicalUids = ...
    physicalIdentity.formationPhysicalUids;
config.formationPhysicalUidsBySensor = ...
    physicalIdentity.formationPhysicalUidsBySensor;
config.sensorLocalRoleUidsByFormation = ...
    physicalIdentity.sensorLocalRoleUidsByFormation;
config.sensorLocalRoleUidsBySensor = ...
    physicalIdentity.sensorLocalRoleUidsBySensor;
config.sensorPhysicalUids = physicalIdentity.sensorPhysicalUids;
...
if isfield(config, 'sceneGeometryVersion') && ...
        strcmp(config.sceneGeometryVersion, ...
...
validateMultistyleFormationFovContract(config);
end

function validateMultistyleFormationFovContract(config)
if ~isfield(config, 'sceneGeometryVersion')
    return;
end
if ~strcmp(config.sceneGeometryVersion, ...
        'formation-fov-multistyle-v5')
    error('Unknown multistyle formation-FoV geometry contract.');
end
requiredFields = { ...
    'sceneStyle', 'sceneCalibrationStatus', ...
    'formalValidationAuthorized', 'trackingOutcomeAuthorized', ...
    'sceneContractSha256', ...
    'formationBackboneMode', 'blockageScheduleMode', ...
    'blockageWindowTimes', 'sensorHardwareProfile', ...
    'sensorFovHeadingMode', 'sensorQuality', ...
    'observationSpaceLimits', 'clutterSpatialProfile', ...
    'minimumSensorTargetSeparation'};
if ~all(isfield(config, requiredFields)) || ...
        ~any(strcmp(config.sceneStyle, { ...
            'parallel-convoy', 'orthogonal-crossing', ...
            'linear-relay'})) || ...
        ~isValidMultistyleCalibrationState(config) || ...
        ~isequal(config.trackingOutcomeAuthorized, false) || ...
        ~ischar(config.sceneContractSha256) || ...
        numel(config.sceneContractSha256) ~= 64 || ...
        ~strcmp(config.formationBackboneMode, ...
            'initial-geometry-mst') || ...
        ~strcmp(config.blockageScheduleMode, ...
            'backbone-sequential') || ...
        size(config.blockageWindowTimes, 2) ~= 2 || ...
        isempty(config.blockageWindowTimes) || ...
        any(~isfinite(config.blockageWindowTimes(:))) || ...
        any(config.blockageWindowTimes(:, 1) < 1) || ...
        any(config.blockageWindowTimes(:, 2) < ...
            config.blockageWindowTimes(:, 1)) || ...
        config.simulationLength ~= 160 || ...
        config.sensorsPerFormation ~= 6 || ...
        config.fovHalfAngleDeg ~= 60 || ...
        config.fovTotalAngleDeg ~= 120 || ...
        ...
        config.measurementNoiseStd ~= 7 || ...
        config.clutterRate ~= 4 || ...
        ~strcmp(config.sensorHardwareProfile, ...
            'formation-shared-120deg-r300-q300-v1') || ...
        ~isstruct(config.sensorQuality) || ...
        ~isfield(config.sensorQuality, 'enabled') || ...
        ~isequal(config.sensorQuality.enabled, true) || ...
        ~isfield(config.sensorQuality, 'referenceRange') || ...
        config.sensorQuality.referenceRange ~= 300 || ...
        ~isequal(config.observationSpaceLimits, ...
            [-1050, 1050; -1050, 1050]) || ...
        ~strcmp(config.clutterSpatialProfile, ...
            'uniform-global-box2100-c4-v1') || ...
        ~isscalar(config.minimumSensorTargetSeparation) || ...
        ~isfinite(config.minimumSensorTargetSeparation) || ...
        config.minimumSensorTargetSeparation < 0 || ...
        (any(strcmp(config.sceneStyle, { ...
                'parallel-convoy', 'linear-relay'})) && ...
            config.minimumSensorTargetSeparation ~= 30) || ...
        (strcmp(config.sceneStyle, 'orthogonal-crossing') && ...
            config.minimumSensorTargetSeparation ~= 0)
    error(['The versioned multistyle formation-FoV scene contract ', ...
        'changed or attempted to authorize an unfrozen outcome.']);
end
computedSha256 = ...
    computeFormationFovMultistyleSceneContractSha256(config);
if ~strcmp(config.sceneContractSha256, computedSha256)
    error('FormationFovSceneContract:InconsistentDigest', [ ...
        'The stored multistyle scene digest does not match the ', ...
        'current config: stored=%s computed=%s.'], ...
        config.sceneContractSha256, computedSha256);
end
expectedSha256 = ...
    getExpectedFormationFovMultistyleSceneContractSha256(config);
if ~strcmp(config.sceneContractSha256, expectedSha256)
    error('FormationFovSceneContract:UnregisteredDrift', [ ...
        'The multistyle scene changed without updating its registered ', ...
        'contract: actual=%s expected=%s.'], ...
        config.sceneContractSha256, expectedSha256);
end
end

function valid = isValidMultistyleCalibrationState(config)
if strcmp(config.sceneStyle, 'orthogonal-crossing')
    valid = strcmp(config.sceneCalibrationStatus, 'stress-only-v5') && ...
        isequal(config.formalValidationAuthorized, false) && ...
        isequal(config.enforceDifficultyRequirements, false) && ...
        isempty(fieldnames(config.difficultyRequirements));
    return;
end
expectedRequirements = ...
    getFormationFovMultistyleAbsoluteDifficultyRequirements(config);
valid = strcmp(config.sceneCalibrationStatus, ...
    ...
    isequal(config.difficultyRequirements, expectedRequirements);
end
