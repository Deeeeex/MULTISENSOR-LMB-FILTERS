...
if isfield(config, 'sceneGeometryVersion') && ...
    ...
    % Keep a model-level trace of the outcome gate.  This duplicate is
    % deliberate: removing or corrupting the nested scenario metadata must
    % not silently turn a calibration-only model into a legacy model.
    model.dynamicTopologyTrackingOutcomeGate = struct( ...
        'contractVersion', ...
            'dynamic-topology-tracking-outcome-gate-v1', ...
        'required', true, ...
        'sceneContractSha256', config.sceneContractSha256);
end
