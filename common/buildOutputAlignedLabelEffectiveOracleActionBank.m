...
bankVariant = char(getField(options, 'bankVariant', ...
    'output-aligned-label-effective-oracle-v150'));
externalActiveTimeOffsets = reshape(getField(options, ...
    'externalActiveTimeOffsets', 0), 1, []);
...
if ~isscalar(maximumCandidates) || ~isfinite(maximumCandidates) || ...
        ...
        isempty(bankVariant) || ...
        isempty(externalActiveTimeOffsets) || ...
        any(~isfinite(externalActiveTimeOffsets)) || ...
        any(externalActiveTimeOffsets ~= round(externalActiveTimeOffsets)) || ...
        any(externalActiveTimeOffsets < 0)
...
fusionConfig.missingLabelFusionMode = getField( ...
    context.triggerConfig, 'missingLabelFusionMode', ...
...
for receiverIdx = reshape(find(any(crossResidual, 2)), 1, [])
    ...
    for senderIdx = crossSenders
        ...
        for objectIdx = 1:numel(senderObjects)
            ...
            [baseScore, details] = observableInfluenceScore( ...
            ...
            candidateFusionDetails = fusionDetails;
            candidateFusionDetails.explicitLabelOmissionRegistered = ...
                false(1, numel(distributions));
            candidateFusionDetails.explicitLabelOmissionRegistered( ...
                sourcePosition) = true;
            candidateFusionDetails.explicitOmittedLabels = ...
                repmat({zeros(2, 0)}, 1, numel(distributions));
            candidateFusionDetails.explicitOmittedLabels{ ...
                sourcePosition} = label;
            candidateFused = fuseLmbPosteriorsByLabel( ...
                candidateDistributions, weights, context.model, ...
                weights, candidateFusionDetails, fusionConfig);
            impact = fusedOutputImpact( ...
                referenceFused, candidateFused, context.model);
            score = impact.score + 1e-3 * baseScore;
            cursor = cursor + 1;
            ...
            candidate.fusedDecisionCrossingCount = ...
                impact.decisionCrossingCount;
            candidate.fusedMapCardinalityChange = ...
                impact.mapCardinalityChange;
            candidate.fusedExistenceL1Change = ...
                impact.existenceL1Change;
            candidate.fusedSpatialShift = impact.spatialShift;
            candidate.fusedOutputImpactScore = impact.score;
...
bank.externalActiveTimeOffsets = externalActiveTimeOffsets;
...
function details = buildFusionDetails(receiverIdx, senders, currentTime)
sourceCount = 1 + numel(senders);
details = struct();
details.sourceIndices = [receiverIdx, senders];
details.isSelf = [true, false(1, numel(senders))];
details.isStale = false(1, sourceCount);
details.currentTime = currentTime;
details.eventType = [0, 2 * ones(1, numel(senders))];
end

function impact = fusedOutputImpact(reference, candidate, model)
labels = unionLabels(reference, candidate);
decisionCrossingCount = 0;
referenceCardinality = 0;
candidateCardinality = 0;
existenceL1Change = 0;
spatialShift = 0;
for labelIdx = 1:size(labels, 2)
    label = labels(:, labelIdx);
    referenceObject = findObject(reference, label);
    candidateObject = findObject(candidate, label);
    referenceExistence = objectExistence(referenceObject);
    candidateExistence = objectExistence(candidateObject);
    referenceDecision = referenceExistence >= 0.50;
    candidateDecision = candidateExistence >= 0.50;
    referenceCardinality = referenceCardinality + referenceDecision;
    candidateCardinality = candidateCardinality + candidateDecision;
    decisionCrossingCount = decisionCrossingCount + ...
        (referenceDecision ~= candidateDecision);
    existenceL1Change = existenceL1Change + ...
        abs(referenceExistence - candidateExistence);
    if ~isempty(referenceObject) && ~isempty(candidateObject)
        [referenceMean, ~] = momentMatch( ...
            referenceObject, model.xDimension);
        [candidateMean, ~] = momentMatch( ...
            candidateObject, model.xDimension);
        positionDimension = min(2, model.xDimension);
        spatialShift = spatialShift + ...
            min(referenceExistence, candidateExistence) * ...
            min(norm(referenceMean(1:positionDimension) - ...
                candidateMean(1:positionDimension)) / ...
                resolvePositionScale(model), 2);
    end
end
mapCardinalityChange = candidateCardinality - referenceCardinality;
score = 10 * decisionCrossingCount + ...
    abs(mapCardinalityChange) + existenceL1Change + ...
    0.50 * spatialShift;
impact = struct( ...
    'decisionCrossingCount', decisionCrossingCount, ...
    'mapCardinalityChange', mapCardinalityChange, ...
    'existenceL1Change', existenceL1Change, ...
    'spatialShift', spatialShift, ...
    'score', score);
end

function labels = unionLabels(first, second)
labels = zeros(2, 0);
objects = [reshape(first, 1, []), reshape(second, 1, [])];
for objectIdx = 1:numel(objects)
    if objects(objectIdx).numberOfGmComponents <= 0
        continue;
    end
...
candidate = struct( ...
    ...
    'spatialDisagreement', 0, ...
    'fusedDecisionCrossingCount', 0, ...
    'fusedMapCardinalityChange', 0, ...
    'fusedExistenceL1Change', 0, ...
    'fusedSpatialShift', 0, ...
    'fusedOutputImpactScore', 0, ...
