...
requiredModel = {'activationThreshold', 'calibrationMargin', ...
    'switchingPenalty', 'tieTolerance', 'controlSynopsisMode'};
...
featureContract = scoreDetails.edgeFeatureDetails;
activeFeatures = logical(reshape(model.activeFeatureMask, 1, []));
if strcmp(model.controlSynopsisMode, 'compact-node-32') && ...
        any(activeFeatures & ~featureContract.compactTelemetryMask)
    error('ScaleEquivariantGatewayV254:UnfundedRichFeature', ...
        'A compact-control V254 model activates a rich pairwise-label feature.');
end
controlBytes = estimateScaleEquivariantGatewayControlBytesV254( ...
    context, referenceAssignment, model.controlSynopsisMode);
...
details.controlSynopsisMode = model.controlSynopsisMode;
details.controlAttemptedBytes = controlBytes.totalAttemptedBytes;
details.controlByteDetails = controlBytes;
...
schedule.controlSynopsisMode = model.controlSynopsisMode;
schedule.controlAttemptedBytes = controlBytes.totalAttemptedBytes;
