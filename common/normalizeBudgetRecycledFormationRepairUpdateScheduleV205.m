...
% The V221 single-pass-label-kla mode is a frozen teacher action that routes
% one complete label into the ordinary KLA input set instead of adding a
...
singlePassLabelKlaMask = strcmp(modes, 'single-pass-label-kla');
labelKlaMask = teacherLabelKlaMask | causalLabelKlaMask | ...
    singlePassLabelKlaMask;
if any(hardMask & abs(weights - 1) > 1e-12) || ...
    ...
    error('BudgetRecycledRepairUpdateV205:InvalidSchedule', ...
        ['Hard restoration requires unit source weight; label-kla is an ', ...
         ...
         'delivery. single-pass-label-kla is a teacher-only input route. ', ...
         'All KLA modes require source weight below one.']);
end
...
schedule.singlePassLabelKlaMask = singlePassLabelKlaMask;
...
if ~ismember(mode, { ...
    ...
    error('BudgetRecycledRepairUpdateV205:InvalidSchedule', ...
        ['Only hard-replacement, label-kla, causal-label-kla, and ', ...
         'single-pass-label-kla updates are registered.']);
end
end
