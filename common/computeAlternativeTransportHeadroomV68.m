    ...
    for incumbentIdx = 1:numel(incumbentCrossSenders)
        ...
        if requireCurrentNovelSupport
            alternatives = alternatives(arrayfun(@(senderIdx) ...
                candidateHasNovelSupport( ...
                    posteriors{receiverIdx}, ...
                    posteriors{incumbentSender}, ...
                    posteriors{senderIdx}), alternatives));
        end
...
function available = candidateHasNovelSupport( ...
        receiver, incumbent, candidate)
...
available = any(candidateSupport > baseline + 1e-12);
end
