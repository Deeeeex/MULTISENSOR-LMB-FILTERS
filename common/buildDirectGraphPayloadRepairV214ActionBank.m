...
repairIdealDeliveryTeacherMode = logical(getField( ...
    options, 'repairIdealDeliveryTeacherMode', false));
repairForcedSourceId = getField(options, 'repairForcedSourceId', 0);
repairForcedLabel = reshape(getField( ...
...
forcedRepairRequested = repairForcedSourceId > 0;
if ~isscalar(repairForcedSourceId) || ...
        ~isnumeric(repairForcedSourceId) || ...
        ~isfinite(repairForcedSourceId) || ...
        repairForcedSourceId < 0 || ...
        repairForcedSourceId ~= round(repairForcedSourceId) || ...
        repairForcedSourceId > numel(context.localPosteriorBySensor) || ...
        numel(repairForcedLabel) ~= 2 || ...
        any(~isfinite(repairForcedLabel)) || ...
        any(repairForcedLabel ~= round(repairForcedLabel)) || ...
        ((repairForcedSourceId == 0) ~= all(repairForcedLabel == 0)) || ...
        (repairIdealDeliveryTeacherMode ~= forcedRepairRequested) || ...
        (forcedRepairRequested && ...
            (~creditRepairMode || numel(requestedFormationIds) ~= 1 || ...
             any(repairForcedLabel < 1)))
    error('DirectGraphPayloadRepairV214:InvalidForcedRepairTeacher', ...
        ['A forced repair is allowed only for one explicit formation ', ...
         'under ideal-delivery teacher mode.']);
end
...
for actionIdx = 2:actionCount
    ...
    if creditRepairMode
        if forcedRepairRequested
            actionNames{actionIdx} = sprintf( ...
                ['v217-teacher-withhold-f%d-plus-label-kla-', ...
                 's%d-l%d-%d'], ...
                formationId, repairForcedSourceId, ...
                repairForcedLabel(1), repairForcedLabel(2));
        else
            actionNames{actionIdx} = sprintf( ...
        ...
        actionRepairPlans{actionIdx} = buildCausalRepairPlan( ...
            ...
            repairSourceWeight, repairIdealDeliveryTeacherMode, ...
            repairForcedSourceId, repairForcedLabel);
    else
        actionNames{actionIdx} = sprintf( ...
...
function plan = buildCausalRepairPlan( ...
        ...
        sourceWeight, idealDeliveryTeacherMode, forcedSourceId, ...
        forcedLabel)
plan = struct();
plan.times = currentTime;
plan.referenceAttemptedBytes = referenceAttemptedBytes;
plan.idealDeliveryTeacherMode = logical(idealDeliveryTeacherMode);
plan.maximumActions = 1;
if plan.idealDeliveryTeacherMode
    plan.forcedFormationId = targetFormationId;
    plan.forcedSourceId = forcedSourceId;
    plan.forcedLabel = reshape(forcedLabel, 2, 1);
    plan.updateMode = 'label-kla';
else
