...
fusionWeights = getField(options, 'fusionWeights', []);
if ~iscell(referenceReceiverDistributions) || ...
        ...
        ~ismember(supportMode, { ...
            'receiver-only', 'receiver-or-cross-sender'})
...
if strcmp(supportMode, 'receiver-or-cross-sender') && ...
        (~isnumeric(fusionWeights) || ...
         ~isequal(size(fusionWeights), [nodeCount, nodeCount]) || ...
         any(~isfinite(fusionWeights(:))) || ...
         any(fusionWeights(:) < -1e-12) || ...
         any(abs(sum(fusionWeights, 2) - 1) > 1e-10))
    error('SetEntryRisk:InvalidInput', ...
        'Network support requires valid current fusion weights.');
end
...
enteredReceiverSupportByReceiver = cell(1, nodeCount);
enteredCrossSenderSupportByReceiver = cell(1, nodeCount);
...
for receiverIdx = 1:nodeCount
    ...
    crossSenderSupport = zeros(1, size(enteredLabels, 2));
    if strcmp(supportMode, 'receiver-or-cross-sender')
        crossSenders = find(fusionWeights(receiverIdx, :) > 1e-12 & ...
            groupIds ~= groupIds(receiverIdx));
        for senderIdx = reshape(crossSenders, 1, [])
            crossSenderSupport = max(crossSenderSupport, ...
                currentAssociationMass( ...
                    localPosteriorBySensor{senderIdx}, enteredLabels));
        end
    end
    enteredSupport = max(receiverSupport, crossSenderSupport);
    ...
    enteredReceiverSupportByReceiver{receiverIdx} = receiverSupport;
    enteredCrossSenderSupportByReceiver{receiverIdx} = ...
        crossSenderSupport;
...
details.supportMode = supportMode;
...
details.enteredReceiverSupportByReceiver = ...
    enteredReceiverSupportByReceiver;
details.enteredCrossSenderSupportByReceiver = ...
    enteredCrossSenderSupportByReceiver;
