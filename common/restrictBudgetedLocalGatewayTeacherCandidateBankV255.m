...
validCoverage = all(slotCandidateCount >= 1);
validCount = candidateCount >= protocol.minimumCandidateCount && ...
...
if ~validCoverage || ~validCount
    error('BudgetedLocalGatewayTeacherV255:RestrictedBankCoverage', ...
        'The local bank does not cover every directed formation slot.');
end
