...
当前是 7 页英文完整初稿，含正文、4 幅 SVG/PDF 图、3 张表、参考文献和 AI 使用披露。
...
|---|---:|---:|---:|
| Lineage OSPA (m) | 2.651 | 1.123 | 0.797 |
...
第一条构建命令使用已生成图表；第二条从保存的数值重新生成参考文献、表格和四组图。
...
已在脱离实验目录的临时目录验证整包重建：生成表格、四张图的 PNG 和七页 PDF 渲染逐字节一致。
记录见 `output/qa/portable_rebuild.json`；该检查使用同一本机运行时与字体，不替代 MATLAB 实验复跑。
