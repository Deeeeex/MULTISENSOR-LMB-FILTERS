...
## Intro and added diagnostic design
...
The fixed-input 2 by 2 experiment is presented as a table to expose all
...
| Sequence gains across link conditions | GCE lowers OSPA under both link conditions in 20 of 25 sequences against No-age KLA and 14 of 25 against Scalar; the scatter also shows heterogeneous gains and sign reversals. | Quantitative grid with two scatter panels, 181 by 83 mm. A point joins the reliable and intermittent gain for the same complete sequence. No-age KLA represents the inherited posterior pool; Scalar isolates the closest existence-only reference. | All 25 sequences appear in each selected comparison, giving 50 two-coordinate points and 100 measured gains. Gain is reference minus GCE. Within each panel both axes have equal scale; ranges may differ between references for legibility. Zero lines, the equal-gain diagonal, and shape-coded outcome categories remain legible in grayscale. No jitter, selective point removal, fitted trend, or causal interpretation. |
...
Fig. 3 shows observations and deterministic sign counts, without confidence
...
main figure's two references are chosen for their methodological roles,
