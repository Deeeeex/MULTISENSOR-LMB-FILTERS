...
设计分成两层：已实现的稀疏因果路由决定“谁给谁发”，验证中的接收规则处理“原定输入没到时怎么办”。图中三个编队只是结构示意，不代表 M24 或 X36 的实际规模。
...
因此，V278 的作用是检验缺失输入处理能否帮助保住目标存在信息，而不是仅让位置指标更好看。原有判据不变，也不预设它会成功。若它不能解决这个缺口，接下来应区分“有效信息没有到达”和“到达后存在概率在融合中下降”，再决定路由或接收规则怎么改；不再仅凭计划连通性、双随机性或更快混合选择新候选。本节来自已打开种子的离线分析，没有重跑滤波。[误差分解记录](../../RUN/GA/dynamic_topology/evidence/tracking_aligned_v279/set_error_budget_seed1301/SET_ERROR_BUDGET_V279.md)
