...
多编队传感器需要交换目标判断，但运动会改变邻居关系，链路也会丢包。我们的主线是：**以因果修复维持稀疏通信骨干，让有限通信真正转化为更好的目标集合估计。** M24/X36 的后验发送字节分别减少 10.0%/21.8%，但整体 E-OSPA 仅改善 2.40%/0.37%；自身权重回填也未解决精度与一致性的取舍。关键卡点是目标存在信息的恢复与长期保留。三个 M24 缓存时刻显示，弱局部估计在最后一次融合之前已经出现；接下来追踪局部观测更新与此前多轮传播，不直接降低输出门槛或扩展参数扫描。新论文已包含两张矢量图和完整主表；这些仍是单种子开发证据。
...
表中均为 24 个接收节点的平均值。标签不是经过真值匹配的目标，存在概率总量也不是召回率；它只是把同一次融合的输入加权和空间影响分开观察。尚未输出的低概率标签仍保留在内部后验中，不能等同于被输出步骤删掉。下一步应追踪局部观测更新之前与之后，以及此前多轮传播的变化，同时区分当前输入加权的影响。这个局部结论尚不能外推到 X36，也不能排除空间不相容在更早时刻反复累积的损失。[三时刻分析与方法决策](../../RUN/GA/dynamic_topology/V281_EXISTENCE_LOSS_LOCALIZATION_FINDING.md)
...
| 当前 | 目标存在信息在哪个环节损失？ | M24 三个缓存时刻已完成单轮分析：弱存在证据在最后融合前已出现，当前空间项和剪枝的额外损失较小。下一步跟踪局部更新与跨时刻传播，并补 X36 轨迹，不据此降低输出门槛或扫描空间项。 |
