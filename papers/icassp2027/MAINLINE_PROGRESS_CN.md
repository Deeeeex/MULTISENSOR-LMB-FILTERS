...
多编队传感器需要交换目标判断，但运动会改变邻居关系，链路也会丢包。我们的主线是：**以因果修复维持稀疏通信骨干，让有限通信真正转化为更好的目标集合估计。** M24/X36 的后验发送字节分别减少 10.0%/21.8%，但整体 E-OSPA 仅改善 2.40%/0.37%；自身权重回填也未解决精度与一致性的取舍。X36 前 40 步的新分析显示：信息曾经有路径到达，不等于现在还保有足够强的目标判断。接下来聚焦观测证据在多跳传递中的保留和时效，不只修改最后一次融合或降低输出门槛。新稿已有两张矢量图、完整主表和多视域相关工作比较；尚无新的跟踪收益，现有结果仍是单种子开发证据。
...
表中 0.5/0.9 只用于描述输入，不是输出阈值或新方法参数；比例统计的是标签融合事件，不是真实目标召回。第 36–40 步，当前没有观测机会的输入仍占约 79.5% 的权重，其中许多已经有历史信息来源，因此不能简单删除所有当前视域外输入。同时，晚期先验占比低也不能排除启动阶段留下的长期影响；改变前几步先验处理后会怎样，尚未做因果对照。
...
| 当前 | 怎样让有限通信保留有用的新观测证据？ | 设计来源、时效与重复信息处理都明确的单一候选，先做 X36 有界配对；同时记录新增元数据成本，并补足多视域 LMB 参考方法的算法级比较。不能把诊断标记、纯粹高概率优先或已有信息量加权当作已验证的新方法。 |
