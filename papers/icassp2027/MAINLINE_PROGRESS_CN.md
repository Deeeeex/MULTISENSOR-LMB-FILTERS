...
**当前要补齐的环节。** 基础通路不等于有用信息已经送到。需要同时保住目标存在信息，并让新增输出获得可靠的位置更新。目前代码按相同出生标签直接融合，没有空间标签匹配；同标签输出的几何分歧可能来自估计或关联不确定性，并不自动推翻共享出生标签模型。固定标签、路由和观测的 LMB-MIL 对照已完成，但它不是完整的不同视域方法。下一步先明确哪些输入对目标真正具有观测依据、如何处理只被部分节点看到的目标，以及保留空间分量带来的实际字节开销。若需要重匹配，再将它作为单独因素；只有使用相同融合规则时，静态与动态路由的差别才是路由收益。随后再研究新观测的来源、时间、重复利用和受预算约束的传递。小协方差不等于来源准确，漏检也不等于新位置测量。这是待验证方向；下图仍只展示已实现骨干和接收规则对照。
...
| 当前 | 有限视域下应保留什么证据，真实字节预算如何约束？ | 先建立明确处理独有视域证据的融合基线；不能把消息条数等同于字节预算，也不因 MIL 对照未通过就认定需要重命名标签。 |
