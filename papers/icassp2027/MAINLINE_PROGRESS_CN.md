...
多编队传感器需要交换目标判断，但运动会改变邻居关系，链路也会丢包。我们的主线是：**以因果修复维持稀疏通信骨干，让有限通信同时支撑目标存在判断与可靠的位置更新。** M24/X36 的后验发送字节分别减少 10.0%/21.8%，但整体 E-OSPA 仅改善 2.40%/0.37%；自身权重回填也未解决精度与一致性的取舍。X36 的进一步分析表明，保住更多目标并不保证新增输出的位置准确，后验协方差小也不能单独作为可靠选源依据。接下来明确新位置观测的来源、时效和重复利用，再研究受预算约束的跨编队传递。新稿已有两张矢量图、完整主表和多视域相关工作比较；尚未建立满足联合目标的新方法，现有性能结果仍是单种子开发证据。
...
**当前要补齐的环节。** 基础通路不等于有用信息已经送到。下一项方法需要同时保住目标存在信息，并让新增输出获得可靠的位置更新。选源也不能简单理解为“谁的协方差最小就听谁的”：后验表达的把握程度不一定对应实际准确度。接下来先明确新位置观测的来源、时间和重复利用，再研究有预算与时效约束的跨编队传递。漏检仍可以提供存在性信息，但不等于获得了新的位置测量。这是待验证的设计方向；下图展示的是已实现骨干和接收规则对照，不是新增模块已经成功。
...
| 当前 | 怎样同时保住目标，并让新增输出位置可靠？ | 先区分本地新位置观测、漏检与历史混合信息，明确来源、时间及重复信息处理，再设计受预算约束的跨编队传递。低协方差、完整后验风险或高存在概率均不能单独保证来源准确；后续 X36 配对需包含同融合规则的静态基线和全部联合指标。 |
