...
**当前要补齐的环节。** 基础通路不等于有用信息已经送到。需要同时保住目标存在信息，并让新增输出获得可靠的位置更新。目前代码按相同出生标签直接融合，没有空间标签匹配；同标签输出出现几何对应分歧，但这可能来自估计或关联不确定性，并不自动推翻共享出生标签模型。下一步增加已有标签匹配方法的融合对照，检验重匹配是否真正改善跟踪，并分清融合规则变化与路由改进的作用，再研究新位置观测的来源、时间、重复利用及传输预算。小协方差不等于来源准确，漏检也不等于新位置测量。这是待验证方向；下图仍只展示已实现骨干和接收规则对照。
...
标签对应是另一个需要补齐的基线。[Li 等，TSP，2019](https://doi.org/10.1109/TSP.2018.2880704)先匹配各节点的标签，再逐标签 GCI；[Gao 等，TAES，2022](https://doi.org/10.1109/TAES.2022.3182642)同时考虑不同视域与标签匹配，但使用约束最小信息损失准则，并非同一个 KLA 实现。这些方法都不能算作我们的创新。当前已核对其书目信息与高层方法，尚未完成算法复现；后续应先保证相同融合底座，再衡量静态与动态路由的差别。
...
| 当前 | 空间重匹配能否真正改善跟踪？ | 先增加有明确文献来源的标签匹配对照，在同一路由下检验融合规则变化，再于同一融合规则下比较静态与稀疏路由；不直接用真值重命名、不预设原共享标签模型错误，也不把已有匹配方法当成创新。实现细节明确后启动 X36 前缀配对，不同时叠加其他接收改动。 |
