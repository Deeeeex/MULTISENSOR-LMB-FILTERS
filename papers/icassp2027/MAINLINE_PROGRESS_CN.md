...
多编队传感器需要交换目标判断，但运动会改变邻居关系，链路也会丢包。我们的主线是：**以因果修复维持稀疏通信骨干，让有限通信同时支撑目标存在判断与可靠的位置更新。** M24/X36 的后验发送字节分别减少 10.0%/21.8%，但整体 E-OSPA 仅改善 2.40%/0.37%；自身权重回填也未解决精度与一致性的取舍。X36 的进一步分析表明，保住更多目标不保证新增输出的位置准确；同标签输出还有几何对应分歧，但尚不能据此认定共享标签模型有错。当前先固定共享标签和稀疏路由，比较 KLA 与文献中的 LMB-MIL 融合准则，再判断是否需要空间重匹配，随后研究观测证据的时效和受预算约束的跨编队传递。新稿已有两张矢量图、完整主表和相关工作比较；尚未建立满足联合目标的新方法，现有性能结果仍是单种子开发证据。
...
**当前要补齐的环节。** 基础通路不等于有用信息已经送到。需要同时保住目标存在信息，并让新增输出获得可靠的位置更新。目前代码按相同出生标签直接融合，没有空间标签匹配；同标签输出的几何分歧可能来自估计或关联不确定性，并不自动推翻共享出生标签模型。因此先固定标签、路由和观测，检验已有 LMB-MIL 融合对照：它按存在概率加权保留输入空间分量，不通过 KLA 的空间重叠项降低存在概率。若需要重匹配，再将它作为单独因素；只有使用相同融合规则时，静态与动态路由的差别才是路由收益。随后才研究新位置观测的来源、时间、重复利用及传输预算。小协方差不等于来源准确，漏检也不等于新位置测量。这是待验证方向；下图仍只展示已实现骨干和接收规则对照。
...
这张图只比较同一尺度内的通信量与 E-OSPA，越靠左下越好；两幅图的坐标范围不同。M24 的稀疏方案以较低通信量接近完整动态路由的集合误差，X36 的集合误差差距更大。每个点是一次已完成的配对开发实验，没有跨种子误差条；条件 RMSE、一致性和编队尾部仍需一起看上表，不能仅凭这两个坐标指定全面最优方法。
...
**X36 的新发现：有过观测信息来源，不等于当前判断足够强。** 现已完成原 X36 实验前 40 步的逐环节记录，没有修改感知条件、路由或滤波规则。实现中检测概率为零的标签，其局部存在概率保持不变至数值精度，因此没有发现“视域外仍承受名义漏检惩罚”的证据。第 40 步在最后一次融合之前，局部后验平均已只输出 5.806 个估计；融合之后为 5.861 个，弱状态并不只出现在最终输出环节。
...
当前已按原文公式接入共享标签的 MIL 对照：缺失标签补为零存在概率，完整拼接空间混合分量、合并完全相同的分量，再按原 8 分量上限截断并记录损失。这既改变融合准则，也明确改变缺失标签处理，不能把效果全归给去掉空间重叠项。它不是完整的不同视域或标签匹配方法；两步运行只确认流程接通，X36 的 40 步配对仍在计算。[解析示意图与基线说明](../../RUN/GA/dynamic_topology/LABEL_MATCHING_BASELINE_NOTES.md)用于解释两种规则的区别，不作为跟踪收益。
...
| 当前 | 更换融合准则能否同时改善数量和定位？ | 保留共享标签与稀疏路由，运行已接通的 LMB-MIL 对照；X36 前 40 步正在计算，不叠加重匹配或新的权重。共同检查 E-OSPA、条件 RMSE、覆盖、一致性、字节和编队尾部，未通过的候选只留实验记录。 |
