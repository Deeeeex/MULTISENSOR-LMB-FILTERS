...
多编队传感器需要交换目标判断，但编队运动会改变邻居关系，链路也会丢包。我们的主线是：**以因果修复维持稀疏通信骨干，再处理消息缺失对融合的影响，让有限通信真正转化为更好的目标估计。** 最小骨干在 M24/X36 上分别减少 10.0%/21.8% 的后验发送字节。新完成的 X36 自身权重回填实验，使四项网络均值相对静态路由同时改善：E-OSPA 下降 0.542%、条件 RMSE 下降 45.124%、一致性改善 0.096%、后验字节减少 21.535%。但相对原稀疏骨干，RMSE 退化 4.836%，未通过后续实验判据。接下来定位目标存在信息在观测、传播、融合和输出哪个环节损失，不再扩展权重或网关参数扫描。新论文已写入这些开发结果及误差分解。
...
| 当前 | 目标存在信息在哪个环节损失？ | 已排查全局观测覆盖和理想时间路径；接下来定位局部更新、实际接收、融合和目标输出，先确定损失环节再设计新改动。 |
