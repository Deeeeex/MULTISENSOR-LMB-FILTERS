...
多编队传感器需要交换目标判断，但运动会改变邻居关系，链路也会丢包。我们的主线是：**以因果修复维持稀疏通信骨干，让有限通信同时支撑目标存在判断与可靠的位置更新。** M24/X36 的后验发送字节分别减少 10.0%/21.8%，但整体 E-OSPA 仅改善 2.40%/0.37%。当前缺口是完整目标集合的恢复，不能用已匹配目标的定位改善代替。新的受控比较表明，目标是否存在与位置怎样合并需要分开分析，但更换融合公式尚未带来均衡收益；传播分析则显示，有了更多通路，近期观测在融合中的作用未必同步增加。下一步聚焦有用新证据的传递与保留，同时控制位置冲突、重复利用和发送字节。这不改变动态路由的研究问题，也不把已有融合思想当成创新。新稿已按引入图、机制图、结果图组织三张矢量图，并用主表与配对口径表承载数值；尚未建立满足联合目标的新方法，现有性能结果仍是单种子开发证据。
...
**位置合并规则已经单独比较。** 固定路由、局部观测更新和共享标签，对“这个目标是否存在”保持 MIL 的加权平均，对“它在哪里”分别采用保留多个位置候选和几何融合。完成的 X36 前 40 步对照中，后者明显缓解了前者的定位与通信代价，但相对原 KLA 仍有编队定位退化，尚不能作为统一替代。两种方法使用同样的存在判断和条件权重公式，后续递归输入仍会因位置更新不同而分化。几何融合也可能在两个冲突位置之间形成错误的中间估计，因此不能把“存在概率较高、分布较集中”直接当作正确性。[完整对照记录](../../RUN/GA/dynamic_topology/V291_CONDITIONAL_SPATIAL_POOL_DESIGN.md)
...
**为什么丢一条消息还会改变目标存在判断？** 下图左侧比较丢包后的两种权重处理，右侧把变化代入一个可精确计算的一维高斯例子：两份输入都认为目标有 80% 的概率存在，但对位置的判断不同。融合后的曲线同时表示位置分布和存在概率，曲线面积就是存在概率。重新归一化放大了剩余网关的影响，也改变了两份位置判断的空间相容性。因此，路由和接收规则并非只影响通信量，也会改变“有没有目标”和“目标在哪里”。图中没有设定真实位置，不能把更高的曲线面积当成更好的跟踪结果；它不是 M24/X36 的新增实验。

![丢包如何通过权重和空间相容性影响目标判断](figures/packet_fusion_mechanism.png)
...
| 场景 | 方法 | E-OSPA / m | 条件位置 RMSE / m | 数量绝对误差 | 焦点窗口一致性 / m | 后验发送 / MB | 计划消息 / 步 |
|:--|:--|--:|--:|--:|--:|--:|:--|
| M24 | 固定编队树 | 125.478 | 22.640 | 11.131 | 133.599 | 40.769 | 46–48 |
| M24 | 完整因果修复 | **122.380** | 14.081 | **10.671** | 131.913 | 44.867 | 48 |
| M24 | 最小因果骨干 V242 | 122.462 | **12.183** | 10.720 | **131.664** | **36.676** | 30 |
| X36 | 固定编队树 | 132.680 | 36.925 | **18.456** | 139.407 | 76.871 | 66–72 |
| X36 | 完整因果修复 | **131.795** | 19.586 | 18.463 | **139.004** | 81.259 | 72 |
| X36 | 最小因果骨干 V242 | 132.192 | **19.329** | 18.597 | 140.489 | **60.090** | 46 |
| X36 | 最小骨干＋自身权重回填 V278 | 131.961 | 20.263 | 18.518 | 139.273 | 60.317 | 46 |
...
| 位置误差的配对检查 | 双方均可计算 RMSE 的单元占比 | 同批单元 RMSE 改善 | 最弱编队 RMSE 改善 |
|:--|--:|--:|--:|
| M24 最小骨干 / 固定树 | 99.870% | +46.190% | −24.085% |
| X36 最小骨干 / 固定树 | 97.465% | +38.431% | +4.444% |
| X36 自身回填 / 最小骨干 | 96.493% | −5.471% | −37.361% |

正数表示改善、负数表示退化。同批单元对齐的是传感器与时刻，不保证匹配到相同目标身份；最弱编队一列仍采用各组原有的有效 RMSE 单元。两种检查各回答一个问题：均值改善是否依赖计算范围变化，以及是否掩盖某个编队的退化。数据直接来自已有结果，没有重跑滤波。
...
**有通路，还要看它能产生多大作用。** 在相同消息投递记录上，我们进一步按融合权重追溯信息来源：把一次接收看成按权重选择来源，计算沿这些来源向过去追溯时，在指定时间窗口内遇到观测机会的概率。它补充了“是否存在任意一条路径”的判断，但仍不模拟真实的标签更新。X36 的完整修复把 8 步内的路径覆盖率从 49.716% 提高到 68.388%，平均加权来源触达分数却只从 21.459% 提高到 21.805%；稀疏修复为 21.806%。额外通路并没有带来同等幅度的加权触达增长。

![几何通路与加权观测来源](../../RUN/GA/dynamic_topology/evidence/tracking_aligned_v292/weighted_observation_transport_seed1301/WEIGHTED_OBSERVATION_TRANSPORT_V292.png)

虚线表示有观测来源路径的比例，实线表示按消息层融合权重计算的平均来源触达分数，三种方法的实线非常接近。两图沿用 M24/X36 的同一个开发种子、完整 160 步及全部四个时间窗口；几何可见不代表实际检测，分数也不是后验质量、标签召回或误差上界。它提示应研究有用新证据怎样经过多跳继续发挥作用，而不是直接认定“调高跨编队权重就能改善跟踪”。来源重复、时效和空间相容性仍需控制。[分析定义与完整数据](../../RUN/GA/dynamic_topology/V292_WEIGHTED_OBSERVATION_TRANSPORT_DESIGN.md)
...
| 已完成 | 存在判断相同时，位置合并方式有什么作用？ | 冻结的 X36 40 步配对已完成，缓解 MIL 的定位与通信代价，但未解决原 KLA 下的编队定位取舍。不改门槛、不直接扩展全程或 M24；数值保留在实验记录。 |
| 已完成 | 更多通路是否带来更强的近期观测作用？ | 复用两尺度的 160 步数据，按消息层权重追溯来源。路径覆盖率明显增加，平均加权来源触达仅小幅增加；该诊断不是实际标签质量，不据此直接调大混合权重。 |
