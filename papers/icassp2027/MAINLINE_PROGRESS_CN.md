...
多编队传感器需要交换目标判断，但运动会改变邻居关系，链路也会丢包。我们的主线是：**以因果修复维持稀疏通信骨干，让有限通信同时支撑目标存在判断与可靠的位置更新。** M24/X36 的后验发送字节分别减少 10.0%/21.8%，但整体 E-OSPA 仅改善 2.40%/0.37%；自身权重回填也未解决精度与一致性的取舍。X36 的进一步分析表明，保住更多目标不保证新增输出的位置准确；同标签输出的几何分歧也不能直接推翻共享标签模型。当前先补齐估计基线：一个位置判断可能跨越视域边界，不能只按中心点判断是否可见。局部视域更新、节点间融合和消息传递将分开比较，再研究观测证据的时效与真实字节预算；这不改变动态路由的研究问题，也不把已有滤波改进当成创新。新稿已有两张矢量图、完整主表和相关工作比较；尚未建立满足联合目标的新方法，现有性能结果仍是单种子开发证据。
...
**当前要补齐的环节。** 基础通路不等于有用信息已经送到，还要区分信息在本地怎样形成、在接收端怎样融合、又通过什么路线传递。当前先检验第一层：当一个目标的可能位置同时覆盖视域内外时，只检查中心点会误判它被看到的可能性。新增对照把跨界的高斯分量细分后再做正常观测更新，使目标存在判断和位置分布一起响应观测，而不是只改一个概率数值。它已接入代码，尚无完成的 40 步跟踪结果；拆分自身也有近似误差，并可能增加计算和发送字节，因此不能仅凭积分更准确就认定方法更好。

这项局部更新属于补齐估计基线，不是新的路由贡献。共享出生标签、KLA 和稀疏骨干均保留；若需要空间重匹配，再作为单独因素。只有局部更新与融合规则相同，静态和动态路线的差别才能归因于路由。随后才研究观测来源、时效、重复利用和有预算约束的传递。下图仍只展示已实现骨干和接收规则对照，不把尚未完成的视域对照画成已有收益。
...
| 当前 | 局部视域近似是否妨碍数量与位置的共同恢复？ | 保持 KLA、共享标签和稀疏路由，已接入观测更新前的边界分量细分；尚无完成的 40 步跟踪结果。使用同一观测协议，检查数量、定位、一致性、逐编队表现及实际字节和计算代价。 |
