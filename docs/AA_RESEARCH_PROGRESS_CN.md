...
2026-06-27 Candidate A 已开始推进: `RUN/AA/runAaBalancedCardinalityValidation.m` 现在支持 fixed target crossing scenario override 和 crossing-window report metrics；新增 launcher `RUN/AA/launchAaTaesManeuverCrossingSmoke.sh`。N1 smoke `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N1_SEED71_20260627_162134.md` 与 N5 smoke `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N5_SEED71_20260627_162621.md` 已跑通。N5 crossing-window local RMSE 为 fixed baseline `2.3126`、full `1.8618`、reference-only `2.1484`，说明 windowed metrics 能捕捉 full-vs-reference separation；但它仍是 structural smoke，不进入当前 TAES manuscript claim。本轮已建立 optional N50 crossing extractor/readiness/source-bundle path，避免后续把 whole-run average 或 N5 smoke 直接写成 paper evidence。

2026-06-27 16:39 CST Candidate A N50 fixed-run 已启动: `AA_CROSSING_TRIALS=50 AA_CROSSING_BASE_SEED=71 RUN/AA/launchAaTaesManeuverCrossingSmoke.sh`。PID `2751`，log `RUN/AA/AA_TAES_CROSSING_N50_BASESEED71_20260627_163913.log`，pid file `RUN/AA/AA_TAES_CROSSING_N50_BASESEED71_20260627_163913.pid`。启动确认: log 已进入 `AA validation trial 1/50`。本轮已把 `scripts/extract_crossing_evidence.py` 接入 `build.sh`、readiness gate、source bundle 和 response-ready evidence docs；在 `crossing_n50_report` 未配置时，build 会清理 stale crossing artifacts 并通过 optional gate。该长跑完成后下一步是把 raw report 加入 `evidence_sources.json`，跑 `./build.sh`，检查 `generated/CROSSING_N50_MANIFEST.md` 与 crossing-window full-vs-reference separation，再决定是否作为 response-ready evidence 或 manuscript supplement。
...
- TAES 首稿已经可编译，且 N50 主表/paired 表/runtime 正文段/N50 reduction 图、held-out N50 table/paragraph、harsh stress summary/response fragment、topology-ring、partial-FOV 与 full-topology scenario summary/response fragments 都已经由 report-driven generated fragments 或 verifier 输出驱动；方法图和结果图已完成矢量化和 TAES-adjacent style calibration。最终 portal submission 仍需要作者/基金/repository/AI disclosure 最终措辞确认和人工审读；更强的 evidence-risk reduction 主要来自 target maneuver/crossing、covariance/reliability consistency 和 recursive-online guarded deployment。
