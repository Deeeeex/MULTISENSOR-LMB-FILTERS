...
| TAES harsh packet-loss stress validation | protocol/launcher 已建立，N1 smoke 完成 | `docs/AA_STRESS_SCENARIO_VALIDATION_CN.md`; `RUN/AA/launchAaTaesHarshLossN50BaseSeed21.sh`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N1_SEED21_20260624_130131.md` | Harsh-loss profile 固定为 `[0.2 0.35 0.5 0.7] / [1 3 2 2]`，方法参数不变。N1 smoke: full method reduces Net OSPA `2.101216 -> 0.466321` and E-OSPA `2.438530 -> 2.066908`, but RMSE is mixed `3.951127 -> 4.011443`; this is a boundary signal, not a tuning target. |
...
- Independent verifier 已经覆盖 network disagreement、runtime 和 local E-OSPA/RMSE/CardErr；baseSeed=11 held-out N50 已通过；harsh packet-loss stress protocol 已建立但 N50 尚未完成，因此当前剩余证据风险仍包括场景族覆盖偏集中。
...
1. 启动或完成 harsh packet-loss N50 stress validation，并根据结果决定放入 main paper、supplement/response-ready package，还是仅强化 limitation。
