...
| TAES generalization scenario-family protocol | 已建立 code/protocol/launcher；`topology-ring` N50 已完成并接入 build/readiness，`partial-fov35` N50 正在运行 | `docs/AA_GENERALIZATION_SCENARIO_PROTOCOL_CN.md`; `RUN/AA/runAaBalancedCardinalityValidation.m`; `RUN/AA/launchAaTaesScenarioFamilySmoke.sh`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED31_20260624_162439.md`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N5_SEED41_20260624_160335.md`; `RUN/AA/AA_TAES_SCENARIO_partial_fov35_N50_BASESEED41_20260624_182339.log`; `RUN/AA/AA_TAES_SCENARIO_topology_ring_N50_BASESEED31_20260624_162437.log`; `docs/paper/taes/manuscript/scripts/extract_scenario_family_evidence.py`; `docs/paper/taes/manuscript/generated/SCENARIO_FAMILY_MANIFEST.md`; `docs/paper/taes/manuscript/generated/scenario_family_section.tex`; `docs/paper/taes/manuscript/generated/scenario_family_summary_sentence.tex` | 验证入口新增可选 `scenarioOverrides`，默认不改变既有 evidence；显式场景族支持 `topology-ring`、`partial-fov35`、`full-topology`，report 会记录 `scenarioLabel`、`neighborMapMode`、FOV 和 sensor-motion settings。`topology-ring` N50 显示 full method 在 sparse ring 下相对 tuned AA 降低 Network OSPA `47.50%`、local E-OSPA `21.53%`、RMSE `13.12%`、CardErr `24.03%`，reference-only RMSE reduction 为 `5.17%`；这是 paper-grade topology-family evidence。`partial-fov35` N5 smoke 显示 full method 降低 Network OSPA `2.297790 -> 1.341733`、Loc disagreement `1.415682 -> 0.213073`、E-OSPA `2.837543 -> 2.642142`、RMSE `4.120264 -> 3.727303`；Card dispersion/CardErr 仅小幅改善且 CI 覆盖零。固定 N50 已启动，PID `40658`，完成前仍是需要确认的边界信号。`./build.sh` 现在会生成 scenario-family manifest/JSON/response fragment/summary sentence；readiness gate 对 topology N50 给出 paper-grade 通过，同时保留 partial-FOV smoke-tier 标记，避免误称全场景泛化。 |
...
- 关键结果: full barycenter 相对 tuned AA 的 network OSPA / local E-OSPA / local RMSE / CardErr reductions 为 `47.50% / 21.53% / 13.12% / 24.03%`；wins 均为 `50/50`，sign-test `p=1.776e-15`。Reference-only RMSE reduction 为 `5.17%`，说明 matched posterior barycenter 仍贡献主要空间收益。
...
1. 视算力和审稿风险决定是否启动 partial-FOV N50、full-topology N50、maneuvering-target 或 covariance-consistency 验证；保持固定参数，不做结果驱动调参。
