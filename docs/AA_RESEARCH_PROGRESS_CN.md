...
| TAES generalization scenario-family protocol | 已建立 code/protocol/launcher；`topology-ring` 与 `partial-fov35` N1 smoke 已通过 | `docs/AA_GENERALIZATION_SCENARIO_PROTOCOL_CN.md`; `RUN/AA/runAaBalancedCardinalityValidation.m`; `RUN/AA/launchAaTaesScenarioFamilySmoke.sh`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N1_SEED31_20260624_155105.md`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N1_SEED41_20260624_155707.md` | 验证入口新增可选 `scenarioOverrides`，默认不改变既有 evidence；显式场景族支持 `topology-ring`、`partial-fov35`、`full-topology`，report 会记录 `scenarioLabel`、`neighborMapMode`、FOV 和 sensor-motion settings。`topology-ring` N1 smoke 显示 full method 在 sparse ring 下相对 tuned AA 降低 Network OSPA `2.771921 -> 1.348601`、E-OSPA `2.580668 -> 2.010548`、RMSE `4.568272 -> 4.035532`。`partial-fov35` N1 smoke 显示 full method 降低 Network OSPA `2.327551 -> 1.516556`、Loc disagreement `1.182761 -> 0.126668`、E-OSPA `2.922353 -> 2.734257`、RMSE `4.461712 -> 4.281871`，但 Card dispersion `0.457500 -> 0.470000` 略差，是需要 N5/N50 确认的边界信号。这些结果只作为接口和机制 smoke，不作为 paper-grade conclusion。 |
...
- Independent verifier 已经覆盖 network disagreement、runtime 和 local E-OSPA/RMSE/CardErr；baseSeed=11 held-out N50 已通过；harsh packet-loss N50 stress 已完成并接入 evidence manifest。当前剩余证据风险主要是 topology、partial-FOV、target maneuver 和 recursive-online 实现覆盖不足，而不是 packet-loss severity 单一。
...
1. 用 `RUN/AA/launchAaTaesScenarioFamilySmoke.sh` 跑 `topology-ring` 与 `partial-fov35` 的固定 N5 smoke；若结构和机制信号合理，再固定启动 N50。
...
4. 继续压缩和润色 TAES `main.tex`: 控制 stress/held-out 证据进入正文的篇幅，避免超过 TAES page budget。
