...
3. 等当前 N50 local-verifier rerun 完成后，将新 report/log 接入 manuscript evidence scripts，重建 PDF，并让 verifier 复算 paper-facing local metrics；随后运行 `RUN/AA/launchAaTaesHeldoutN50BaseSeed11.sh` 做 paper-grade held-out N50。
