...
| 算法设计与理论证明 | 已建立，online 化第一步通过 N50，本轮增强 | `docs/AA_LABEL_BARYCENTER_THEORY_CN.md`; `docs/paper/taes/manuscript/main.tex` | 已整理 output-level projection 的可证明性质、reference-only ablation 解释、online/distributed label-barycenter AA 的收敛条件和边界；TAES 正文已补 stable-matching consensus limit，并新增 `Weighting is not matching` label-swap 反例命题；neighborhood iterative prototype 已完成 N50。 |
...
- 当前状态: 2026-06-24 10:56 CST 时进程仍存活，log 已进入 `AA validation trial 22/50`。
- 查看进度:
