...
实验结果里仍不完善的部分已经单独梳理到 `docs/AA_EXPERIMENT_GAP_ANALYSIS_CN.md`。当前判断是: 主线证据足够支撑 output-level active-track label/moment projection 这篇 TAES 稿；尚不能过度声称广义 maneuver/crossing/birth-death robustness、covariance/reliability optimality 或 recursive-online LMB validity。这些不完善点更适合作为下一阶段方法创新入口，而不是继续对现有 formation 数据搜索式调参。

2026-06-27 Candidate A 已推进为 fixed-window N50 evidence: `RUN/AA/runAaBalancedCardinalityValidation.m` 支持 fixed target crossing scenario override 和 crossing-window report metrics；launcher `RUN/AA/launchAaTaesManeuverCrossingSmoke.sh` 已跑通 N1/N5 structural smoke，并完成 N50 report `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED71_20260627_163915.md`。N50 crossing-window full method 的 E-OSPA / RMSE reductions 为 `11.08% / 16.99%`，reference-only 为 `4.98% / 5.38%`，说明预指定 crossing window 能捕捉 full-vs-reference separation。N1/N5 仍只是 structural smoke，不进入当前 TAES manuscript claim。

2026-06-27 Candidate A N50 fixed-run 完成并接入 TAES package: `docs/paper/taes/manuscript/evidence_sources.json` 已加入 `crossing_n50_report`，`scripts/extract_crossing_evidence.py` 已接入 `build.sh`、readiness gate、source bundle 和 response-ready evidence docs。`./build.sh` 生成 `generated/CROSSING_N50_MANIFEST.md`、`generated/crossing_n50_evidence.json`、`generated/crossing_n50_section.tex` 和 `generated/crossing_n50_summary_sentence.tex`；readiness gate 检查 baseSeed=71、50 trials、trial seeds、scenario label、crossing window `[9,17]`、source SHA-256、whole-run/window metric coverage、paired CI/wins/p-values，以及 crossing-window full-vs-reference separation。主文 Discussion 只插入 concise generated sentence；完整 crossing table 保持 response-ready / supplement-candidate evidence。
...
TAES 投稿源文件首版已建立到 `docs/paper/taes/manuscript/`，当前可用 `./build.sh` 生成 `main.pdf`。这一版已经把 title/abstract/intro/method/theory/experiment/results/limitations/conclusion 和 DOI 核验过的核心引用落入真实 TAES template；方法图、实现 outline 和 N50 reduction 图已经过矢量化/版面 polish；baseSeed=11 的 held-out N50 robustness evidence、baseSeed=21 的 harsh packet-loss N50 stress evidence、baseSeed=31 的 topology-ring N50、baseSeed=41 的 partial-FOV N50、baseSeed=51 的 full-topology N50 scenario-family evidence 和 baseSeed=71 的 fixed crossing-window N50 evidence 也已接入正文/generated evidence/readiness gate。按“作者信息等可以先占位”的项目约定，当前目标应拆成 metadata-allowed content readiness 与 portal-submission readiness: 前者的非元数据 blocking gates 已闭合，但当前 `main.pdf` 为 10 页并触发 page-count warning；后者还要求作者/基金/repository/corresponding author 等投稿元数据替换完成并处理页数 warning。证据主线已覆盖主 tiered packet-loss profile、跨 seed held-out robustness、更严 packet-loss profile、sparse topology ring、partial-FOV sensing geometry、ideal full-neighborhood ceiling 和一个预指定 crossing window；full-topology 只解释为 zero-disagreement equivalence boundary，crossing-window 只解释为 fixed assignment-stability boundary，不作为 all-dynamics 或 birth/death guarantee。

本轮新增 `docs/AA_GENERALIZATION_SCENARIO_PROTOCOL_CN.md` 和通用 launcher `RUN/AA/launchAaTaesScenarioFamilySmoke.sh`。`RUN/AA/runAaBalancedCardinalityValidation.m` 新增可选 `scenarioOverrides` 参数，默认保持原 4+4 formation 实验不变；显式传入时可固定切换 `neighborMapMode` 或 FOV，并把 `scenarioLabel`、topology/FOV/sensor-motion metadata 写入 report。首批预注册场景族为 `topology-ring`、`partial-fov35` 和 `full-topology`，用于检验方法泛化和边界，不允许根据结果回调 `H`、threshold、projection cutoff、barycenter weights 或 label rules。当前 `topology-ring`、`partial-fov35` 和 `full-topology` 均已完成固定 N50 并接入 manuscript/readiness；其中 full-topology 被解析为 ideal full-neighborhood 的 zero-disagreement equivalence boundary。
...
2026-06-25 02:35 CST 本轮把 PDF visual QA 从代表页抽查升级为全页覆盖。`render_pdf_visual_qa.py` 现在生成六张代表页、9 张全页 PNG 和一个无字体依赖的 `main_contact_sheet.png`；`check_submission_readiness.py` 新增 `PDF visual QA full-page coverage` 与 `PDF visual QA contact sheet` gates，并把 stale-output cleanup 扩展到所有当前 PDF QA 产物。`./build.sh`、contact sheet 人工视觉检查、`generated/submission_readiness.json` 和解压后的 source-bundle fallback build 均通过；当时唯一 pending 是投稿元数据占位符，full-topology 状态已由后续完成并接入的 scenario-family checkpoint 覆盖。

2026-06-25 02:43 CST 本轮做了一次 Nature-style reviewer/polish 口径的摘要边界校验。摘要中原来的 `before existence and spatial fusion` 被收紧为 active Bernoulli output components 的 `label/moment correspondence`，避免把本文 output-level projection 误读成 Bernoulli existence consumer 的替代或前置输入。`check_submission_readiness.py` 新增 `abstract active-output boundary` gate，要求摘要保留 active-output/label-moment markers，并禁止旧的过强说法。`./build.sh` 后该 gate、abstract sentence load、implementation-alignment、PDF visual QA、TeX overfull 和 source-bundle freshness 均通过。

2026-06-25 02:49 CST 本轮新增 `paper-facing overclaim hygiene` gate，扫描摘要和正文中是否出现 unsupported universal / recursive / assignment-guarantee / fusion-weight-optimization claim patterns。当前稿件通过该 gate；它用于防止最终润色或元数据替换时把本文写成 replacing AA/KLA、optimizing fusion weights、guaranteeing correct assignment、validated recursive LMB、all target dynamics 或 all communication topologies 这类未由当前证据支持的强 claim。
...
2026-06-25 18:04 CST 本轮按用户确认，把 Nature 系列技能作为高水平论文质量层，而不是把目标期刊切换为 Nature。后续写作采用 `nature-writing` 的 algorithmic/research argument discipline、`nature-polishing` 的 reader-path / claim-evidence-boundary / overclaim hygiene，以及 `nature-figure` 的 figure-as-visual-argument 标准；但 journal axis 保持 generic，最终格式、页数、模板、source bundle、metadata、AI disclosure 和投稿规则全部继续服从 IEEE TAES / AESS 要求。`ml-paper-writing` 不再作为主写作路线，只在需要 ML 会议式结构对照时辅助参考。
...
| 算法设计与理论证明 | 已建立，online 化第一步通过 N50，本轮增强 | `docs/AA_LABEL_BARYCENTER_THEORY_CN.md`; `docs/paper/taes/manuscript/main.tex`; `multisensorLmb/applyCrossLocalLabelConsensusProjection.m` | 已整理 output-level projection 的可证明性质、reference-only ablation 解释、online/distributed label-barycenter AA 的收敛条件和边界；TAES 正文已补 stable-matching consensus limit，并新增 `Weighting is not matching` label-swap 反例命题；本轮又明确 projection 是 active-track label/moment 层，existence convexity 属于上游 AA existence consumer；本轮新增 `Sufficient assignment stability` 命题，把 Hungarian matching 正确性的 separation/error 充分条件写清楚；本轮新增 `Cardinality-equivalent control` 命题，把 reference-only/full 的 identical cardinality-only metrics 解释为共享 reference-cardinality projection 的结构后果；neighborhood iterative prototype 已完成 N50。 |
...
| TAES manuscript first draft | 已完成首版，可编译 PDF，但当前有页数 warning | `docs/paper/taes/manuscript/main.tex`; `docs/paper/taes/manuscript/main.pdf`; `docs/paper/taes/manuscript/references.bib` | 使用官方 `IEEEtaes.cls/.bst`，`tectonic` 编译通过，当前 `main.pdf` 为 10 页；这触发 TAES Regular Paper overlength-charge warning，需要正式上传前压回 9 页或接受页费风险。只剩 underfull/internal-consistency 类非阻塞 TeX warning，没有 overfull warning。 |
| TAES manuscript evidence-chain pass | 非元数据 blocking gate 已闭合，PDF 页数为 warning | `docs/paper/taes/manuscript/main.tex`; `docs/paper/taes/manuscript/references.bib`; `docs/paper/taes/manuscript/evidence_sources.json`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED1_20260623_232622.md`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED31_20260624_162439.md`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED41_20260624_182341.md`; `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED71_20260627_163915.md` | 已补近期 DOI 核验引用、Related Work 方法线、operator 伪代码、reference-label invariance、paired CI/wins/sign-test p 表、runtime 正文段和 full-width N50 reduction 图；Table IV 现在直接显示 full vs reference-only 的 sign-test evidence；scenario-family evidence path 已接入 topology-ring、partial-FOV、full-topology boundary，crossing evidence path 已接入 fixed-window N50，并把 source-package ledger 扩展到 harsh-loss、scenario-family 与 crossing-window boundary checks。 |
...
- Independent verifier 已经覆盖 network disagreement、runtime 和 local E-OSPA/RMSE/CardErr；baseSeed=11 held-out N50、harsh packet-loss N50 stress、topology-ring N50、partial-FOV N50、fixed crossing-window N50 和 full-topology N50 均已完成并接入 evidence manifest。当前剩余证据风险主要是 broader birth/death/maneuvering beyond the configured crossing window、covariance/reliability consistency 和 recursive-online guarded deployment，而不是 packet-loss severity、sparse-ring topology、partial-FOV、fixed crossing-window assignment-stability 或 ideal full-neighborhood ceiling。
...
- TAES 首稿已经可编译，且 N50 主表/paired 表/runtime 正文段/N50 reduction 图、held-out N50 table/paragraph、harsh stress summary/response fragment、topology-ring、partial-FOV、crossing-window 与 full-topology scenario summary/response fragments 都已经由 report-driven generated fragments 或 verifier 输出驱动；方法图和结果图已完成矢量化和 TAES-adjacent style calibration。最终 portal submission 仍需要作者/基金/repository/AI disclosure 最终措辞确认和人工审读；更强的 evidence-risk reduction 主要来自 broader birth/death/maneuvering beyond the configured crossing window、covariance/reliability consistency 和 recursive-online guarded deployment。
...
## 已完成长跑: maneuver/crossing baseSeed=71 N50 assignment-stability check

- 目的: 检查 fixed-design neighborhood label-barycenter 在预指定 close-crossing window 下是否仍保持 full-vs-reference spatial separation，同时避免用 whole-run average 掩盖 assignment-window 行为。
- 启动命令: `AA_CROSSING_TRIALS=50 AA_CROSSING_BASE_SEED=71 RUN/AA/launchAaTaesManeuverCrossingSmoke.sh`。
- PID file: `RUN/AA/AA_TAES_CROSSING_N50_BASESEED71_20260627_163913.pid`。
- Log: `RUN/AA/AA_TAES_CROSSING_N50_BASESEED71_20260627_163913.log`。
- 完成状态: 2026-06-27 生成 report `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED71_20260627_163915.md`。
- 关键结果: crossing-window full label-barycenter 相对 fixed spatial-KLA AA baseline 的 E-OSPA / RMSE / CardErr reductions 为 `11.08% / 16.99% / 25.02%`；reference-only 为 `4.98% / 5.38% / 25.02%`。Full RMSE wins `49/50`，E-OSPA wins `50/50`，均为 `p<10^-3`。CardErr 相同符合 shared reference-cardinality projection 解释，空间收益来自 matched posterior barycenter。
- 接入状态: `docs/paper/taes/manuscript/evidence_sources.json` 已把 `crossing_n50_report` 指向该 N50 report，`./build.sh` 生成 `generated/CROSSING_N50_MANIFEST.md`、`generated/crossing_n50_evidence.json`、`generated/crossing_n50_section.tex` 和 `generated/crossing_n50_summary_sentence.tex`；readiness checker 的 crossing source hash/protocol/metric coverage/full-vs-reference separation gates 已通过。主文 Discussion 只引入 concise generated sentence，完整 table 保留为 response-ready/supplement-candidate evidence。边界: 该结果不是 all target dynamics、birth/death、assignment guarantee 或 recursive-online claim。

## 已完成长跑: full-topology baseSeed=51 N50 ceiling check
...
- 完成状态: 2026-06-25 04:57 CST 前后生成 report `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED51_20260625_000938.md`。
- 关键结果: 三个 arms 的 Network OSPA、localization disagreement、cardinality dispersion 全部为 `0.000000`；local E-OSPA/RMSE/CardErr 也完全相同，分别为 `1.634331`、`3.277037`、`0.073400`。
- 接入状态: 已通过 scenario-family extractor、source hash freshness、readiness gate 和 source-bundle fallback build；主文只把它写成 ideal full-neighborhood 的 zero-disagreement equivalence boundary，不作为额外 method gain。

## 下一步

1. 保持 full-topology N50 的 ceiling/boundary 解释，不把 zero-disagreement equivalence 写成 deployed graph-local gain，也不据此回调 `H`、threshold、projection cutoff、barycenter weights 或 label rules。
...
4. 继续按 TAES + Nature-style reviewer 标准润色 `main.tex`: 控制 stress/held-out/scenario-family/crossing-window 证据进入正文的篇幅，避免超过 TAES page budget，并保持 fixed-baseline/no-search wording。
