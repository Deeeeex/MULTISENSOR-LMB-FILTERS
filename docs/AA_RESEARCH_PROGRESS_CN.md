...
| TAES manuscript held-out sanity pass | 已完成 sanity checkpoint，paper-grade N50 launcher 已准备 | `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N5_SEED11_20260622_172034.md`; `docs/paper/taes/manuscript/generated/HELDOUT_SANITY_MANIFEST.md`; `RUN/AA/launchAaTaesHeldoutN50BaseSeed11.sh` | Build now records a tracked N5 base-seed-11 sanity check: Network OSPA full `0.305452` vs tuned `1.702915`, local E-OSPA full `1.691451` vs tuned `2.032799`, RMSE full `3.450998` vs tuned `3.588145`; reference-only RMSE reduction is `-3.59%`. New launcher prepares N50/baseSeed=11 held-out run and refuses to start while the TAES local-verifier PID is active unless explicitly overridden. |
...
3. 运行 `RUN/AA/launchAaTaesHeldoutN50BaseSeed11.sh` 做 paper-grade held-out N50，再决定是否进入正文、supplementary，或只作为 response-ready robustness evidence。
