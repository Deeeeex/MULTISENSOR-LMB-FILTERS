...
def rows_to_metric_grid(rows: list[dict[str, object]]) -> tuple[list[int], list[str], dict[str, dict[str, np.ndarray]]]:
...
def save_plot(rows: list[dict[str, object]], output_base: Path) -> list[Path]:
    apply_style()
    levels, level_labels, grid = rows_to_metric_grid(rows)
    x = np.asarray(levels, dtype=float)
    offsets = {
        "Fixed Metropolis": -0.055,
        "Balanced mode": 0.0,
        "Cardinality-critical mode": 0.055,
    }

    fig, axes = plt.subplots(1, 3, figsize=(11.8, 3.75), constrained_layout=True)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--input-csv",
        type=Path,
        help="CSV generated by runMultisensorFilters_formation_4plus4_CommLevelThreeMethodCompare.",
    )
    parser.add_argument(
        "--mock",
        action="store_true",
    ...
    parser.add_argument(
        "--output-base",
        type=Path,
        default=Path(__file__).resolve().parent / "comm_level_three_method_compare_mock",
