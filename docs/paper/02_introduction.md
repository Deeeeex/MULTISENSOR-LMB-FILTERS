...
Motivated by this gap, this paper proposes a communication-aware adaptive fusion-weight allocation family for distributed GA-LMB fusion. The proposed design factorizes each fusion weight into covariance-quality, realized link-quality, and existence-confidence terms, and then uses branch-specific refinements for the two parts of the LMB posterior. A weak structure-aware decoupled KLA refinement regularizes the spatial branch, so graph structure acts only as a mild prior rather than the primary source of weight allocation. An FID-FIA-informed existence refinement then imports the Fisher-information-accumulation idea of Cao and Zhao \cite{CaoZhao2025InfoGeometryFusion}, but restricts it to the Bernoulli existence branch where it improves cardinality decisions without replacing the spatial fusion rule. This yields two recommended operating modes: a Balanced mode for spatial/position-sensitive operation and a Cardinality-critical mode for cardinality-sensitive operation.

The method is evaluated primarily in an eight-sensor distributed formation-tracking scenario composed of two four-sensor formations under a tiered heterogeneous packet-loss model. This setting preserves the historical average communication intensity while introducing persistent cross-node link heterogeneity, making it more suitable for assessing communication-aware weighting than a uniform packet-drop model. A supporting ideal-communication comparison is also included to test whether the same weighting logic remains useful when link degradation is removed. The evaluation treats computational cost as part of the metric system rather than as an implementation footnote, because practical distributed fusion must trade consensus gain against filter/fusion runtime. The experimental results show that adaptive weighting substantially improves consensus quality over fixed-weight fusion, that existence confidence provides the missing gain beyond covariance-and-link-only weighting, and that restricting FID-FIA to the existence branch improves target-number agreement while retaining the spatial-consensus advantages of the communication-aware KLA backbone.
...
3. We demonstrate in a tiered packet-loss dual-formation eight-sensor scenario, supported by an ideal-communication comparison and runtime measurement, that the proposed design improves consensus performance over fixed-weight and partial-factor baselines, and we provide a usage-oriented distinction between the low-overhead Balanced mode and the higher-cost Cardinality-critical mode.
