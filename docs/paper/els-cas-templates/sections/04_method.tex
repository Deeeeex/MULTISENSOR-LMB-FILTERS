...
The retained \emph{Balanced mode} uses the three-factor adaptive backbone, decoupled spatial and existence scores, and weak structure-aware refinement. It uses the instantaneous normalized weights: EMA smoothing and final-weight floors are disabled on both branches. The controlled component ablation shows that the former stabilization pair improves local RMSE slightly but degrades network disagreement, local E-OSPA, and local cardinality error, so it is not part of the recommended configuration.

The \emph{Cardinality-critical mode} inherits the same no-stabilization configuration and changes only the existence score through the FID-FIA cue. The 50-trial evaluation shows that this mode further reduces cardinality dispersion and local cardinality error, while increasing localization disagreement, local RMSE, and computation relative to Balanced. The two modes are therefore retained for different deployment priorities rather than ranked by a single aggregate score.
...
After score normalization, the resulting branch weights are used directly. This preserves responsiveness to realized packet delivery and allows a sufficiently weak neighbor to receive a near-zero weight. EMA smoothing and minimum-weight enforcement remain implementation options for diagnostics, but they are disabled in the retained method.
