...
The paper body focuses on GA/KLA fusion because the method is designed around conservative logarithmic pooling and branch-specific GA-LMB fusion weights. A secondary arithmetic-average experiment was run with the earlier stabilized weight configuration. It is retained only as design-history evidence and is not used to support the current no-stabilization Balanced configuration. The diagnostic shortens the AA run horizon and applies stronger AA-specific pruning to keep mixture growth tractable.
...
\noindent\begin{minipage}{\linewidth}
  ...
  \begin{tabular}{lccc}
    ...
    stabilized branch-decoupled AA & 3.449 & 4.306 & 0.102 \\
    stabilized FID-FIA AA & 3.457 & 4.328 & 0.095 \\
    \bottomrule
  \end{tabular}
\end{minipage}
\par\medskip

The stabilized adaptive AA variants reduce disagreement relative to fixed AA, but their configuration no longer matches the retained GA method. These values therefore motivate a future no-stabilization AA rerun rather than a transfer claim in the present paper.

\subsection{NIS-based consistency weighting}

This appendix-level experiment studies whether innovation-based consistency information should enter the adaptive weight model. NIS is a standard filter-diagnostic statistic in tracking and navigation, and recent work also uses related distributional checks for estimator auto-tuning \cite{BarShalom2001Estimation,Chen2023NISAutoTuning}. The relevant comparison here is $\texttt{w/o NIS} \rightarrow \texttt{robust NIS} \rightarrow \texttt{plain NIS}$. The NIS term is treated as a consistency penalty rather than a monotonic quality reward because innovation consistency is structurally coupled with covariance-based posterior quality.
...
Taken together, these appendix results delimit the explored design space. Only components supported by the controlled no-stabilization GA experiment are promoted into the retained method.
