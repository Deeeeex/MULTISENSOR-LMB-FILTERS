...
The main claim is that communication-aware adaptive weighting substantially improves distributed consensus quality in the eight-sensor dual-formation GA-LMB scenario under tiered heterogeneous packet loss. Starting from fixed Metropolis weights, the current best adaptive configuration combines covariance, realized link quality, existence confidence, and a weak structure-aware decoupled KLA refinement. The latest 20-trial main comparison also includes a Cao-Zhao FID-FIA baseline. Fixed weights yield consensus OSPA `2.453677`, consensus RMSE `2.335508`, and consensus cardinality disagreement `0.714625`. The Cao-Zhao FID-FIA baseline yields `1.820229`, `1.647412`, and `0.126188`. The current best adaptive method yields `1.785873`, `1.562521`, and `0.192938`.

These reductions are sizeable in all three consensus metrics. Relative to fixed weights, the Cao-Zhao FID-FIA baseline lowers consensus cardinality disagreement by `82.34%`, making it the strongest cardinality-consensus baseline. The current best adaptive method lowers consensus OSPA by `27.22%` and consensus RMSE by `33.10%`, making it the strongest spatial-consensus method. This result is important because the method is not being evaluated in a benign homogeneous communication setting. The communication layer deliberately preserves the historical mean packet-loss rate while introducing persistent cross-node heterogeneity. The observed gain therefore supports the central paper claim that fusion weights should depend not only on posterior concentration but also on realized communication reliability and existence decisiveness.

The local truth-referenced metrics clarify the cardinality story. FID-FIA has the lowest local cardinality error, `0.392313`, so its cardinality-consensus advantage is not merely agreement around a wrong target count. Its tradeoff is local RMSE, where it reaches `1.715746`, compared with `1.598561` for the current best adaptive method.
