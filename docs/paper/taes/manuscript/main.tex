...
Heterogeneity and field-of-view mismatch create a separate difficulty. RFS fusion across heterogeneous sensors can be formulated without assuming identical local multi-object densities \cite{YiChai2021HeterogeneousRFS}, and labeled RFS fusion with different fields of view has been studied explicitly \cite{Gao2022DifferentFOV}. These lines motivate robust fusion under nonidentical local views. They do not by themselves remove the output-level failure in which two local LMB filters carry different labels for the same physical target after packet loss.
