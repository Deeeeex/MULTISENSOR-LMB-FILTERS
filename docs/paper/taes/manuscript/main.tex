...
\caption{Active-output correspondence projection. The composite panel shows why scalar fusion weights specify neighbor probability mass but not component correspondence, then gives the graph-local reference, assignment, and moment-barycenter projection used in validation. The input is the active neighborhood LMB output, not a global label dictionary. The operator consumes only graph-neighborhood outputs, passes upstream active-track existence scores through, and rewrites only labels and Gaussian moments.}
\label{fig:method}
\end{figure*}
...
\caption{Mechanism separation in paired percentage reductions. Values are the paired reductions in Table~\ref{tab:paired} relative to the fixed spatial-KLA AA baseline. Blue bars show the full label-barycenter operator, and gray-blue bars show the reference-only ablation. The cardinality rows are shared reference-cardinality controls, not moment-barycenter effects. Spatial E-OSPA/RMSE separation appears only after matched posterior barycenters are added.}
\label{fig:n50}
\end{figure*}
