...
\end{proposition}
\begin{proof}
The Euclidean least-squares objective in $q$ is minimized by the sample mean, giving $\bar{\mu}=\frac{1}{M}\sum_i\mu_i$ and $\bar{M}=\frac{1}{M}\sum_i M_i$. Substituting $M_i=\Sigma_i+\mu_i\mu_i^T$ and subtracting $\bar{\mu}\bar{\mu}^T$ gives the displayed covariance expression; each summand is positive semidefinite. The mean coordinate also satisfies $\sum_i\|\mu_i-z\|_2^2=\sum_i\|\mu_i-\bar{\mu}\|_2^2+M\|z-\bar{\mu}\|_2^2$, so the update removes within-group mean disagreement after correspondence is fixed.
\end{proof}
