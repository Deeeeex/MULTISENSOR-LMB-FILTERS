...
\caption{Active-output correspondence projection. Scalar weights route probability mass but do not infer component correspondence. The operator chooses a graph-local reference, assigns neighbor tracks to it, and moment-matches the assigned states. Existence scores pass through from upstream AA; only labels and Gaussian moments are rewritten, with no global label dictionary.}
\label{fig:method}
\end{figure*}
...
\caption{Mechanism separation in paired percentage reductions. Blue bars show the full label-barycenter operator, gray-blue bars show the reference-only ablation, and the right inset repeats the RMSE separation for the primary and held-out N50 runs. Cardinality rows are shared controls; the spatial E-OSPA/RMSE gap appears only after matched posterior barycenters are added.}
\label{fig:n50}
\end{figure*}
...
\IfFileExists{generated/crossing_n50_summary_sentence.tex}{\input{generated/crossing_n50_summary_sentence.tex}\par}{}

These checks broaden packet-loss severity, sparse-topology coverage, partial-field-of-view sensing, and one pre-specified maneuver/crossing window. They still should not be read as validation of all maneuvering-target, birth/death, covariance-consistency, or recursive-online settings.
