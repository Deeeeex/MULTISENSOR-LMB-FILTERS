...
稿件已经进入 metadata allowance 下的 `candidate_with_warnings` 阶段: TAES 模板、核心方法叙事、理论性质、N50 主实验、ablation、runtime、带 CI/wins/sign-test p 的 baseSeed=11 held-out N50 robustness check、harsh packet-loss N50 stress check、topology-ring N50、partial-FOV N50、fixed maneuver/crossing-window N50 和 full-topology N50 checks、以及 disclosure skeleton 都已经落到 `main.tex` 和 `generated/` evidence fragments 中。按“作者/基金/repository 可先占位”的项目约定，当前非元数据 mechanical/evidence/citation/PDF/source-bundle blocking gates 已经闭合；`generated/SUBMISSION_READINESS_REPORT.md` 显示 metadata allowance 后无 blocking gate，但 `main.pdf` 现在是 10 TAES-template pages，触发 page-count warning。本轮已把 reproducibility ledger 扩展为覆盖 primary AA N50、held-out AA N50、harsh-loss AA N50、scenario-family boundary checks、crossing-window boundary check、contextual GA N50 和 independent verifier，并把 topology-ring、partial-FOV、crossing-window 与 full-topology 均升级为 paper-grade parsed evidence；其中 full-topology 被解释为 zero-disagreement equivalence boundary，而 crossing-window 被解释为 fixed assignment-stability boundary，不是 all-dynamics guarantee。method section 和 readiness checker 已明确 projection 是 labels/moments-only 的 active-track 层，不重估 Bernoulli existence。稿件还不能标记为 portal-submission-ready，因为投稿元数据仍缺最后闭环:
...
2. 当前 `main.pdf` 为 10 页，TAES Regular Paper 的 overlength-charge warning threshold 已经被触发；正式上传前需要压回 9 页或明确接受页费风险。
3. 当前实证已从主 tiered packet-loss formation 扩展到更严 harsh packet-loss profile、sparse topology-ring N50、partial-FOV N50、fixed crossing-window N50 和 full-topology zero-disagreement ceiling；covariance-consistency、recursive-online 和更广 birth/death/maneuvering 场景族验证仍会降低审稿风险。

## 2026-06-27 Crossing-Window N50 Checkpoint

本轮把固定 maneuver/crossing assignment-stability N50 接入投稿证据链。原始报告为 `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED71_20260627_163915.md`，配置为 `Trials=50`、`baseSeed=71`、`scenarioLabel=maneuver-crossing-assignment`、`crossingWindow=[9,17]`。`evidence_sources.json` 已加入 `crossing_n50_report`，`extract_crossing_evidence.py` 生成 `generated/CROSSING_N50_MANIFEST.md`、`generated/crossing_n50_evidence.json`、`generated/crossing_n50_section.tex` 和 `generated/crossing_n50_summary_sentence.tex`。

Crossing-window 结果支持 fixed-window full-vs-reference separation: full label-barycenter 的 E-OSPA / RMSE reductions 为 `11.08% / 16.99%`，reference-only 为 `4.98% / 5.38%`；CardErr reduction 两者同为 `25.02%`，符合 shared reference-cardinality control 的结构解释。主文 Discussion 只插入 concise generated sentence，完整表格保留为 response-ready evidence。边界保持为: 该结果不是 all target dynamics、birth/death、assignment-guarantee、covariance-consistency 或 recursive-online claim。
