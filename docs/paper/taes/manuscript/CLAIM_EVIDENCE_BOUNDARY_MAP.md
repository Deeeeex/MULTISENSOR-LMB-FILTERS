...
| Related Work | Position AA/KLA, heterogeneous RFS fusion, label-matching/multiview LMB work, assignment, and consensus as adjacent foundations. | The complementarity claim: correspondence projection is orthogonal to density-pooling weights and distinct from existing label-matching or multiview LMB lines by its active-output projection and reference-only ablation. | A paper-by-paper catalog without the missing-correspondence throughline, or an overclaim that prior label-matching work ignores labels. |
...
| Structural Properties | Give conditional algebraic support and formal boundaries. | Fixed-time active-output analysis scope, sufficient assignment stability, first-two-moment least-squares projection, barycenter-versus-copying inequality, and cardinality-equivalent reference-control logic. | Claiming finite-round optimality or guaranteed assignment under crossings/births/deaths. |
| Experimental Setup | Define the mechanism-isolation protocol and fixed parameters. | Same seeds/measurements across arms, fixed target-wise scalar-weight baseline, reference-only ablation, no per-scenario search. | Treating held-out/boundary runs as tuning sources or confusing raw validation-report implementation labels with the paper-facing `Fixed spatial-KLA AA` baseline. |
...
| Discussion and Limitations | Interpret why the ablation supports matched barycentering and where the method can fail. | Assignment ambiguity, fixed crossing-window boundary evidence, covariance/reliability limits, recursive lifecycle boundary. | Turning one crossing-window check into all-dynamics or recursive-deployment evidence. |
...
| Recent LMB literature already includes multiview and efficient-label-matching directions adjacent to this problem. | Related Work | `Jin2023MultiviewLMB`; `Ding2025EfficientLabelMatching`; DOI resolver report | Citation-backed positioning | Use these as adjacent foundations and contrasts; do not claim they are inferior or that they did not address label matching. |
| The proposed operator canonicalizes labels by a graph-local reference and assignment before replacing matched Gaussian states by first-two-moment barycenters. | Method, Fig. 1 composite implementation panel | TikZ fragment `generated/method_pipeline.tex`; `figures/fig_method_pipeline.svg`; `scripts/render_figures.py` | Source-backed, machine-generated, and rendered in the PDF | Keep the method as an output-space projection layer; do not call it a new density-pooling rule. |
...
| Under fixed correct correspondence, the moment update is the least-squares representative in first-two-moment coordinates, equals the first two moments of an equally weighted matched Gaussian mixture, and has no larger within-group squared moment error than copying any matched reference input. | Structural Properties | `Moment-space projection` proposition and proof in `main.tex` | Algebraic identity with positive-semidefinite covariance expression and a bias-variance decomposition against reference copying | This is not a covariance-consistency, reliability-optimality, or truth-error guarantee when assignment is wrong. |
...
| Reference-only and full projections have identical cardinality-only metrics on the same trials because they share the median-cardinality reference-control path. | Structural Properties, Results | `Cardinality-equivalent control` proposition; `tab:paired`; generated N50/held-out/stress/scenario paired rows | Structural argument plus report-driven equality of Card. disp./CardErr effects | Use cardinality-only metrics as evidence for the shared reference-cardinality projection, not as evidence that moment barycentering improves localization. |
| Matched posterior barycenters, not label copying alone, drive the main spatial tracking gain. | Results, `tab:paired`, `fig:n50`, Discussion | Main N50 full-vs-reference-only ablation; generated TikZ paired-reduction panel with held-out RMSE inset; held-out N50 replication; harsh-loss, topology-ring, partial-FOV, crossing-window, and full-topology equivalence-boundary generated evidence | Report-driven fragments plus independent verifier for the main N50 local metrics and source-hash/readiness gates for optional boundary evidence | Do not collapse reference-only and full effects; cardinality effects are shared by both arms, crossing evidence is fixed-window only, and full-topology is a zero-disagreement ceiling rather than a gain claim. |
| The full operator improves both network agreement and local tracking metrics relative to the fixed target-wise spatial-KLA AA baseline in the primary N50 validation. | Abstract, Results, `tab:n50`, `tab:paired`, `fig:n50` | `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED1_20260623_232622.md`; `generated/n50_evidence.json`; `generated/N50_VERIFICATION_REPORT.md` | Primary report parsed and independently recomputed for network, runtime, and local metrics | State the scenario: eight-sensor tiered packet-loss formation with fixed method parameters; keep raw validation-report arm labels in provenance artifacts, not as the paper-facing baseline name. |
...
| Harsh packet loss, topology-ring, partial-FOV, and full-topology checks broaden the boundary evidence without forming a tuning loop. | Discussion, Supplementary/response evidence package | `generated/STRESS_HARSH_MANIFEST.md`; `generated/SCENARIO_FAMILY_MANIFEST.md`; `generated/stress_harsh_summary_sentence.tex`; `generated/scenario_family_summary_sentence.tex` | Fixed-parameter generated fragments and source hashes | Keep these as boundary checks within the formation family; full-topology is an idealized zero-disagreement equivalence boundary and does not replace maneuver, covariance, or recursive-online validation. |
| Maneuver/crossing evidence is an assignment-stability boundary check based on pre-specified crossing-window metrics. | Discussion; Supplementary/response evidence package; optional reviewer response | `RUN/AA/AA_BALANCED_CARDINALITY_VALIDATION_N50_SEED71_20260627_163915.md`; `generated/CROSSING_N50_MANIFEST.md`; `generated/crossing_n50_summary_sentence.tex`; `generated/crossing_n50_section.tex`; `generated/crossing_n50_evidence.json` | Fixed-protocol N50 source-hash/readiness gate with crossing-window full-vs-reference separation | Do not use N1/N5 smoke or whole-run averages as paper-facing crossing evidence; even N50 crossing evidence does not prove general assignment correctness under all births, deaths, or close maneuvers. |
...
- The method does not guarantee correct assignment during close crossings, births, deaths, or cardinality disagreement; optional crossing-window evidence can only reduce this risk within its fixed scenario.
...
- The parsed full-topology N50 result must be used only as an idealized full-neighborhood zero-disagreement equivalence boundary, not as additional method gain or as the main deployed graph-local topology.
