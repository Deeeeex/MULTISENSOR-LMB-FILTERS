...
| The proposed operator canonicalizes labels by a graph-local reference and assignment before replacing matched Gaussian states by first-two-moment barycenters. | Method, Fig. 1 composite implementation panel | `generated/method_pipeline.tex`; `figures/fig_method_pipeline.svg`; `scripts/render_figures.py` | Source-backed, machine-generated, and rendered in the PDF | Keep the method as an output-space projection layer; do not call it a new density-pooling rule. |
