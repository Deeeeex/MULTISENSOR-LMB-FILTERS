...
The evaluation is fixed-design and report-driven. The manuscript compares the method with a fixed target-wise spatial-KLA AA baseline, a reference-only label-consensus ablation, and contextual GA reference rows. Raw validation reports retain their internal implementation labels for provenance, but the submitted manuscript uses fixed-baseline terminology throughout. It also includes a separate 50-trial held-out replication, a harsh packet-loss N50 stress check, fixed-parameter topology-ring and partial-FOV N50 scenario-family checks, and a full-topology N50 equivalence-boundary check. The source package keeps these evidence roles explicit through generated manifests, an independent N50 verifier, DOI-resolved references, rendered PDF visual QA, and a reproducible source bundle.
