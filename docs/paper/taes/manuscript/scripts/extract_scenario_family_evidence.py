...
def reduction_value(value: str) -> float:
    if re.search(r"\bNaN\s*%", value, flags=re.IGNORECASE):
        return math.nan
...
def positive_reduction(value: str) -> bool:
    parsed = reduction_value(value)
...
def metric_close(lhs: float, rhs: float, tol: float = 1.0e-9) -> bool:
    return abs(lhs - rhs) <= tol


def zero_disagreement_ceiling(payload: dict[str, object]) -> bool:
    config = payload["config"]
    if config.get("neighbor_map_mode") != "full":
        return False
    network = payload["network"]
    local = payload["local"]
    for metric in NETWORK_METRICS:
        if any(abs(float(network[arm][metric])) > 1.0e-9 for arm in ARM_ORDER):
            return False
    for metric in LOCAL_METRICS:
        baseline = float(local[ARM_ORDER[0]][metric])
        if any(not metric_close(float(local[arm][metric]), baseline) for arm in ARM_ORDER[1:]):
            return False
    return True


def classify_result(payload: dict[str, object]) -> str:
    if zero_disagreement_ceiling(payload):
        return "zero_disagreement_ceiling_equivalence"
    paired_network = payload["paired_network"][FULL_ARM]
    paired_local = payload["paired_local"][FULL_ARM]
    ospa_ok = positive_reduction(paired_network["OSPA"]["reduction"])
    loc_ok = positive_reduction(paired_network["Loc. disag."]["reduction"])
    eospa_ok = positive_reduction(paired_local["E-OSPA"]["reduction"])
    rmse_ok = positive_reduction(paired_local["RMSE"]["reduction"])
...
def scenario_row(payload: dict[str, object]) -> str:
    ...
    def display_reduction(value: str) -> str:
        parsed = reduction_value(value)
        return "n/a" if not math.isfinite(parsed) else value
    ...
    fields = [
        ...
        tex_escape(display_reduction(paired_network["OSPA"]["reduction"])),
        tex_escape(display_reduction(paired_network["Loc. disag."]["reduction"])),
        tex_escape(display_reduction(paired_local["RMSE"]["reduction"])),
        tex_escape(display_reduction(paired_network["Card. disp."]["reduction"])),
    ]
...
\begin{{table*}}[t]
\caption{{Scenario-family AA checks under fixed method parameters. Reductions are relative to the fixed spatial-KLA AA baseline; n/a marks zero-denominator ceiling metrics. These runs vary topology, sensing geometry, or an idealized full-neighborhood ceiling without per-scenario threshold, barycenter-weight, or label-rule search; smoke tiers are recorded as boundary probes, not paper-grade statistical evidence.}}
...
The scenario-family fragment records {payload['scenario_count']} configured checks. The tier split is {paper_grade_phrase} and {smoke_phrase}. These artifacts support supplement or reviewer-response use, and main-text claims should remain restricted to paper-grade fixed-parameter runs.
"""
...
def write_summary_sentence(payload: dict[str, object]) -> None:
    ...
    for item in paper_grade:
        ...
        if item.get("interpretation_class") == "zero_disagreement_ceiling_equivalence":
            local = item["local"][FULL_ARM]
            ceiling_clauses.append(
                f"The {tex_escape(scenario_name)} check gives zero network disagreement and identical local E-OSPA/RMSE/CardErr "
                f"({float(local['E-OSPA']):.3f}/{float(local['RMSE']):.3f}/{float(local['CardErr']):.3f})"
            )
            continue
        positive_clauses.append(
            f"{tex_escape(scenario_name)} "
            f"(RMSE {tex_escape(paired_local['RMSE']['reduction'])} vs reference-only "
            f"{tex_escape(ref_local['RMSE']['reduction'])})"
        )
    topics = ["sparse topology", "partial-field-of-view sensing"]
    if ceiling_clauses:
        topics.append("full-neighborhood ceiling")
    lead = "Fixed-parameter N50 scenario-family checks probe " + ", ".join(topics[:-1]) + ", and " + topics[-1] + ". "
    parts = []
    if positive_clauses:
        parts.append("Full-versus-reference RMSE separation persists in " + "; ".join(positive_clauses) + ".")
    if ceiling_clauses:
        parts.append("; ".join(ceiling_clauses) + ", marking an equivalence boundary rather than a gain claim.")
