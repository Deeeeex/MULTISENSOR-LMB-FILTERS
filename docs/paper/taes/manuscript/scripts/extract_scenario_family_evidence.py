...
The scenario-family parser records {payload['scenario_count']} configured checks. {payload['paper_grade_count']} are paper-grade N50-or-larger checks and {payload['smoke_count']} are smoke-tier checks. These artifacts are intended for supplement or reviewer-response use unless the corresponding tier is upgraded to a paper-grade run.
"""
...
def write_summary_sentence(payload: dict[str, object]) -> None:
    ...
    sentence = (
        "% Auto-generated by scripts/extract_scenario_family_evidence.py.\n"
        f"A fixed-parameter {tex_escape(scenario_name)} N50 check "
        f"(base seed {config['base_seed']}, topology {tex_escape(config['neighbor_map_mode'])}) "
        f"also supports the mechanism boundary: the full operator reduces network OSPA by "
