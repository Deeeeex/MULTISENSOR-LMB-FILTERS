...
\begin{{table*}}[t]
\caption{{Scenario-family AA checks under fixed method parameters. Reductions are relative to the fixed spatial-KLA AA baseline, reported under the validation label Tuned spatial-KLA AA. These runs vary topology or sensing geometry without per-scenario threshold, barycenter-weight, or label-rule search; smoke tiers are recorded as boundary probes, not paper-grade statistical evidence.}}
...
def write_summary_sentence(payload: dict[str, object]) -> None:
    ...
    for item in paper_grade:
        ...
        clauses.append(
            f"In the {tex_escape(scenario_name)} case "
            f"(base seed {config['base_seed']}, topology {tex_escape(config['neighbor_map_mode'])}, "
            f"FOV {float(config['sensor_fov_half_angle_deg']):.0f} deg), "
            "the full operator reduces network OSPA/local E-OSPA/RMSE by "
            f"{tex_escape(paired_network['OSPA']['reduction'])}/"
            f"{tex_escape(paired_local['E-OSPA']['reduction'])}/"
            f"{tex_escape(paired_local['RMSE']['reduction'])}, "
            f"compared with a {tex_escape(ref_local['RMSE']['reduction'])} RMSE reduction for reference-only"
        )
    if len(clauses) == 1:
        lead = (
            "An additional fixed-parameter N50 scenario-family check probes a topology "
            "or sensing-geometry change. "
        )
    else:
        lead = (
            "Additional fixed-parameter N50 scenario-family checks probe topology "
            "and sensing-geometry changes. "
        )
