...
def write_summary_sentence(payload: dict[str, object]) -> None:
    ...
    for item in paper_grade:
        ...
        if item.get("interpretation_class") == "zero_disagreement_ceiling_equivalence":
            ...
            ceiling_clauses.append(
                f"{tex_escape(scenario_name)} gives zero network disagreement and identical local E-OSPA/RMSE/CardErr "
    ...
    topics = ["topology", "sensing-geometry"]
    if ceiling_clauses:
    ...
    if ceiling_clauses:
        parts.append("; ".join(ceiling_clauses) + ", so it is an equivalence boundary rather than a gain claim.")
