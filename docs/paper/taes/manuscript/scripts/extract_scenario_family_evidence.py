...
\begin{{table*}}[t]
\caption{{Scenario-family AA checks under fixed method parameters. Reductions are relative to the fixed spatial-KLA AA baseline. These runs vary topology or sensing geometry without per-scenario threshold, barycenter-weight, or label-rule search; smoke tiers are recorded as boundary probes, not paper-grade statistical evidence.}}
