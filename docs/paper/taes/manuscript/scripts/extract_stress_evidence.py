...
The harsh packet-loss stress run keeps the design fixed before the stress N50 result is observed: base seed {config['base_seed']}, {config['trials']} paired trials, existence threshold {config['existence_threshold']:.2f}, and no per-scenario search over projection cutoff, barycenter weights, label rules, or thresholds. Under this profile, the full operator changes network OSPA by {tex_escape(full_network['OSPA']['reduction'])}, local E-OSPA by {tex_escape(full_local['E-OSPA']['reduction'])}, and local RMSE by {tex_escape(full_local['RMSE']['reduction'])} relative to the fixed spatial-KLA AA baseline. The RMSE mechanism-control comparison is full {full_local['RMSE']['wins']} with {tex_pvalue_expr(full_local['RMSE']['p_value'])} versus reference-only {ref_local['RMSE']['wins']} with {tex_pvalue_expr(ref_local['RMSE']['p_value'])}. This fragment is scenario-family stress evidence, not a tuning target.
"""
    (OUT / "stress_harsh_section.tex").write_text(section, encoding="utf-8")
    summary = (
        "% Auto-generated by scripts/extract_stress_evidence.py.\n"
        "The harsh packet-loss check kept the same fixed parameters and evaluated "
        f"$p_\\mathrm{{drop}}\\in{config['p_drop_levels']}$ with sensor counts "
        f"{config['p_drop_level_counts']}. "
        "Under this pre-specified setting, the full operator preserves the full-versus-reference separation: "
        f"network OSPA falls by {tex_escape(full_network['OSPA']['reduction'])}, "
        f"local E-OSPA by {tex_escape(full_local['E-OSPA']['reduction'])}, "
        f"and RMSE by {tex_escape(full_local['RMSE']['reduction'])}, "
        f"whereas reference-only RMSE falls by {tex_escape(ref_local['RMSE']['reduction'])}.\n"
    )
