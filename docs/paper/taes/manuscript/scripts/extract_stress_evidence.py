...
"""
    ...
    summary = (
        ...
        f"The fixed-parameter harsh packet-loss stress check shifts the packet-loss profile to "
        f"$p_\\mathrm{{drop}}\\in{config['p_drop_levels']}$ over sensor counts {config['p_drop_level_counts']} "
        "and preserves the mechanism separation: "
        f"the full operator reduces network OSPA by {tex_escape(full_network['OSPA']['reduction'])}, "
        f"local E-OSPA by {tex_escape(full_local['E-OSPA']['reduction'])}, "
        f"and RMSE by {tex_escape(full_local['RMSE']['reduction'])}, "
        f"whereas reference-only reduces RMSE by {tex_escape(ref_local['RMSE']['reduction'])}.\n"
    )
