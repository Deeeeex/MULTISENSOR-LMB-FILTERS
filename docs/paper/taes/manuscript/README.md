...
In the full repository, the build first regenerates the N50 evidence fragments, regenerates contextual reference rows, records optional held-out/stress evidence, runs the verifier, renders figures, writes the reproducibility ledger, then compiles the TAES PDF. In an unpacked source-bundle directory where the raw `RUN/` reports are not present, `./build.sh` automatically falls back to compiling from the bundled `generated/` fragments. Use `TAES_EVIDENCE_MODE=refresh ./build.sh` to require raw evidence regeneration, or `TAES_EVIDENCE_MODE=bundled ./build.sh` to force bundled-fragment compilation. Do not edit `generated/n50_*.tex`, `generated/reference_baseline_rows.tex`, `generated/reproducibility_ledger*.tex`, or `generated/method_pipeline.tex` by hand; update the validation report, extraction script, verifier, or figure renderer and rerun the full-repository build.
