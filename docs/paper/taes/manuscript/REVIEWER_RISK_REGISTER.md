...
| Equal moment barycenters may ignore covariance/reliability quality. | The equal barycenter is a deliberately simple design used to isolate correspondence repair after target-wise AA routing is fixed. | Method definition, Discussion limitations, and fixed-design experimental setup. | Covariance-aware or reliability-weighted barycenters remain future design points; covariance-consistency validation is not part of the current evidence. |
