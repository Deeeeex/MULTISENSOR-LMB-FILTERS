...
| The baseline or method may look tuned to the reported data. | The manuscript now uses `fixed target-wise spatial-KLA AA baseline` for paper-facing comparisons and states that `Tuned spatial-KLA AA` is the retained validation-report implementation label. The Experimental Setup also states that all paper-facing comparisons use a fixed parameterization rather than per-scenario retuning, with no per-scenario search over projection cutoffs, barycenter weights, thresholds, or trial-specific label rules. | Abstract, Introduction, Experimental Setup, `fixed-design baseline wording` readiness gate, and `CLAIM_EVIDENCE_BOUNDARY_MAP.md`. | Do not describe held-out, harsh-loss, topology-ring, partial-FOV, or future full-topology runs as tuning sources. Keep method-level innovations separate from earlier diagnostic baseline development. |
