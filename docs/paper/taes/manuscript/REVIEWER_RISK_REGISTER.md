...
Visible evidence base: primary paired N50 validation, reference-only ablation, held-out N50 replication, harsh packet-loss N50 stress check, topology-ring, partial-FOV, and full-topology N50 scenario-family checks, contextual GA rows, independent N50 verifier, DOI resolver check, PDF visual QA, and source-bundle rebuild.

Missing materials affecting confidence: final author/funding/repository metadata, completed and parsed maneuvering/crossing N50 validation, covariance-consistency validation, and recursive-online LMB validation with lifecycle guards. The parsed full-topology ceiling result is informative only as a zero-disagreement equivalence boundary, not as additional method gain. The method-level follow-up design for the remaining validation families is recorded in `docs/AA_NEXT_STAGE_GENERALIZATION_PROTOCOL_CN.md`; the optional crossing evidence path is response-ready only after `crossing_n50_report` is configured and passes readiness checks.
...
- Consensus technical risks: recursive LMB validity, assignment ambiguity, covariance/reliability weighting, target maneuvers, and possible overinterpretation of the full-topology equivalence ceiling.
...
- Not assessable from current evidence: recursive online filtering with feedback, births, deaths, completed N50 close-crossing behavior, target maneuvers, and lifecycle-consistency guards.
...
- Follow-up protocol status: `docs/AA_NEXT_STAGE_GENERALIZATION_PROTOCOL_CN.md` defines fixed no-search candidate validations for maneuver/crossing assignment stress, covariance/reliability mismatch, and recursive guarded projection. The optional crossing parser/readiness path exists, but crossing evidence is not current manuscript evidence until the N50 report is configured and parsed.
...
| Generality beyond the main formation scenario may be limited. | The manuscript includes held-out, harsh packet-loss, topology-ring, partial-FOV, and full-topology checks while keeping parameters fixed. | `generated/HELDOUT_N50_MANIFEST.md`, `generated/STRESS_HARSH_MANIFEST.md`, `generated/SCENARIO_FAMILY_MANIFEST.md`, and `SUPPLEMENTARY_EVIDENCE_PACKAGE.md`. | These checks do not substitute for maneuvering-target, covariance-consistency, or recursive-online validation. |
| Close-crossing assignment stability may be questioned. | A fixed maneuver/crossing N50 parser and readiness path is available so any completed run is evaluated by crossing-window RMSE/E-OSPA and full-vs-reference separation, not whole-run averages. | `scripts/extract_crossing_evidence.py`, `generated/CROSSING_N50_MANIFEST.md`, `generated/crossing_n50_section.tex`, and `docs/AA_NEXT_STAGE_GENERALIZATION_PROTOCOL_CN.md`. | Current N1/N5 smoke reports are structural checks only; until `crossing_n50_report` is configured and parsed, crossing remains a reviewer-risk boundary rather than a manuscript claim. |
