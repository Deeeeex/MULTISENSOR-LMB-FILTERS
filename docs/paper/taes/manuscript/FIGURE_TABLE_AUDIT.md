...
| Paired-reduction bar figure | `fig:n50` | Make the full-versus-reference separation visually inspectable with bar-end percentage labels and a self-contained caption that names the table source, color mapping, and cardinality-control boundary. | `scripts/extract_n50_evidence.py`; `generated/n50_reduction_bars.tex`; visual QA page `main_all_p08.png`. | Visualizes table-driven reductions only; the identical cardinality-error row is a shared reference-cardinality control, while E-OSPA/RMSE separation supports matched moment barycentering. |
