...
| Maneuver/crossing assignment-stability table | `tab:crossing-window` | Preserve fixed crossing-window reductions and full-vs-reference separation for reviewer response use. | `generated/crossing_n50_section.tex`; `generated/CROSSING_N50_MANIFEST.md`. | Optional only after `crossing_n50_report` is configured and parsed; do not substitute N1/N5 smoke or whole-run averages for crossing-window evidence. |
