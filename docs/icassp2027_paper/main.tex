...
\begin{figure*}[t]
    ...
    \caption{Confirmatory communication evidence over 50 paired trials. Left: the violin and points show the distribution of per-trial attempted-byte reductions; the diamond and bar mark the 58.28\% mean and its 95\% bootstrap interval. Right: each light line joins the full-GM and moment payloads from one trial; the dark line joins their means, giving an absolute saving of 14.63~MB per trial. Exact receiver-output equivalence is reported separately in Sec.~\ref{sec:experiments}.}
