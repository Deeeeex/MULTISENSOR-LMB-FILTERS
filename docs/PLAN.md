...
## Summary
Create `docs/FID_FIA_EXISTENCE_REFINEMENT_PLAN_CN.md` and implement a new hybrid adaptive-weight variant whose goal is: **all three main network disagreement metrics beat the current FID-FIA baseline**, while preserving the current method’s spatial/position strengths as much as possible.
...
- First run a 1-trial smoke test:
  ...
  - inspect OSPA consensus error/matched localization disagreement/Card and local CardErr/RMSE.
...
- If the new hybrid beats FID-FIA on all three network disagreement metrics, promote it as the new headline method.
...
- The target is ambitious: all three network disagreement metrics should beat FID-FIA. The implementation should make this measurable, but the final paper positioning depends on the 20-trial result.
...
The Cardinality-critical mode beats the FID-FIA baseline on all three network disagreement metrics and on local CardErr. Its local RMSE is worse than the Balanced mode but remains slightly better than the FID-FIA baseline.
...
- Do not use `Consensus Card` or `Card` in manuscript tables except as a space-constrained abbreviation with an explicit `Card. disp.` header.
...
- State that their ingredients are standard or literature-supported: OSPA/GOSPA-style finite-set distances, OSPA localization/cardinality decomposition, Hungarian-matched position error, cardinality-consensus/global-target-number agreement ideas, and pairwise consensus disagreement.
...
- Always report the primary disagreement metrics together with local E-OSPA, local RMSE, and local cardinality error to show that consensus gains are not obtained by local tracking collapse.
...
- Cardinality-consensus and global-target-number agreement papers for the target-number agreement motivation.
