...
正式 v2 preflight 虽从干净提交 `57f65d0` 复现了三条数值轨迹，但后续对抗审查
发现它的因果输入证明不成立：只删除了三个顶层真值字段，`model` 内嵌的目标
轨迹仍可由策略回调读取；运行时 context 还暴露了未来链路投递随机数和未来
丢包概率页。“控制器自报未使用”不足以替代“这些字段在结构上不可访问”。
因此 v2 preflight、对应 permit 与 runner 已在产生任何正式 X36 结果前撤销，
旧 MAT 的 `3/3` 资格不得再使用。

下一版必须先把策略回调限制到当前本地后验、当前链路概率、当前位置与过去拓扑
历史，并用机器可检查的 contract 证明嵌套真值、未来链路随机量和未来概率页均
不存在；同时预先冻结参考路线与候选路线两条完整 H=3 指纹。完成新的干净
source-only preflight、独立 permit 和源码绑定前，任何 X36 tracking 仍不允许
启动。
