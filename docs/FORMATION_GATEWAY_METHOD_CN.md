...
## 14. v36：三个额外时间窗的稳定性检查

v35 在 t=72--74 的结果较好，但单个短窗口可能只是偶然。v36 因此没有继续
调参数，而是先在结果不可见的条件下，从同一条 M24、seed 211 仿真轨迹中
冻结三个相隔较远的起点：t=60、104 和 124。每个起点都只允许比较两种方案：
原来的固定参考路线，以及由 v35 根据当时后验和链路状态给出的唯一动态路线。

正式计算前，代码先完成无真值的三步预检，并锁定以下路线：

| 起点 | 三步内暂停的编队 | 行为特点 |
|---:|:--|:--|
| 60 | `[2,4] -> [2,4] -> []` | 保持两步，末步保守回到固定参考路线 |
| 104 | `[1,2,4] -> [1,2] -> [4]` | 第二步恢复编队 4，末步按新后验轮换保护对象 |
| 124 | `[2,3,4] -> [2,3] -> [4]` | 第二步恢复编队 4，末步按新后验轮换保护对象 |

三组路线都通过了标签保留、消息预算、物理连边和三步联合连通检查。随后才
计算 tracking、一致性与通信量，得到：

| 起点 | 平均 tracking | 最差传感器 | 四个编队 | 窗口一致性 | 末步一致性 | 三步通信节省 | 严格通过 |
|---:|---:|---:|:--|---:|---:|---:|:--:|
| 60 | +1.4017% | +11.9980% | `[0, +2.3462%, 0, +2.9353%]` | +6.0074% | +1.3877% | +2.3108% | 否 |
| 104 | +10.3944% | +36.4080% | `[+18.7786%, +11.1852%, 0, +8.9231%]` | +24.3372% | +8.8198% | +0.3468% | 是 |
| 124 | +7.7641% | +31.6245% | `[0, +10.0519%, +15.1681%, +5.5993%]` | +16.4216% | +2.6183% | +0.4051% | 是 |

预先规定的稳定性标准是：至少两组严格通过，三组平均 tracking 收益的中位数
不少于 2%，任一组不得低于 -1%，并且至少两组末步一致性改善。实际结果为
`2/3` 严格通过、中位数 `+7.7641%`、最弱状态 `+1.4017%`、三组末步均改善，
所以 v36 通过。t=60 不是负例：它的所有方向都改善，只是平均 tracking
`+1.4017%` 没达到人为设定的 2% 强结果门槛。

另一个值得保留的现象是，三组动态路线在最后一步都比固定参考路线多发送了
一些数据，但前两步减少得更多，所以三步总通信量仍下降。这说明方法的收益
来自完整的时间安排，不能写成“每一步都更省通信”。

连同先前的 t=72 主结果，目前四个相隔时间窗都出现了正向开发证据。这足以
开始检查 X36 是否也存在相同的源端机制，但还不能称为跨规模泛化。下一步只
允许先做 X36 的无真值场景与动作预检；在该预检冻结并通过前，不计算 X36
tracking，不训练 GNN，也不使用保留验证种子或 X48。

详细审计见
`RUN/GA/dynamic_topology/FORMATION_STAGGERED_RECOVERY_M24_V36_AUDIT_20260802.md`。

## 15. v37：X36 源端轨迹冻结与结果隔离

X36 不直接沿用 M24 的结果，也不先看 tracking 指标再决定比较路线。v37 先在
`x36-formation-fov`、seed 211 的三个阻塞阶段中各冻结一个起点：t=72、100
和 128，分别对应编队对 `[1,2]`、`[3,4]` 和 `[5,6]` 的受限窗口。场景仍
使用 120° 视域和 300 m 距离限制，共 6 个编队、36 个传感器和 24 个目标。

预检只读取每个起点的本地后验、当时链路和过去两步拓扑，不读取目标真值，
也不计算 tracking 或基数误差。为避免“只锁定第一步、后两步悄然变化”，
当前协议把完整三步运行轨迹压成一个可复核指纹。指纹包含每一步的有向图、
融合权重、实际动作、通信量、编队保留与释放、风险值、三步联合连通结果和
历史窗口，共 35 个字段。开发态发现得到的三组指纹为：

| 起点 | 三步实际动作编号 | 完整轨迹 SHA-256 |
|---:|:--|:--|
| 72 | `[53,17,3]` | `fef95ee2...5431f7a` |
| 100 | `[52,28,41]` | `8f71e1c1...7c47b7a4c` |
| 128 | `[44,14,37]` | `c39e67d4...821067` |

这里的开发态运行只用于发现指纹，明确产生 `0/3` 个结果授权。早期 v1
preflight 曾把“源端复现通过”直接写成“允许运行结果”，授权边界过宽，现已
作废并移出正式路径。v2 将两个阶段拆开：干净提交上的正式 preflight 只能
证明三个候选比较对具备资格；它本身仍授权 0 次 tracking 运行。只有后续独立
permit 同时绑定正式 preflight 文件哈希、场景、seed、起点、缓存、参考路线、
候选路线及完整轨迹指纹后，才允许每个状态各运行一次固定的两臂比较。

正式 v2 preflight 已从干净提交 `57f65d0` 完成三组复现，结果为 `3/3`
具备资格、`0` 次 tracking 授权、最大授权数 `0`。正式 MAT 的 SHA-256 为
`de3b17cc...180d64`，三条轨迹仍与上表完全一致。这说明“比较什么、为什么
可比较、如何防止事后换路线”的源端冻结已经闭环，但仍不是 X36 收益结果。
独立 permit 和无参数 exact-two-arm runner 已实现并通过定向单元测试。runner
只能按固定顺序执行三个状态，每个状态恰好比较参考路线与唯一候选路线；正式
结果写入独立的 `x36_outcome/screen`，已有结果不可覆盖。permit、runner、通用
执行器、安全动作库与运行指纹构造器还需要由源码哈希共同锁定，并从干净提交
通过 loader 复核；完成该提交前，任何 X36 tracking 仍不允许启动。
