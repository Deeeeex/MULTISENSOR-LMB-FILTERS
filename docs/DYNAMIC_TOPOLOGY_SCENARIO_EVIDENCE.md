...
11. Generate paired H=3 returns for the 70 frozen privileged targets and
    their state-matched action-24 references. Use those returns to determine
    which truth-labelled graphs are admissible proposal-distillation targets;
    graph difference alone is not a value label. Train a deployable
    truth-free proposal head only if this enlarged oracle reaches the frozen
    7% aggregate gain gate with tail, byte and safety gates intact. Require
    its top-K output to capture at least 80% of states with a privileged
    admissible gain of at least 5% before fitting or evaluating a critic.
