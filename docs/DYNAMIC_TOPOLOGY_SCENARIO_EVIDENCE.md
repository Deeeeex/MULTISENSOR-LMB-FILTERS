...
| C68 | Four linear ridge edge-score heads, selected over three supervision schemes and four regularization values by leave-one-seed-out design diagnostics, recover none of the frozen value graphs: target-state capture is 0/10 and target-graph recall is 0/34. The best mean edge F1 is 0.267 while 3.3 distinct safe graphs are still produced per state, localizing the failure to proposal ranking rather than safety-projection collapse. | High | E83 | This rejects the single-shot linear four-head architecture; it does not reject nonlinear/GNN scoring or a diverse k-best projection bank. The result is design-seen and does not authorize critic training or X36. |
| C69 | Expanding each linear head by banning each of its three selected edges and re-solving the exact safety projection increases the nominal bank from top-4 to top-16, but still recovers 0/10 value-bearing states and 0/34 target graphs. The best mean edge F1 rises only to 0.300 and the bank contains 5.9 distinct safe graphs per state, so one-edge local search around a misranked linear score is insufficient. | High | E84 | This rejects only the linear first-order exclusion bank. It motivates a higher-capacity relational scorer and a larger truth-free-behavior distillation set; critic and X36 remain blocked. |
| C70 | The expanded M24 proposal-distillation artifact contains 54 predecision states from six seeds over t=75:83, all collected under the frozen posterior-analytic action-80 behavior policy. Its behavior graphs, selected histories and 432-by-42 edge features use neither truth nor future outcomes. Offline current-truth teachers attach 210 exact rolling-B=3 safe graphs, with 3–4 targets per state and action-code counts 54/54/51/51 for 00/90/91/92. All 54 behavior graphs, 54 feature states and 210 target graphs are distinct; all behavior actions are repair-free, feasible and B3-valid. | High | E85 | Truth is used in the offline teacher labels, so the artifact is development-only proposal supervision. No H=3 value filtering was run on these 54 states. The preflight establishes integrity and removes the privileged-behavior state-distribution defect of the earlier 18-state data, but it does not establish proposal capture, M24 benefit, critic accuracy, generalization or X36 benefit. |
| C71 | A preregistered shared four-head screen compared raw/contextual ridge models with graph-context MLPs of width 16/32/64, selecting hyperparameters only by six-seed LOSO teacher-graph imitation. The best single-shot model is the width-64 graph-context MLP, but it captures only 2/54 expanded target states. One-edge exclusion expands the nominal bank to top-16 yet still captures 2/54 states and 2/210 graphs, with 0% minimum-seed capture, mean best edge F1 0.198 and only 3.93 distinct projected graphs per state. On the non-selecting legacy value diagnostic it captures 1/10 states and 1/34 value graphs. | High | E86 | This rejects independent per-edge BCE with local one-edge exclusion, including the tested nonlinear shared representation. It does not reject set-level structured learning, permutation-equivariant message passing or broader truth-free-behavior data. The frozen 80%/two-thirds/80% gates all fail, so no H=3 return generation, critic training or X36 experiment is authorized. |
| C72 | Replaying the final width-64 MLP on the same 54 states used to fit it yields 54/54 top-16 state capture, 123/210 target-graph recall and mean best edge F1 1.0, while identical-projector LOSO remains 2/54 and 2/210. The 96.296-percentage-point state-capture gap localizes the failure to seed-specific generalization rather than insufficient network capacity or a broken safety projector. | High | E87 | This is an in-sample-versus-LOSO development diagnosis, not held-out validation. More MLP capacity is not authorized. The next proposal method must use a graph/set-level structured objective and materially broader truth-free-behavior seed diversity; H=3 return generation, critic training and X36 remain blocked. |
| C73 | The frozen structured-proposal M24 v2 artifact extends the immutable six-seed base with clean seed-27/31/37 shards and contains 81 distinct truth-free-behavior states plus 309 distinct current-truth, repair-free, exact rolling-\(B=3\) safe teacher graphs. The fail-closed preflight verifies all four source artifacts, exact action-code counts 81/81/77/70, 81 distinct behavior graphs and feature states, zero behavior/feature truth use, and repair-free/feasible/B3 behavior on every seed. | High | E88 | This is a development-data integrity result. Teacher labels use current truth, and the audit establishes neither LOSO proposal capture nor H=3 return, M24 effect, critic accuracy, validation or X36 benefit. |
| C74 | The preregistered relative-only exchangeable-head set-softmax model fails the nine-seed M24 proposal gate for every width/regularization setting. The selected width-32 model captures 0/81 LOSO states and 0/309 target graphs, captures 0/10 legacy value-bearing states, and reaches mean best edge F1 0.2181. It still emits 4.185 distinct exact-projected graphs per state on average with only 10 projection failures, while its all-seed sampled-bank loss falls from 3.9463 to 0.5701. | High | E89 | This rejects the sampled-bank set-softmax method and blocks H=3 returns, critic training and X36. The result alone does not distinguish training-set projector underfit from seed generalization; the final all-seed model requires an identical-projector training-state audit before selecting the next redesign. |
| C75 | Identical-projector replay of the final structured set model on all 81 states used to fit it also captures 0/81 states and 0/309 target graphs, with mean best edge F1 0.2551 and 4.198 distinct proposals per state. Because the sampled-bank training loss nevertheless falls from 3.9463 to 0.5701, the primary failure is not seed-specific generalization but a mismatch between the sampled softmax bank and the exact global projector used at deployment. | High | E90 | This is a training-state development diagnosis, not validation. It authorizes exact-oracle hard-negative constraint generation as the next proposal redesign, not more capacity, H=3 return generation, critic training or X36. |
| C76 | The frozen exact-oracle v2 response adds 310/323/319 globally selected hard-negative graphs over three rounds, growing each all-seed training bank to 35–41 complete safe graphs, yet still captures 0/81 LOSO states, 0/309 teacher graphs and 0/10 legacy value states. Mean best LOSO edge F1 is 0.2058 and the third-round violation count does not decline, so the fixed-round additive edge scorer has not closed the exact-separation gap. | High | E91 | This rejects the preregistered three-round exact-hard-negative version and keeps H=3 returns, critic training and X36 blocked. The final all-seed model still requires identical-projector training-state replay before attributing the residual failure to representation rather than incomplete constraint convergence. |
| C77 | Identical-projector replay of the exact-oracle final model captures 3/81 training states and 4/309 training target graphs, versus 0/81 LOSO, and raises training mean best edge F1 to 0.5021. Together with 319 newly violated graphs in the third mining round, this shows that exact hard negatives improve local ranking but do not make the additive edge energy a usable complete-graph predictor under the frozen schedule. | High | E92 | Constraint generation is not numerically inert, but neither the training nor LOSO gate passes. Repeating the same fixed-round schedule, generating H=3 returns, training a critic and running X36 remain unauthorized. The next method should stop using exact current-risk graph identity as its primary target and optimize graph-level multi-step return directly. |
| C78 | The preregistered return-first M24 headroom screen compares the exact rolling-\(B=3\) action space with local-only and same-weight fixed-index/fixed-cycle controls. Current-risk and minimax privileged arms pass tail, byte, B3, zero-repair and zero-emergency checks but reach E-OSPA 21.6318/21.4258 versus 20.0187 fixed-index, i.e. -8.06%/-7.03% minimum control-relative mean gains. | High | E93 | This rejects return learning on the replacement action space before critic fitting and blocks X36 policy evaluation. It does not reject graph-value learning on a different action space; the next screen must preserve the strong fixed-index route and place dynamic cross-formation exchange in a low-weight additive residual channel. |
...
| E82 | versioned value-target artifact/preflight/code/test | `RUN/GA/dynamic_topology/datasets/rolling_safe_privileged_value_targets_m24_hard_t75_v2.mat` (SHA-256 `cfda289bb32ecf35161786476878210b76a0ccdb3b4dfb4ee719b576e66bea63`), `RUN/GA/dynamic_topology/privileged_value_target_dataset_preflight_m24.mat` (SHA-256 `93b2336c40660a8d77d81f20607e171e8dab30487ae40fef65e883c57292ec3c`), `RUN/GA/dynamic_topology/datasets/ROLLING_SAFE_PRIVILEGED_VALUE_TARGETS_M24_HARD_T75_V2.md`, `RUN/GA/dynamic_topology/PRIVILEGED_VALUE_TARGET_DATASET_PREFLIGHT_M24.md`, `RUN/GA/generatePrivilegedRollingSafeValueTargetDataset.m`, `RUN/GA/auditPrivilegedRollingSafeValueTargetDataset.m`, `common/getRollingSafeJointActionCriticProtocol.m`, `tests/test_dynamic_topology_scenarios.m`, dataset generation commit `240be48`, audit generation commit `045ad79` | C67: exact state/proposal/return join, zero feature replay error, 34 multi-solution value targets, truth-free feature computation and privileged state-collection boundary. | strong |
| E83 | clean-commit linear proposal model/report/code | `RUN/GA/dynamic_topology/models/rolling_safe_value_proposal_heads_m24_hard_t75_v1.mat` (SHA-256 `430aab248cb438c6ce96d0c6bede96fb27a920a005f9904ce79695ff61ab4a24`), `RUN/GA/dynamic_topology/VALUE_PROPOSAL_HEAD_LOSO_DIAGNOSTIC_M24.md`, `RUN/GA/trainPrivilegedRollingSafeValueProposalHeads.m`, generation commit `f6d6ae2` | C68: 12-setting LOSO screen, exact rolling-B3 projection, zero exact/value capture, partial edge overlap and preserved proposal diversity. | strong |
| E84 | clean-commit linear k-best model/report/code/test | `RUN/GA/dynamic_topology/models/rolling_safe_value_proposal_heads_m24_hard_t75_v2.mat` (SHA-256 `2ec031cdf80f5e1ce417ab6ddc47a99883c099ee7b7c7f80b642db7f0be355f6`), `RUN/GA/dynamic_topology/VALUE_PROPOSAL_HEAD_KBEST_LOSO_DIAGNOSTIC_M24.md`, `RUN/GA/trainPrivilegedRollingSafeValueProposalHeads.m`, `common/replayRollingSafeRolloutProjection.m`, generation commit `aa1c567` | C69: top-16 one-edge exclusion screen, exact allowed-edge projection, zero target capture and slightly improved overlap/diversity. | strong |
| E85 | versioned truth-free-behavior dataset/preflight/code/test | `RUN/GA/dynamic_topology/datasets/rolling_safe_proposal_distillation_m24_hard_t75_t83_v1.mat` (SHA-256 `ce815847e6e540e379934e2f09bbdb4f461cc2b4c7dc4ac918f35ff56feccf06`), `RUN/GA/dynamic_topology/proposal_distillation_dataset_preflight_m24.mat` (SHA-256 `a57ebdfdda8d840e94f9525dd120769630c5124506ad4dc6d3d39a3d79f31ec8`), `RUN/GA/dynamic_topology/datasets/ROLLING_SAFE_PROPOSAL_DISTILLATION_M24_HARD_T75_T83_V1.md`, `RUN/GA/dynamic_topology/PROPOSAL_DISTILLATION_DATASET_PREFLIGHT_M24.md`, `RUN/GA/generateTruthFreeBehaviorProposalDistillationDataset.m`, `RUN/GA/mergeTruthFreeBehaviorProposalDistillationShards.m`, `RUN/GA/auditTruthFreeBehaviorProposalDistillationDataset.m`, `common/getRollingSafeProposalDistillationProtocol.m`, `tests/test_dynamic_topology_scenarios.m`, dataset generation commit `30d5ca3`, audit generation commit `d13365b` | C70: exact 54-state grid, six-shard provenance, truth-free behavior/feature/history boundary, 210 aligned safe current-truth teacher targets, distinct-state/graph coverage and clean B3 execution. | strong |
| E86 | clean-commit shared relational proposal model/report/code/test | `RUN/GA/dynamic_topology/models/rolling_safe_expanded_proposal_heads_m24_t75_t83_v1.mat` (SHA-256 `3633d1eeaee7dbd4b00a538f553521b940ac19afbd8d56daedec2f8eca941a0b`), `RUN/GA/dynamic_topology/EXPANDED_PROPOSAL_HEAD_LOSO_DIAGNOSTIC_M24.md`, `RUN/GA/trainExpandedRollingSafeProposalHeads.m`, `common/getRollingSafeProposalDistillationProtocol.m`, `tests/test_dynamic_topology_scenarios.m`, generation commit `a7c83d1` | C71: preregistered ridge/MLP selection, identical-fold initialization, exact top-16 safety projection, 2/54 expanded and 1/10 legacy-value state capture, low proposal diversity and failed progression gates. | strong |
| E87 | clean-commit generalization-gap audit/code/report | `RUN/GA/dynamic_topology/expanded_proposal_generalization_audit_m24.mat` (SHA-256 `56a7354e5d76a9b2a0a79c75beb8b228b8782de85377a1087388c7e7ff3da4da`), `RUN/GA/dynamic_topology/EXPANDED_PROPOSAL_GENERALIZATION_AUDIT_M24.md`, `RUN/GA/auditExpandedProposalHeadGeneralizationGap.m`, generation commit `5c9442d` | C72: identical top-16 projector replay on final-model training states versus frozen LOSO folds, 54/54 versus 2/54 capture, 96.296-point gap and seed-specific generalization diagnosis. | strong |
| E88 | versioned nine-seed dataset/preflight/code/report | `RUN/GA/dynamic_topology/datasets/rolling_safe_structured_proposal_m24_hard_t75_t83_v2.mat` (SHA-256 `f75cb2eef4665bb1bbe624c92aa29348f1e00acd5ad066337bc108e7c5c89d99`), `RUN/GA/dynamic_topology/structured_proposal_dataset_preflight_m24.mat` (SHA-256 `c81b77d6ff01945065eab0677852cc272972510252fcd19f43b288e855735528`), `RUN/GA/dynamic_topology/datasets/ROLLING_SAFE_STRUCTURED_PROPOSAL_M24_HARD_T75_T83_V2.md`, `RUN/GA/dynamic_topology/STRUCTURED_PROPOSAL_DATASET_PREFLIGHT_M24.md`, `RUN/GA/mergeStructuredProposalDistillationDataset.m`, `RUN/GA/auditStructuredProposalDistillationDataset.m`, `common/getRollingSafeStructuredProposalProtocol.m`, dataset generation commit `dd86712`, audit generation commit `3f23ba5` | C73: exact 81-state nine-seed grid, four-source provenance, 309 aligned safe teacher graphs, truth boundary, uniqueness and behavior safety. | strong |
| E89 | clean-commit structured set model/code/report | `RUN/GA/dynamic_topology/models/rolling_safe_structured_set_proposal_m24_t75_t83_v1.mat` (SHA-256 `9df630eba89c2da790514684793cf92e9455bc694b418791d51940851fdcf89d`), `RUN/GA/dynamic_topology/STRUCTURED_SET_PROPOSAL_LOSO_DIAGNOSTIC_M24.md`, `RUN/GA/trainStructuredSetProposalModel.m`, `common/assignExchangeableProposalHeads.m`, `common/computeExchangeableSetSoftmaxLoss.m`, `common/getRollingSafeStructuredProposalProtocol.m`, training commit `48379aa` | C74: four-setting LOSO failure, exact top-16 replay, legacy-value failure, proposal diversity, projection-failure count and sampled-bank optimization trace. | strong |
| E90 | clean-commit training-state projector audit/code/report | `RUN/GA/dynamic_topology/structured_set_proposal_generalization_audit_m24.mat` (SHA-256 `4b893ee9b2c860669f09764c59e255af215bad596c8767362e4511ca58a87776`), `RUN/GA/dynamic_topology/STRUCTURED_SET_PROPOSAL_GENERALIZATION_AUDIT_M24.md`, `RUN/GA/auditStructuredSetProposalGeneralizationGap.m`, generation commit `8f38c04` | C75: identical exact top-16 replay on final-model training states versus frozen LOSO, zero capture on both, noncollapsed proposal diversity and sampled-objective/projector mismatch diagnosis. | strong |
| E91 | clean-commit exact-oracle model/code/report | `RUN/GA/dynamic_topology/models/rolling_safe_exact_oracle_set_proposal_m24_t75_t83_v2.mat` (SHA-256 `5fa14365f67bf5a17fd974fb31f3e100b57d7abf0d80e94ce7178576ef5eb5b7`), `RUN/GA/dynamic_topology/EXACT_ORACLE_SET_PROPOSAL_LOSO_DIAGNOSTIC_M24.md`, `RUN/GA/trainExactOracleSetProposalModel.m`, `RUN/GA/trainStructuredSetProposalModel.m`, `common/getRollingSafeStructuredProposalProtocol.m`, training commit `acf182c` | C76: three-round exact top-1 hard-negative growth, frozen single-configuration LOSO failure, legacy-value failure and unchanged downstream gates. | strong |
| E92 | clean-commit exact-oracle training-state audit/code/report | `RUN/GA/dynamic_topology/exact_oracle_set_proposal_generalization_audit_m24.mat` (SHA-256 `ac2664777a718272550e881c415daaf9c868aed1f5b0e9ce60a24d4e20de2015`), `RUN/GA/dynamic_topology/EXACT_ORACLE_SET_PROPOSAL_GENERALIZATION_AUDIT_M24.md`, `RUN/GA/auditStructuredSetProposalGeneralizationGap.m`, generation commit `354db50` | C77: identical exact top-16 training-state replay, 3/81 versus 0/81 LOSO, higher local F1, incomplete constraint convergence and explicit rejection of repeating the fixed schedule. | strong |
| E93 | clean-commit return-first action-space screen/audit/code/report | `RUN/GA/dynamic_topology/evidence/return_first_headroom/m24_seed7/DYNAMIC_TOPOLOGY_CONTINUATION_M24_HARD_T75_N1_20260729_180116.mat` (SHA-256 `e2f8eb3fbba47c4741c444e3e57627feea38d1ba524205a8c73a85ec4fc32232`), `RUN/GA/dynamic_topology/evidence/return_first_headroom/m24_seed7/return_first_headroom_audit_m24_hard_seed7.mat` (SHA-256 `182a49548b9d038331d7f3fcc7374a713e87fb1b828887ae0c90913cf55a53d7`), paired source/audit reports, `RUN/GA/runReturnFirstDynamicGraphHeadroom.m`, `RUN/GA/auditReturnFirstDynamicGraphHeadroom.m`, `common/getReturnFirstDynamicGraphProtocol.m`, source generation commit `4cc9083`, audit generation commit `c669a63` | C78: complete six-arm M24 replacement-space headroom failure, simultaneous local/fixed-index/fixed-cycle comparison, exact backbone attribution and fail-closed downstream authorization. | strong |
...
11. Keep v1 rejected and use the frozen v2 contract only for development:
    ...
    The single-shot linear four-head attempt is already rejected at 0/10
    exact/value state capture, and its one-edge top-16 expansion also remains
    at 0/10. The next proposal iteration must add nonlinear relational scoring
    and a larger truth-free-behavior distillation set rather than retuning the
    critic or widening the same local linear neighborhood.
