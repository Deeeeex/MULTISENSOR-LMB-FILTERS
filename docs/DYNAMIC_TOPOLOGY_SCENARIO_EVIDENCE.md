...
11. Keep the privileged first-graph/action-24-continuation v1 result rejected:
    its strict aggregate gain is 5.699%, below the frozen 7% gate. Before
    training a proposal head, freeze a v2 continuation contract that either
    treats later exact safety projection as a legal fallback (while retaining
    first-action exactness, mean/tail/byte/B3/emergency/infeasibility gates)
    or proposes the whole H=3 safe sequence. The observed 8.340% repair-
    monitored result is design evidence only and must not be relabelled as
    the v1 primary result. After v2 passes, require a deployable truth-free
    top-K head to capture at least 80% of states with a privileged admissible
    gain of at least 5% before fitting or evaluating a critic.
