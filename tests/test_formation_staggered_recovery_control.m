...
observableContext = buildObservableTopologyPolicyContext(context);
observableControl = ...
    buildFormationStaggeredRecoveryControl(observableContext);
assert(isequaln(observableControl.selectedAdjacency, ...
    control.selectedAdjacency));
assert(isequaln(observableControl.selectedFusionWeights, ...
    control.selectedFusionWeights));
assert(isequaln(observableControl.selectedFormationIds, ...
    control.selectedFormationIds));
assert(observableContext.observableInputContract.passed);
assert(observableContext.observableInputContract. ...
    modelStaticFusionInputsOnly);

fullSafetyBank = buildTemporalCrossEdgeSuspensionActionBank(context);
subsetSafetyBank = buildTemporalCrossEdgeSuspensionActionBank( ...
    context, struct('posteriorSafetyActionIndices', [1, 2]));
assert(strcmp(subsetSafetyBank.posteriorSafetyScoringMode, ...
    'explicit-index-subset-v1'));
assert(isequal( ...
    subsetSafetyBank.posteriorSafetyRequestedActionIndices, [1, 2]));
assert(isequal(subsetSafetyBank.posteriorSafetyScoredMask, ...
    logical([1, 1, zeros(1, 14)])));
assert(~any(subsetSafetyBank.actionPosteriorSafetyMask(3:end)));
for actionIdx = [1, 2]
    assert(isequal( ...
        fullSafetyBank.actionAdjacency(:, :, actionIdx), ...
        subsetSafetyBank.actionAdjacency(:, :, actionIdx)));
    assert(isequal( ...
        fullSafetyBank.actionFusionWeights(:, :, actionIdx), ...
        subsetSafetyBank.actionFusionWeights(:, :, actionIdx)));
    assert(fullSafetyBank.networkDisagreementRisk(actionIdx) == ...
        subsetSafetyBank.networkDisagreementRisk(actionIdx));
    assert(fullSafetyBank.retentionRisk(actionIdx) == ...
        subsetSafetyBank.retentionRisk(actionIdx));
    assert(isequaln( ...
        fullSafetyBank.networkDisagreementDetails{actionIdx}, ...
        subsetSafetyBank.networkDisagreementDetails{actionIdx}));
    assert(isequaln( ...
        fullSafetyBank.retentionDetails{actionIdx}, ...
        subsetSafetyBank.retentionDetails{actionIdx}));
    assert(fullSafetyBank.actionPosteriorSafetyMask(actionIdx) == ...
        subsetSafetyBank.actionPosteriorSafetyMask(actionIdx));
end
assert(isnan(subsetSafetyBank.networkDisagreementRisk(3)));
assert(isempty(subsetSafetyBank.networkDisagreementDetails{3}));
assertInvalidSafetySubset(context, struct( ...
    'scorePosteriorSafety', false, ...
    'posteriorSafetyActionIndices', [1, 2]));
assertInvalidSafetySubset(context, struct( ...
    'posteriorSafetyActionIndices', 2));
assertInvalidSafetySubset(context, struct( ...
    'posteriorSafetyActionIndices', [1, 1]));
assertInvalidSafetySubset(context, struct( ...
    'posteriorSafetyActionIndices', [2, 1]));
assertInvalidSafetySubset(context, struct( ...
    'posteriorSafetyActionIndices', [1, 17]));
...
mutatedReferenceContext = context;
mutatedReferenceContext.commConfig.pDropByEdge = ...
    0.95 * (ones(24) - eye(24));
for sensorIdx = 1:numel( ...
        mutatedReferenceContext.localPosteriorBySensor)
    mutatedReferenceContext. ...
        localPosteriorBySensor{sensorIdx}.r = ...
            0.01 + 0.98 * mod(sensorIdx, 2);
    mutatedReferenceContext. ...
        localPosteriorBySensor{sensorIdx}.mu = { ...
            sensorIdx * ones(4, 1)};
end
[mutatedReferenceAdjacency, mutatedReferenceDetails] = ...
    selectFormationStaggeredRecoveryRuntimePolicy( ...
        mutatedReferenceContext, 1, initial, [1, 1, 1]);
assert(isequal(mutatedReferenceAdjacency, adjacency));
assert(isequal(mutatedReferenceDetails.fusionWeightMatrix, ...
    details.fusionWeightMatrix));
assert(~mutatedReferenceDetails.posteriorUsed);
assert(~mutatedReferenceDetails.currentLinkReliabilityUsed);
...
function assertInvalidSafetySubset(context, options)
failed = false;
try
    buildTemporalCrossEdgeSuspensionActionBank(context, options);
catch errorInfo
    failed = strcmp(errorInfo.identifier, ...
        'TemporalSuspension:InvalidBankContract');
end
assert(failed);
end
