...
% V270 admits only the first successfully stored packet in a continuous
% formation-label episode while still completing its pending second hop.
config. ...
    topologyPolicySourcePreservingLabelPacketOneShotPerEpisodeEnabled = ...
    true;
[~, oneShotCache, ~, episodeState] = ...
    prepareSourcePreservingLabelPacketRouteV268( ...
        posteriors, physical, delivered, edgeMask, schedule, ...
        model, 1, struct([]), config, struct([]));
[oneShotRoute, oneShotCache, oneShotPage, episodeState] = ...
    prepareSourcePreservingLabelPacketRouteV268( ...
        posteriors, physical, delivered, edgeMask, schedule, ...
        model, 2, oneShotCache, config, episodeState);
assert(oneShotRoute.active && oneShotPage.secondHopDelivered);
assert(oneShotPage.firstHopPolicyActive);
assert(~oneShotPage.firstHopScheduled);
assert(oneShotPage.firstHopSuppressedByEpisode);
assert(isempty(oneShotCache));
[oneShotRoute, ~, oneShotPage] = ...
    prepareSourcePreservingLabelPacketRouteV268( ...
        posteriors, physical, delivered, edgeMask, schedule, ...
        model, 3, oneShotCache, config, episodeState);
assert(~oneShotRoute.active && oneShotPage.firstHopSuppressedByEpisode);
