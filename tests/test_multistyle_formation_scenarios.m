...
for presetIdx = 1:numel(presets)
    ...
    if strcmp(config.sceneStyle, 'orthogonal-crossing')
        ...
        assert(config.formalValidationAuthorized);
        assert(config.enforceDifficultyRequirements);
        assert(isequal(config.difficultyRequirements, ...
...
strippedProtectedTrace = inputs.model;
strippedProtectedTrace = rmfield(strippedProtectedTrace, ...
    'dynamicTopologyTrackingOutcomeGate');
protectedFields = intersect(fieldnames( ...
    strippedProtectedTrace.dynamicTopologyScenario.config), { ...
        ...
        'scaleControlReferencePreset'}, 'stable');
strippedProtectedTrace.dynamicTopologyScenario.config = rmfield( ...
    strippedProtectedTrace.dynamicTopologyScenario.config, ...
    protectedFields);
assertTrackingGateError(@() ...
    assertDynamicTopologyTrackingOutcomeAuthorized( ...
        strippedProtectedTrace));
...
strippedMetadata = rmfield(inputs.model, { ...
    'dynamicTopologyScenario', ...
    'dynamicTopologyTrackingOutcomeGate'});
assertErrorIdentifier(@() ...
    assertDynamicTopologyTrackingOutcomeAuthorized(strippedMetadata), ...
    'DynamicTopology:IncompleteScenarioMetadata');

runtimeOnlyTrace = rmfield(strippedMetadata, { ...
    'explicitSensorTrajectories', ...
    'explicitTargetTrajectories', ...
    'sensorTrajectories'});
assertErrorIdentifier(@() runEventTriggeredDistributedLmbFilter( ...
    runtimeOnlyTrace, inputs.measurements, inputs.sensorTrajectories, ...
    inputs.neighborMap, inputs.commConfig, struct()), ...
    'DynamicTopology:IncompleteScenarioMetadata');
end
...
function assertErrorIdentifier(callback, expectedIdentifier)
failed = false;
try
    callback();
catch errorInfo
