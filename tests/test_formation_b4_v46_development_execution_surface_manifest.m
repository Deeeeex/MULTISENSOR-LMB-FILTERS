...
assert(~isempty(strfind(launcherText, '--no-init-path'))); %#ok<STREMP>
...
assertMissingNoInitPathInvocationRejected(launcherText, resolution);
assertOctavePathInvocationRejected(launcherText, resolution);
...
command = sprintf([ ...
    ...
    ['/opt/homebrew/bin/octave --quiet --no-gui --no-init-path ', ...
     '--eval %s 2>&1']], ...
...
function assertMissingNoInitPathInvocationRejected(launcherText, resolution)
token = regexp(launcherText, '--eval "(.*)"', 'tokens', 'once');
assert(~isempty(token));
repoRoot = pwd;
directories = cell(size(resolution.relativePaths));
...
command = sprintf([ ...
    ...
    'FORMATION_B4_V46_ARM_ID=v46-repaired-reference-a70-e05 ', ...
    'FORMATION_B4_V46_OUTPUT_PATH='''' ', ...
    ['/opt/homebrew/bin/octave --quiet --no-gui --no-init-all ', ...
     '--eval %s 2>&1']], ...
    shellQuote(launcherPath), shellQuote(repoRoot), ...
    shellQuote(trackedPath), shellQuote(token{1}));
[status, output] = builtin('system', command);
...
function assertOctavePathInvocationRejected(launcherText, resolution)
token = regexp(launcherText, '--eval "(.*)"', 'tokens', 'once');
assert(~isempty(token));
repoRoot = pwd;
directories = cell(size(resolution.relativePaths));
...
launcherPath = fullfile(repoRoot, 'RUN', 'GA', ...
    'launchFormationB4V46DevelopmentArmShard.sh');
extraPathDirectory = tempname();
mkdir(extraPathDirectory);
cleanup = onCleanup(@() cleanupEmptyDirectory(extraPathDirectory));
command = sprintf([ ...
    '/usr/bin/env -i HOME=/Users/dex TMPDIR=/tmp ', ...
    'PATH=/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin ', ...
    'GIT_PAGER=cat OCTAVE_PATH=%s ', ...
    'FORMATION_B4_V46_INVOCATION_CONTRACT=', ...
        'formation-b4-v46-development-invocation-v1 ', ...
    'FORMATION_B4_V46_LAUNCHER_PATH=%s ', ...
    'FORMATION_B4_V46_INIT_FILES_DISABLED=true ', ...
    ...
    'FORMATION_B4_V46_OUTPUT_PATH='''' ', ...
    ['/opt/homebrew/bin/octave --quiet --no-gui --no-init-all ', ...
     '--no-init-path --eval %s 2>&1']], ...
    shellQuote(extraPathDirectory), shellQuote(launcherPath), ...
    shellQuote(repoRoot), ...
    shellQuote(trackedPath), shellQuote(token{1}));
[status, output] = builtin('system', command);
assert(status ~= 0);
