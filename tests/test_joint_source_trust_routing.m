...
bank = buildFormationModeInterventionBank( ...
    projection, groupIds);
assert(bank.actionCount == 7);
assert(isequal(bank.actionModes, [ ...
    1, 1; 2, 1; 3, 1; 4, 1; ...
    1, 2; 1, 3; 1, 4]));
assert(isequal(bank.actionAdjacency(:, :, 1), ...
    logical(projection.modeAdjacencyByIndex{1})));
assert(max(abs(reshape( ...
    bank.actionFusionWeights(:, :, 1) - ...
        projection.modeFusionWeightsByIndex{1}, [], 1))) < 1e-12);
