...
testPosteriorRiskTailSafeIntegration();
testX36SelectorIntegration();
fprintf('test_joint_source_trust_routing passed\n');
end

function testPosteriorRiskTailSafeIntegration()
groupIds = repelem(1:2, 6);
...
[adjacency, details] = ...
    selectLabelSetAdaptiveDominantRoutingPolicy( ...
        context, ['composite-balanced-certified-overlap-', ...
            'consensus-joint-trust-posterior-risk-', ...
            'tail-safe-payload-aware'], struct( ...
...
assert(nnz(adjacency) >= 14 && nnz(adjacency) <= 20);
assert(strcmp(details.contractVersion, ...
    'adaptive-dominant-posterior-risk-formation-tail-v10-prototype'));
projection = details.consensusProjection;
assert(strcmp(projection.objectiveMode, 'posterior-bayes-risk'));
assert(isequal(size(projection.objectiveByFormationMode), [2, 4]));
assert(all(abs(projection.objectiveByFormationMode(:, 1)) < 1e-12));
assert(projection.formationRiskGuardEnabled);
assert(projection.projector.formationRiskConstraintPassed);
assert(all(projection.selectedFormationRisk <= ...
    projection.formationRiskLimit + 1e-12));
assert(strcmp(projection.projector.contractVersion, ...
    'joint-source-trust-formation-tail-projector-v2'));
assert(~projection.posteriorRiskObjectiveDetails.truthUsed);
