function [objects,stats]=fuseAsymmetricEvidence(inputs,weights,model,details,cfg,arm,t)
% Conservative inherited posterior with selectively admitted current evidence.
assert(ismember(arm,{'asymmetric','asymmetric_no_history','asymmetric_no_mark'}));
assert(numel(unique(details.sourceIndices))==numel(details.sourceIndices));
stats=struct('weightChange',0,'lineageExcluded',0,'observableAbsences',0,'records',zeros(0,37));
...
for k=1:size(keys,2)
    ...
    beta=b;
    if ~strcmp(arm,'asymmetric_no_history') && age < -1e-12,beta=q;end
    ...
    if isfield(cfg,'captureIterationRecords') && cfg.captureIterationRecords
        ...
        stats.records(end+1,:)=[t,details.sourceIndices(1),keys(:,k)',one.mu{1}(1:2)', ...
            ...
            gates,beta,boost,original(:)',negativeGates]; %#ok<AGROW>
    end
