function runAsymmetricReplay(firstIndex,lastIndex,mode)
...
checkStableMarkedEvidence();checkAsymmetricEvidence();
...
for sequenceIndex=firstIndex:lastIndex
    ...
    for condition={'reliable','intermittent'}
        ...
        runs=cell(size(arms));
        for a=1:numel(arms)
            arm=arms{a};fprintf('START ASYMMETRIC %04d %s %s %d frames\n',seq,radio,arm,T);
            ...
            fprintf('DONE ASYMMETRIC %04d %s %s OSPA %.6f runtime %.2fs\n',seq,radio,arm,mean(runs{a}.ospa,'all'),run.runtimeSeconds);
        result=struct('protocol','asymmetric-current-evidence-v1','implementation','asymmetric-v1', ...
...
fprintf('COMPLETED ASYMMETRIC indices %d--%d cohort=%s\n',firstIndex,lastIndex,mode);
end
...
template.lastDirectOpportunity=0;template.directEvidenceCeiling=0;template.positiveConfirmation=false;template.localLogOddsIncrement=0;template.positiveInnovationSupport=0;template.negativeInnovationSupport=0;
...
run=struct('arm',arm,'estimates',{cell(2,T)},'rawEstimates',{cell(2,T)},'labels',{cell(2,T)}, ...
    ...
    'packetBytes',zeros(2,T),'localIncrementRecords',zeros(0,12),'iterationRecords',zeros(0,37),'maximumBernoulliCount',0,'runtimeSeconds',0,'totalWireBytes',0);
end
