"""Execute a registered asymmetric stage in three independent MATLAB processes."""
...
def main():
    ...
    def worker(unit):
        ...
        command = ['/Applications/MATLAB_R2024a.app/bin/matlab', '-singleCompThread', '-batch',
                   f"addpath('trials/icra_asymmetric_evidence'); runAsymmetricReplay({index},{index},'{cohort}');"]
        print('ASYMMETRIC START', cohort, f'{seq:04d}', flush=True)
        ...
        with (LOG / f'{cohort}_{seq:04d}.log').open('w') as log:
            for line in process.stdout:
                log.write(line)
                log.flush()
                if f'COMPLETED ASYMMETRIC indices {index}--{index} cohort={cohort}' in line:
                    completion = True
        code = process.wait()
        ...
        print('ASYMMETRIC END', cohort, f'{seq:04d}', 'return', code, 'complete', completion, 'files', entry['files'], flush=True)
        return entry
    ...
    print('ALL ASYMMETRIC STAGE RUNS COMPLETE:', args.cohort, len(units), 'sequences;', 2 * len(arms) * len(units), 'files.', flush=True)
