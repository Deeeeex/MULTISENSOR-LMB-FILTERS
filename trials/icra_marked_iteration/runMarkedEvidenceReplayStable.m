function runMarkedEvidenceReplayStable(firstSequence,lastSequence,smoke,arms)
...
if nargin<4,arms={'marked_lineage','marked_er','marked_ceiling_association','marked_ceiling_score','marked_ceiling_calibrated'};end
...
priorCeiling=fullfile(root,'trials','icra_ceiling_iteration');addpath(priorCeiling);
checkStableMarkedEvidence();
...
resultDir=fullfile(out,'results_stable');if ~isfolder(resultDir),mkdir(resultDir);end
manifest=jsondecode(fileread(fullfile(original,'v2v4real_input_manifest.json')));
sourceManifest=jsondecode(fileread(fullfile(out,'source_sha256_stable.json')));
for seq=firstSequence:lastSequence
    ...
    ratios=load(fullfile(out,'data_likelihoods',sprintf('likelihoods_%04d.mat',seq)));
    ...
    for condition={'reliable','intermittent'}
        ...
        for a=1:numel(arms)
            ...
            runs{a}=runDensity(model,measurements,positions,delivered,arms{a},marks,ratios);
        ...
        result=struct('protocol','marked-evidence-ceiling-v1','implementation','mecr-v1-stable-adapter', ...
...
for t=1:T
    ...
    for n=1:2
        ...
        else,[local{n},~,W]=updateMarkedLmbStable(predicted,num2cell(measurements{n,t},1),model,n,t,ratios.likelihoodRatios{n,t});end
        mark=ones(1,size(measurements{n,t},2));
        if strcmp(arm,'marked_ceiling_score'),mark=marks.rawScores{n,t};end
        if strcmp(arm,'marked_ceiling_calibrated'),mark=marks.calibratedScores{n,t};end
    ...
    if strcmp(arm,'local')
        ...
        for n=1:2,[decoded{n},packet]=ceilingLmbPacket(local{n},model,n,t);sizes(n)=numel(packet);end
        for n=1:2
            other=3-n;
            ...
            [posterior{n},stats]=fuseMarkedInputsStable(inputs,[.5,.5],model,details,cfg,arm,t);
