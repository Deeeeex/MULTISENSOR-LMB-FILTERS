"""Independent marked-local-update control and fused-existence audit."""
...
CANDIDATES=['marked_lineage','marked_er','marked_ceiling_association','marked_ceiling_score','marked_ceiling_calibrated']
...
def main():
    ...
    parser.add_argument('--output-dir',type=Path);parser.add_argument('--stable',action='store_true');args=parser.parse_args()
    destination=args.output_dir.resolve() if args.output_dir else OUT
    source=json.loads((OUT/('source_sha256_stable.json' if args.stable else 'source_sha256.json')).read_text())
    ...
    unmarked=json.loads((OUT.parent/'icra_ceiling_iteration/summary_development.json').read_text())
    for r in unmarked['runs']:
        if r['arm']!='qualified_exist':baseline[r['sequence'],r['condition'],r['arm']]=r
    calibration=json.loads((OUT.parent/'icra_ceiling_iteration/calibration.json').read_text())
    mark_manifest=json.loads((OUT/'likelihood_manifest.json').read_text())
    assert hashlib.sha256((OUT.parent/'icra_ceiling_iteration/calibration.json').read_bytes()).hexdigest()==mark_manifest['calibration_sha256']
    for entry in mark_manifest['inputs']:
        name=entry['sequence'];pi=entry['positive_prior']
        assert name not in entry['training_sequences'] and len(entry['training_sequences'])==8
        probability=loadmat(OUT.parent/'icra_ceiling_iteration/data_marks'/f'marks_{name}.mat')['calibratedScores']
        path=OUT/'data_likelihoods'/f'likelihoods_{name}.mat'
        assert hashlib.sha256(path.read_bytes()).hexdigest()==entry['likelihoods_sha256']
        actual=loadmat(path)['likelihoodRatios']
        for index in np.ndindex(probability.shape):
            p=np.clip(probability[index],1e-6,1-1e-6);expected=p/(1-p)*(1-pi)/pi
            assert np.array_equal(actual[index],expected)
    rows=[];diagnostics=[];inputs=[];common=[];audited=0;parity=0;retry_parity=0
    for seq in range(args.last+1):
        name=f'{seq:04d}'
        for condition in ['reliable','intermittent']:
            path=OUT/('results_stable' if args.stable else 'results_development')/f'{name}_{condition}.json.gz';data=read(path)
            ...
            assert data['implementation']==('mecr-v1-stable-adapter' if args.stable else 'mecr-v1') and not data['smoke'] and data['sourceSha256']==matlab
            for key in ['truth','truthIds','positions','time','delivered','inputSha256']:assert data[key]==old[key],key
            assert [r['arm'] for r in data['runs']]==CANDIDATES
            first=OUT/'diagnostics_before_process_retry'/path.name
            if args.stable and first.exists():
                saved=read(first)
                for a,b in zip(data['runs'],saved['runs']):
                    for key in a:
                        if key!='runtimeSeconds':assert a[key]==b[key],(name,condition,a['arm'],key,'runtime refactor output parity')
                    retry_parity+=2*T
            matches={}
            for run in old['runs']:
            ...
            for run in data['runs']:
                ...
                if arm=='marked_lineage':expected=r0
                elif arm=='marked_er':expected=rER
                else:expected=np.minimum(rER,np.maximum(r0,cap))
                ...
                if arm!='marked_lineage':
                    ...
                    assert np.allclose(records[age<0,6],rER[age<0],atol=2e-12)
            ...
            for candidate in CANDIDATES:
                for reference in ['marked_lineage','marked_er','qualified_exist','lineage','mil_support','tc_ospa2_w5','tc_ospa2_w10']:
            ...
            print('MARKED AUDITED',name,condition,audited,'node-frames',flush=True)
    ...
    arms=CANDIDATES+['ceiling_association','ceiling_score','ceiling_calibrated','qualified_exist','lineage','conservative_recency','confirmation_recency','mil_support','tc_ospa2_w5','tc_ospa2_w10','local']
    for condition in ['reliable','intermittent']:
        for arm in arms:
    ...
    result=dict(protocol='marked-evidence-ceiling-v1',scope='Development only; all selected sequences, no outcome exclusion; all five arms share the same sequence-held-out marked local filter.',
                sequences=n,audited_node_frames=audited,implementation=data['implementation'],
                first_attempt_exact_output_parity_node_frames=retry_parity,
                fixed_calibration_likelihood_cells_verified=True,
                ...
                packet_note='All marked development arms transmit one support scalar per object for instrumentation; No-age and ER do not require it. Historical unmarked controls use their originally audited bytes. No native baseline communication superiority is claimed.')
    ...
    print('MARKED DEVELOPMENT AUDIT PASSED',audited,'node-frames; exact unity-mark adapter parity is a separate analytic preflight.')
