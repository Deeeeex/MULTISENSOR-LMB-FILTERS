function runPruneInformationReplay(configPath,unitIndex)
...
addpath(fullfile(root,'trials','icra_temporal_association'));
addpath(fullfile(root,'trials','icra_range_detection'));addpath(fullfile(root,'trials','icra_known_censor'));addpath(out);
...
qualityPath=fullfile(root,'trials','icra_range_detection','range_quality');addpath(qualityPath);
...
checkPersistentAssociation();checkColumnAssociation();checkScreenedAssociation();
...
assert(strcmp(which('Hungarian'),fullfile(runtimePath,'Hungarian.m')));
checkRangeDetection(baseQuality);checkKnownCensor();checkPruneInformation();
...
for condition=reshape(unit.conditions,1,[])
    ...
    for a=1:numel(unit.arms)
        ...
        result=struct('protocol','icra-prune-information-v1','stage',config.stage, ...
...
template.lastDirectOpportunity=0;template.directEvidenceCeiling=0;template.positiveConfirmation=false;template.localLogOddsIncrement=0;template.positiveInnovationSupport=0;template.negativeInnovationSupport=0;template.localSpatialLogRatio=zeros(1,15);template.localDirectObservation=zeros(1,8);
...
run=struct('arm',arm,'estimates',{cell(2,T)},'rawEstimates',{cell(2,T)},'labels',{cell(2,T)}, ...
    ...
    'packetGaussianRecords',zeros(0,19),'localAssociationWeights',{cell(2,T)},'localDirectRecords',zeros(0,12),'packetDirectRecords',zeros(0,12),'associationRecords',zeros(0,10),'associationStats',zeros(0,11),'associationAbstentions',zeros(0,4),'associationReopenings',zeros(0,8),'associationRemoteAbstentions',zeros(0,4),'associationSplits',zeros(0,6),'associationConflicts',zeros(0,4),'iterationRecords',zeros(0,60),'maximumBernoulliCount',0,'runtimeSeconds',0,'totalWireBytes',0);
end
...
pruneInformationEnabled=endsWith(rule,'_prune_info');
...
associationMode='';
if endsWith(rule,'_assoc_quality_nis'),associationMode='quality_nis';rule=erase(rule,'_assoc_quality_nis');
elseif endsWith(rule,'_assoc_quality'),associationMode='quality';rule=erase(rule,'_assoc_quality');
elseif endsWith(rule,'_assoc_nis'),associationMode='nis';rule=erase(rule,'_assoc_nis');
elseif endsWith(rule,'_assoc_direct'),associationMode='direct';rule=erase(rule,'_assoc_direct');
elseif endsWith(rule,'_assoc_temporal'),associationMode='temporal';rule=erase(rule,'_assoc_temporal');
elseif endsWith(rule,'_assoc_reopen'),associationMode='reopen';rule=erase(rule,'_assoc_reopen');
...
run=emptyRun(arm,T);run.rangeDetectionMode=model.rangeDetectionMode;
...
timer=tic;
for t=1:T
    local=cell(1,2);knownLocal=cell(1,2);
    for n=1:2
        ...
        for j=1:numel(predicted)
            o=predicted(j);[pd,~,quality]=evaluateSensorQuality(model,n,o.mu{1},t);predictedPd(j)=pd;
        ...
        knownLocal{n}=zeros(numel(local{n}),6);
        for j=1:numel(local{n})
    ...
    if strcmp(rule,'local')
        ...
        decoded=cell(1,2);sizes=zeros(1,2);sentPackets=cell(1,2);baseSizes=zeros(1,2);
        for n=1:2
        ...
        for n=1:2
            ...
            if isempty(associationMode)
                ...
                if ismember(associationMode,{'quality','nis','quality_nis'})
                    [minimumQuality,cutoffs]=associationScreenSettings(associationMode);
                    [remote,matching,associationMemory{n}]=alignScreenedLmbPair(local{n},decoded{other},50,associationMemory{n},'split',t,n,minimumQuality,cutoffs);
                elseif ismember(associationMode,{'direct','temporal'})
                    ...
                    [remote,matching,associationMemory{n}]=alignPersistentColumn(local{n},decoded{other},50,associationMemory{n},associationMode,t,n);
                end
            end
