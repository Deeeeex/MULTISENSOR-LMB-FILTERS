...
def main():
    ...
    if args.stage=='prune_info_shared':
        audit=json.loads((OUT/'audit_prune_info_controls.json').read_text());assert audit['passed'] and len(audit['parity'])==6
        for name,h in audit['inputs'].items():assert sha(ROOT/name)==h,name
    logdir=ROOT/'RUN/ICRA_PRUNE_INFORMATION'/args.stage;ledger=OUT/('runtime_'+args.stage+'.json')
    ...
    def worker(index,u):
        ...
        command=['/Applications/MATLAB_R2024a.app/bin/matlab','-singleCompThread','-batch',
            f"addpath('trials/icra_prune_information');runPruneInformationReplay('{cfgpath.relative_to(ROOT)}',{index+1});"]
    ...
    assert len(completed)==2;print('ALL PRUNE INFORMATION STAGE RUNS COMPLETE',args.stage,flush=True)
