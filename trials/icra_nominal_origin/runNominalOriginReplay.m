function runNominalOriginReplay(configPath,unitIndex)
...
assert(strcmp(which('Hungarian'),fullfile(runtimePath,'Hungarian.m')));
checkRangeDetection(baseQuality);checkInitialNegative();
...
for condition=reshape(unit.conditions,1,[])
    ...
    for a=1:numel(unit.arms)
        ...
        result=struct('protocol','icra-nominal-origin-intervention-v1','stage',config.stage, ...
...
initialNegativeEnabled=endsWith(rule,'_once_initial_negative');
