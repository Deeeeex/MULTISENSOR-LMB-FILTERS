...
%*************************************************
  ...
  while exit_flag
    diagnosticIteration=diagnosticIteration+1;
    if diagnosticIteration>10000
      global ICRA_MIL_DIAG_CONTEXT;
      snapshot=fullfile(getenv('ICRA_MIL_DIAG_DIR'),'hungarian_stop.mat');
      save(snapshot,'Perf','P_cond','M','r_cov','c_cov','stepnum','diagnosticIteration','ICRA_MIL_DIAG_CONTEXT');
      error('Diagnostic:AssignmentIterations','Diagnostic-only iteration stop; saved original matrix, no official result.');
    end
    switch stepnum
      case 1
...
while zflag  
    % Find the first uncovered zero
      % Transpose preserves the original row-major first-zero tie break.
