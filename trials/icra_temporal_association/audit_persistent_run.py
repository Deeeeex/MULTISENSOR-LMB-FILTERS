...
from persistent_math import objective, audit_branches
...
def associations(run, data, frames, mode):
    ...
    for t in range(1, len(data['time']) + 1):
        for n in [1, 2]:
            ...
            info, states[n] = objective(left, right, states[n], t)
            ...
            branch = audit_branches(info, pairs, left['keys'], right['keys'], mode, n)
