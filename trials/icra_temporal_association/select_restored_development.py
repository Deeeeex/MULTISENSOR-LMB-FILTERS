...
def main():
    ...
    for stage, expected in [('association_restored_preflight', 20), ('association_restored_development_rest', 56)]:
    ...
    result = dict(passed=True, created_utc=datetime.now(timezone.utc).isoformat(),
        protocol_sha256=sha(OUT / 'PATH_RESTORATION.md'), selection_rule='Among candidates passing both OSPA conditions and the pooled 2 m wrong-pair gate, choose the lowest two-condition sequence mean; exact ties follow D, T, R, S order.',
    ...
    with (OUT / 'restored_development_comparison.csv').open('w') as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0]), lineterminator='\n')
        writer.writeheader()
        writer.writerows(rows)
    print('RESTORED DEVELOPMENT SELECTION', json.dumps(selected, allow_nan=False), flush=True)
