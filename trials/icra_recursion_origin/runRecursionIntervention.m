function runRecursionIntervention(configPath,unitIndex)
...
assert(strcmp(which('Hungarian'),fullfile(runtimePath,'Hungarian.m')));
checkRangeDetection(baseQuality);checkRecursionIntervention();
...
for condition=reshape(config.conditions,1,[])
    ...
    for a=1:numel(runArms)
        ...
        result=struct('protocol','icra-recursion-intervention-v1','stage',config.stage, ...
