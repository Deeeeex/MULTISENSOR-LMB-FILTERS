"""Execute a registered marked joint stage in three independent MATLAB processes."""
...
ROOT = OUT.parents[1]
LOG = ROOT / 'RUN/ICRA_MARKED_JOINT'
...
def main():
    ...
    def worker(unit):
        ...
        command = ['/Applications/MATLAB_R2024a.app/bin/matlab', '-singleCompThread', '-batch',
                   f"addpath('trials/icra_marked_joint'); runMarkedJointReplay({index},{index},'{cohort}');"]
        print('MARKED JOINT START', cohort, f'{seq:04d}', flush=True)
        ...
        with (LOG / f'{cohort}_{seq:04d}.log').open('w') as log:
            for line in process.stdout:
                log.write(line)
                log.flush()
                if f'COMPLETED MARKED JOINT indices {index}--{index} cohort={cohort}' in line:
                    completion = True
        code = process.wait()
        paths = [OUT / f'results_{cohort}' / f'{seq:04d}_{condition}_{arm}.json.gz'
                 for condition in frozen['conditions'] for arm in frozen['arms']]
        ...
        print('MARKED JOINT END', cohort, f'{seq:04d}', 'return', code, 'complete', completion, 'files', entry['files'], flush=True)
        return entry

    with ThreadPoolExecutor(max_workers=3) as executor:
        for future in as_completed([executor.submit(worker, unit) for unit in units]):
            future.result()
    assert len(completed) == len(units) and all(r['returncode'] == 0 and r['completion_line'] and r['files'] == 4 for r in completed)
    print('ALL MARKED JOINT STAGE RUNS COMPLETE:', args.cohort, len(units), 'sequences;', 4 * len(units), 'files.', flush=True)
