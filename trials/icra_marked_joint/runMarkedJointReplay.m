function runMarkedJointReplay(firstIndex,lastIndex,mode)
...
for sequenceIndex=firstIndex:lastIndex
    ...
    for condition={'reliable','intermittent'}
        ...
        runs=cell(size(arms));
        for a=1:numel(arms)
            arm=arms{a};fprintf('START MARKED JOINT %04d %s %s %d frames\n',seq,radio,arm,T);
            ...
            fprintf('DONE MARKED JOINT %04d %s %s OSPA %.6f runtime %.2fs\n',seq,radio,arm,mean(runs{a}.ospa,'all'),run.runtimeSeconds);
        result=struct('protocol','marked-current-innovation-v1','implementation','marked-joint-v1', ...
...
fprintf('COMPLETED MARKED JOINT indices %d--%d cohort=%s\n',firstIndex,lastIndex,mode);
end
...
run=struct('arm',arm,'estimates',{cell(2,T)},'rawEstimates',{cell(2,T)},'labels',{cell(2,T)}, ...
    ...
    'packetBytes',zeros(2,T),'localIncrementRecords',zeros(0,8),'iterationRecords',zeros(0,26),'maximumBernoulliCount',0,'runtimeSeconds',0,'totalWireBytes',0);
end
