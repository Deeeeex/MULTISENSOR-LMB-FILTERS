"""Audit the two registered cache-only fusion ablations against v1 controls."""
...
EXTENSION_ARMS = ['lineage_recent', 'qualified_exist']
...
def main():
    ...
    for scene in SCENES:
        ...
        for seed in SEEDS:
            ...
            with gzip.open(OUT/'results'/f'{scene}_seed{seed}_extension.json.gz','rt') as stream:
    ...
    hashes=json.loads((OUT/'extension_source_sha256.json').read_text())
    ...
    (OUT/'summary_extension.json').write_text(json.dumps(result,indent=2,allow_nan=False)+'\n')
