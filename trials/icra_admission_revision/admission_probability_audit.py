...
ARMS = ['marked_gaussian_evidence_decoupled_'+mode for mode in ['all','positive','negative','consensus']] + ['marked_gaussian_evidence_fixedx_'+code for code in ['000','050','100','125']]
...
from admission_gaussian_audit import check_gaussians
...
def audit_probability(run, data):
    arm = run['arm']
    fixed = '_fixedx_' in arm
    strength = int(arm.rsplit('_', 1)[1])/1000 if fixed else None
