...
from admission_probability_audit import audit_probability as audit_new_probability
...
def main():
    ...
    result=dict(passed=True,stage=args.stage,cohort=cfg['cohort'],sequences=len(cfg['units']),
                ...
                auditor_sha256={str(p.relative_to(ROOT)):sha(p) for p in [Path(__file__),OUT/'admission_probability_audit.py',OUT/'admission_gaussian_audit.py',OUT.parent/'icra_reviewer_revision/review_probability_audit.py',OUT.parent/'icra_reviewer_revision/review_gaussian_audit.py']})
