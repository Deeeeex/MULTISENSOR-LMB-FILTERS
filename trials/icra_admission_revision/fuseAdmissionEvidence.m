function [objects,stats]=fuseAdmissionEvidence(inputs,weights,model,details,cfg,arm,t)
% Conservative inherited posterior with selectively admitted current evidence.
assert(ismember(arm,{'gaussian_evidence_decoupled_all','gaussian_evidence_decoupled_positive', ...
    'gaussian_evidence_decoupled_negative','gaussian_evidence_decoupled_consensus', ...
    'gaussian_evidence_fixedx_000','gaussian_evidence_fixedx_050', ...
    'gaussian_evidence_fixedx_100','gaussian_evidence_fixedx_125'}));
fixed=startsWith(arm,'gaussian_evidence_fixedx_');lambda=0;
if fixed,lambda=str2double(extractAfter(arm,'gaussian_evidence_fixedx_'))/1000;end
...
for k=1:size(keys,2)
    ...
    existenceKappa=kept;
    if strcmp(arm,'gaussian_evidence_decoupled_all')
        existenceKappa=rawKappa;
    elseif strcmp(arm,'gaussian_evidence_decoupled_positive')
        existenceKappa=kept+(rawKappa-kept).*(increments>0);
    elseif strcmp(arm,'gaussian_evidence_decoupled_negative')
        existenceKappa=kept+(rawKappa-kept).*(increments<0);
    elseif strcmp(arm,'gaussian_evidence_decoupled_consensus')
        % Restore a rejected negative scalar only with current negative
        % evidence from every participating source. No truth enters here.
        agreed=sum(active)>=2 && all(present(active)) && ...
            all(stamps(active)==t) && all(increments(active)<0);
        if agreed,existenceKappa=rawKappa;end
    end
    one.r=logistic(sum(beta.*logits)+sum(existenceKappa.*increments)+logIntegral);
