function runCompatibleReplay(configPath,unitIndex)
...
addpath(fullfile(root,'trials','icra_reviewer_revision'),fullfile(root,'trials','icra_admission_revision'),fullfile(root,'trials','icra_projected_admission'));
addpath(out);
checkStableMarkedEvidence();checkGaussianEvidence();checkReviewerEvidence();checkAdmissionEvidence();checkProjectedEvidence();checkCompatibleEvidence();
...
for t=1:T
    ...
    if strcmp(rule,'local')
        ...
        for n=1:2
            ...
            elseif startsWith(rule,'gaussian_evidence_compatible_')
                [posterior{n},stats]=fuseCompatibleEvidence(inputs,[.5,.5],model,details,cfg,rule,t);
            elseif startsWith(rule,'gaussian_evidence_projected_')
                [posterior{n},stats]=fuseProjectedEvidence(inputs,[.5,.5],model,details,cfg,rule,t);
            elseif startsWith(rule,'gaussian_evidence_decoupled_') || startsWith(rule,'gaussian_evidence_fixedx_')
                [posterior{n},stats]=fuseAdmissionEvidence(inputs,[.5,.5],model,details,cfg,rule,t);
