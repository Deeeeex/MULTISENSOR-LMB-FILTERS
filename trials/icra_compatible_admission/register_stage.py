...
def main():
    ...
    paths += [OUT / 'PROTOCOL.md', OUT / 'COMPATIBILITY_DIAGNOSTIC.json', OUT.parent / 'icra_admission_revision/SOURCE_REVIEW_CN.md', OUT.parent / 'icra_admission_revision/REJECTION_DIAGNOSTIC.json', previous]
    ...
    paths += list((OUT.parent / 'icra_admission_revision').glob('*.m'))
    paths += [OUT.parent / 'icra_admission_revision' / n for n in ['admission_probability_audit.py','admission_gaussian_audit.py','PROTOCOL.md']]
    paths += list((OUT.parent / 'icra_projected_admission').glob('*.m'))
    paths += [OUT.parent / 'icra_projected_admission' / n for n in ['projected_probability_audit_v2.py','projected_gaussian_audit.py','projected_ratio.py']]
    ...
    config = dict(protocol='icra-compatible-admission-v1', stage=args.stage, cohort=args.cohort,
