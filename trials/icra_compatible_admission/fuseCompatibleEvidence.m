function [objects,stats]=fuseCompatibleEvidence(inputs,weights,model,details,cfg,arm,t)
% Conservative inherited posterior with selectively admitted current evidence.
assert(ismember(arm,{'gaussian_evidence_compatible_agreement_all','gaussian_evidence_compatible_agreement_positive', ...
    'gaussian_evidence_compatible_agreement_conflict','gaussian_evidence_compatible_positive_consensus', ...
    'gaussian_evidence_compatible_positive_only'}));
...
for k=1:size(keys,2)
    ...
    overlap=exp(min(0,rec.spatialLogNormalizer));
    assert(rec.spatialLogNormalizer<1e-7 && overlap>=0 && overlap<=1);
    factor=ones(size(weights));
    if strcmp(arm,'gaussian_evidence_compatible_agreement_all')
        factor(:)=overlap;
    elseif strcmp(arm,'gaussian_evidence_compatible_agreement_positive')
        factor(increments>0)=overlap;
    elseif strcmp(arm,'gaussian_evidence_compatible_agreement_conflict')
        if any(active & increments>0) && any(active & increments<0),factor(:)=overlap;end
    elseif strcmp(arm,'gaussian_evidence_compatible_positive_consensus')
        if ~all(increments(active)>0),factor(increments>0)=0;end
    elseif strcmp(arm,'gaussian_evidence_compatible_positive_only')
        factor(increments<0)=0;
    end
    rawKappa=rawKappa.*factor;
    if any(rawKappa>0)
