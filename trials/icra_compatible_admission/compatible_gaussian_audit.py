...
from compatibility_math import factors
...
def check_gaussians(run, records, original, present, active, joint, beta, logits, delta, gates, negative):
    ...
    name = run['arm'].split('_compatible_', 1)[1]
    raw *= factors(records, delta, active, joint)[name]
    called = (raw > 0).any(1)
    ...
    existence_kept = kept.copy()
    if run['arm'].endswith('decoupled_all'):
        existence_kept = raw
    elif run['arm'].endswith('decoupled_positive'):
        existence_kept += (raw-kept)*(delta > 0)
    elif run['arm'].endswith('decoupled_negative'):
        existence_kept += (raw-kept)*(delta < 0)
    elif run['arm'].endswith('decoupled_consensus'):
