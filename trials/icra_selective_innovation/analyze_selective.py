...
def audit_file(path, source, mat, cohort, name, condition, arm):
    data = read(path)
    assert data['protocol'] == 'selective-current-innovation-v1' and data['implementation'] == 'selective-v1'
    ...
    width = 208 if arm == 'marked_conservative' else 224
...
def summarize(reports, frozen, cohort):
    ...
    result = dict(protocol=frozen['protocol'], cohort=cohort, primary=PRIMARY, sequences=len(names),
                  ...
                  packet_note='Selective arms 224 B/object. Native No-age/ER/CR 208 B/object in seen-transfer; older development No-age/ER retained an unused extra scalar, so their recorded bytes are not native 208 B costs.')
    ...
    print('ALL SELECTIVE OUTPUTS AUDITED', cohort, result['new_node_frames'], 'new node-frames;',
...
def preflight(source):
    ...
    for condition in ['reliable', 'intermittent']:
        ...
        for arm in ['marked_conservative'] + ARMS:
            ...
            if arm == 'marked_conservative':
                original_path = CONTROL / 'results_development' / f'0000_{condition}_marked_conservative.json.gz'
    ...
    result = dict(passed=True, independently_scored_node_frames=sum(2 * r['frames'] for r in rows),
                  exact_cr_node_frames=sum(2 * r['frames'] for r in rows if r['arm'] == 'marked_conservative'),
                  cr_trajectory_metric_packet_and_original_diagnostics_exact=True, scalar_radio_packet_and_extraction_checked=True,
                  source_files=len(source), rows=rows, input_sha256=hashes, auditor_sha256=sha(Path(__file__)))
    assert result['independently_scored_node_frames'] == 2940 and result['exact_cr_node_frames'] == 588
    (OUT / 'preflight_audit.json').write_text(json.dumps(result, indent=2, allow_nan=False) + '\n')
    print('SELECTIVE PREFLIGHT AUDIT PASSED: 2940 node-frames, 588 exact CR node-frames.', flush=True)
    ...
    for unit in units:
        while True:
            ...
            if entry is not None:
                assert entry['returncode'] == 0 and entry['completion_line'] and entry['files'] == 8, entry
                break
        ...
        print('SELECTIVE AUDITED', unit['cohort'], f"{unit['sequence']:04d}", len(reports), '/', len(units), flush=True)
