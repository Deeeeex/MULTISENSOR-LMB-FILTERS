function runSelectiveReplay(firstIndex,lastIndex,mode)
if nargin<1,firstIndex=0;end
if nargin<2,lastIndex=0;end
if nargin<3,mode='development_check';end
assert(any(strcmp(mode,{'development_check','development','seen_transfer'})));
...
addpath(port,priorIteration,priorCeiling,priorMarked,original,fullfile(root,'common'), ...
    fullfile(root,'lmb'),fullfile(root,'multisensorLmb'),fullfile(root,'trials','icra_reunion_fusion'));
checkStableMarkedEvidence();checkSelectiveInnovations();
...
if strcmp(mode,'seen_transfer')
    assert(isfile(fullfile(port,'METHOD_FREEZE.json')),'Final method and comparisons must be frozen before any holdout output.');
    inputManifest=jsondecode(fileread(fullfile(port,'input_manifest.json')));
...
for sequenceIndex=firstIndex:lastIndex
    ...
    if strcmp(mode,'seen_transfer')
        data=load(fullfile(port,'data',sprintf('v2v4real_%04d.mat',seq)));
    ...
    for condition={'reliable','intermittent'}
        ...
        runs=cell(size(arms));
        for a=1:numel(arms)
            arm=arms{a};fprintf('START SELECTIVE %04d %s %s %d frames\n',seq,radio,arm,T);
            ...
            fprintf('DONE SELECTIVE %04d %s %s OSPA %.6f runtime %.2fs\n',seq,radio,arm,mean(runs{a}.ospa,'all'),run.runtimeSeconds);
        result=struct('protocol','selective-current-innovation-v1','implementation','selective-v1', ...
...
fprintf('COMPLETED SELECTIVE indices %d--%d cohort=%s\n',firstIndex,lastIndex,mode);
end
...
template.lastDirectOpportunity=0;template.directEvidenceCeiling=0;template.positiveConfirmation=false;template.localLogOddsIncrement=0;template.positiveInnovationSupport=0;
...
run=struct('arm',arm,'estimates',{cell(2,T)},'rawEstimates',{cell(2,T)},'labels',{cell(2,T)}, ...
    ...
    'packetBytes',zeros(2,T),'localIncrementRecords',zeros(0,10),'iterationRecords',zeros(0,35),'maximumBernoulliCount',0,'runtimeSeconds',0,'totalWireBytes',0);
end
...
for t=1:T
    ...
    for n=1:2
        ...
        for j=1:numel(local{n})
            ...
            run.localIncrementRecords(end+1,:)=[t,n,local{n}(j).birthTime,local{n}(j).birthLocation, ...
                predicted(j).r,local{n}(j).r,delta,current,support(j),associationMass(j)]; %#ok<AGROW>
        end
    ...
    if strcmp(rule,'local')
        ...
        for n=1:2
            if startsWith(rule,'selective'),[decoded{n},packet]=selectiveInnovationPacket(local{n},model,n,t);
