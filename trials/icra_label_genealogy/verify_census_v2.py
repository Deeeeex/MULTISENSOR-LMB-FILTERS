...
def main():
    ...
    cfg=json.loads((OUT/'CENSUS_FREEZE_V2.json').read_text());report=json.loads((OUT/'CENSUS_RESULTS.json').read_text())
    ...
    assert report['freeze_sha256']==execution['freeze_sha256']==sha(OUT/'CENSUS_FREEZE_V2.json')
    ...
    result=dict(passed=True,robot_frames=len(population),target_phase_rows=len(targets),checks=checks,
        ...
        report_sha256=sha(OUT/'CENSUS_RESULTS.json'),freeze_sha256=sha(OUT/'CENSUS_FREEZE_V2.json'),
        execution_sha256=sha(OUT/'CENSUS_EXECUTION_V2.json'),verifier_sha256=sha(Path(__file__)))
