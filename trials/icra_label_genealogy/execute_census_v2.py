...
def main():
    receipt=OUT/'CENSUS_EXECUTION_V2.json';log=ROOT/'RUN/ICRA_LABEL_GENEALOGY/census_v2.log'
    ...
    state=dict(started_utc=datetime.now(timezone.utc).isoformat(),completed=False,
        command=[sys.executable,'-u',str(OUT/'census_v2.py')],source_sha256=sha(Path(__file__)),
        freeze_sha256=sha(OUT/'CENSUS_FREEZE_V2.json'),log=str(log.relative_to(ROOT)))
