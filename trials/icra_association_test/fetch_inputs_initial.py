...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        for future in as_completed([pool.submit(source.read_file,e) for e in prospective['files']]):
            fetched.append(future.result())
            if len(fetched)%200==0:
                print('FETCHED ADDITIONAL TEST',len(fetched),'/',len(prospective['files']),flush=True)
