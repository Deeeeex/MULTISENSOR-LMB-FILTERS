function runTemperedReplay(firstIndex,lastIndex,mode)
...
for sequenceIndex=firstIndex:lastIndex
    ...
    for condition={'reliable','intermittent'}
        ...
        runs=cell(size(arms));
        for a=1:numel(arms)
            arm=arms{a};fprintf('START TEMPERED %04d %s %s %d frames\n',seq,radio,arm,T);
            ...
            fprintf('DONE TEMPERED %04d %s %s OSPA %.6f runtime %.2fs\n',seq,radio,arm,mean(runs{a}.ospa,'all'),run.runtimeSeconds);
        result=struct('protocol','evidence-tempered-recency-v1','implementation','tempered-v1', ...
...
fprintf('COMPLETED TEMPERED indices %d--%d cohort=%s\n',firstIndex,lastIndex,mode);
end
...
for t=1:T
    ...
    for n=1:2
        ...
        if strcmp(rule,'tempered_score'),mark=marks.rawScores{n,t};end
        if strcmp(rule,'tempered_calibrated'),mark=marks.calibratedScores{n,t};end
    ...
    if strcmp(rule,'local')
        ...
        for n=1:2
            if startsWith(rule,'tempered_'),[decoded{n},packet]=ceilingLmbPacket(local{n},model,n,t);
