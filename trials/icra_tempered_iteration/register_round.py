...
def main():
    ...
    assert preflight['auditor_sha256'] == sha(OUT / 'analyze_tempered.py')
    ...
    files = [OUT / name for name in ['PROTOCOL.md', 'source_sha256.json', 'preflight_audit.json',
                                     'analyze_tempered.py', 'run_sequences.py', 'register_round.py']]
    ...
    result = dict(protocol='evidence-tempered-recency-v1', timestamp_utc=datetime.now(timezone.utc).isoformat(),
    ...
    print('TEMPERED ROUND FIXED: T-C primary; three arms; all nine development units first; no independent-holdout claim.', flush=True)
