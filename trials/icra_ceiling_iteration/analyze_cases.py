...
def main():
    ...
    candidate='ceiling_association'
    source=json.loads((OUT/'source_sha256_cases.json').read_text())
    ...
    for entry in baseline['inputs']:
        ...
        old=read(path);path=OUT/'results_cases'/f'{stem}_ecr.json.gz';new=read(path)
        ...
        print('AUDITED ECR case',scene,seed,audited,flush=True)
    assert audited==60*8*120 and len(rows)==60
    references=['qualified_exist','lineage','mil_support','confirmed_exist','conservative_recency','confirmation_recency','tc_ospa2_w5','tc_ospa2_w10']
    oldrows={(r['scene'],r['seed'],r['arm']):r for r in baseline['runs']}
    for file in ['summary_cases.json','summary_cases_confirmed.json']:
        for r in json.loads((OUT.parent/'icra_method_iteration'/file).read_text())['runs']:
            oldrows[r['scene'],r['seed'],r['arm']]=r
    external=json.loads((OUT.parent/'icra_external_fusion/summary_case_studies.json').read_text())
    for r in external['runs']:oldrows[r['scene'],r['seed'],r['arm']]=r
    ...
    print('CASE ECR AUDIT PASSED',audited,'new node-frames; original source and baseline hashes unchanged.')
