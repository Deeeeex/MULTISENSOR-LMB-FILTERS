function runEvidenceCeilingCases(firstSeed,lastSeed)
...
for seed=firstSeed:lastSeed
    for name={'split_latebirth','churn_departure','split_no_new'}
        ...
        for j=1:numel(data.model.birthParameters),data.model.birthParameters(j).directEvidenceCeiling=0;end
        data.model.object=data.model.birthParameters([]);
        fprintf('START ECR case %s seed %d\n',name{1},seed);
        run=runArm(data.model,data.measurements,data.uniforms,data.scene, ...
            data.adjacency,'ceiling_association',data.scene.T);
        ...
        result=data.result;result.runs=run;result.protocol='evidence-ceiling-case-studies-v1';
        result.sourceSha256=sources;
        path=fullfile(dest,[stem,'_ecr.json']);writeJson(path,result);gzip(path);delete(path);
        fprintf('DONE ECR case %s seed %d OSPA %.6f count %.6f runtime %.2fs\n', ...
            name{1},seed,mean(run.ospa,'all'),mean(run.countError,'all'),run.runtimeSeconds);
    end
end
fprintf('COMPLETED ECR cases seeds %d--%d\n',firstSeed,lastSeed);
end

function run=runArm(model,measurements,uniforms,scene,adjacency,arm,T)
...
for t=1:T
    ...
    if strcmp(arm,'local')
        ...
        for n=1:N
            ...
            [posterior{n},stats]=fuseEvidenceCeiling(inputs,weights,model,details,cfg,arm,t);
...
for k=1:numel(objects)
    ...
    values=[values,o.birthTime,o.birthLocation,o.r,o.numberOfGmComponents, ...
        double(o.hasObservationLineage),o.lastDirectOpportunity,double(o.positiveConfirmation),o.directEvidenceCeiling]; %#ok<AGROW>
    for j=1:o.numberOfGmComponents
