function runEvidenceCeilingReplay(firstSequence,lastSequence,smoke,arms)
...
if nargin<4,arms={'qualified_exist','ceiling_association','ceiling_score','ceiling_calibrated'};end
...
for seq=firstSequence:lastSequence
    ...
    marks=load(fullfile(out,'data_marks',sprintf('marks_%04d.mat',seq)));
    ...
    for condition={'reliable','intermittent'}
        ...
        for a=1:numel(arms)
            ...
            runs{a}=runDensity(model,measurements,positions,delivered,arms{a},marks);
        ...
        result=struct('protocol','direct-evidence-ceiling-v1','implementation','ecr-v1', ...
...
function run=runDensity(model,measurements,positions,delivered,arm,marks)
...
for t=1:T
    ...
    if strcmp(arm,'local')
        ...
        for n=1:2
            ...
            [posterior{n},stats]=fuseEvidenceCeiling(inputs,[.5,.5],model,details,cfg,arm,t);
