...
def audit_file(path, source, mat, cohort, name, condition, arm):
    data = read(path)
    assert data['protocol'] == 'coherent-gaussian-evidence-v1' and data['implementation'] == 'gaussian-evidence-v1'
    ...
    width = 352 if arm in NEW_ARMS else 232
...
def summarize(reports, frozen, cohort):
    ...
    conservative = [r for r in primary if r['reference'] in ['marked_conservative', 'marked_ceiling_score', 'marked_asymmetric']]
    ...
    result = dict(protocol=frozen['protocol'], cohort=cohort, primary=PRIMARY, sequences=len(names),
                  ...
                  packet_note='Gaussian evidence arms 352 B/object; scalar M-AE control 232 B/object. Different summaries and costs, not an equal-byte comparison. Native No-age/ER/CR 208 B/object in seen-transfer; older development No-age/ER retained an unused extra scalar, so their recorded bytes are not native 208 B costs.')
    ...
    print('ALL GAUSSIAN EVIDENCE OUTPUTS AUDITED', cohort, result['new_node_frames'], 'new node-frames;',
...
def preflight(source):
    ...
    for condition in ['reliable', 'intermittent']:
        ...
        for arm in ['marked_asymmetric'] + ARMS:
            ...
            if arm == 'marked_asymmetric':
                original_path = ASYMMETRIC / 'results_development' / f'0000_{condition}_marked_asymmetric.json.gz'
                original = read(original_path)
                hashes[str(original_path.relative_to(ROOT))] = sha(original_path)
                for key, value in data['runs'].items():
                    if key not in ['runtimeSeconds', 'iterationRecords', 'localGaussianRecords', 'packetGaussianRecords']:
                        assert value == original['runs'][key], (condition, 'AS port parity', key)
                a = np.asarray(data['runs']['iterationRecords'], float)
                b = np.asarray(original['runs']['iterationRecords'], float)
                assert a.shape == (len(b), 60) and b.shape[1] == 37
                assert np.array_equal(a[:, :37], b, equal_nan=True)
                assert np.all(a[:, 37:] == 0)
    ...
    result = dict(passed=True, independently_scored_node_frames=sum(2 * r['frames'] for r in rows),
                  exact_as_node_frames=sum(2 * r['frames'] for r in rows if r['arm'] == 'marked_asymmetric'),
                  as_trajectory_metric_packet_and_original_diagnostics_exact=True, scalar_radio_packet_and_extraction_checked=True,
    ...
    assert result['independently_scored_node_frames'] == 2940 and result['exact_as_node_frames'] == 588
    (OUT / 'preflight_audit.json').write_text(json.dumps(result, indent=2, allow_nan=False) + '\n')
    print('GAUSSIAN EVIDENCE PREFLIGHT AUDIT PASSED: 2940 node-frames, 588 exact AS node-frames.', flush=True)
    ...
    for unit in units:
        while True:
            ...
            if entry is not None:
                ...
                break
            assert args.watch, ('Gaussian evidence unit is not complete', unit)
            time.sleep(10)
        ...
        print('GAUSSIAN EVIDENCE AUDITED', unit['cohort'], f"{unit['sequence']:04d}", len(reports), '/', len(units), flush=True)
