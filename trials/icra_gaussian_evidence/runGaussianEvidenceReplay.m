function runGaussianEvidenceReplay(firstIndex,lastIndex,mode)
...
checkStableMarkedEvidence();checkGaussianEvidence();
...
for sequenceIndex=firstIndex:lastIndex
    ...
    for condition={'reliable','intermittent'}
        ...
        runs=cell(size(arms));
        for a=1:numel(arms)
            arm=arms{a};fprintf('START GAUSSIAN EVIDENCE %04d %s %s %d frames\n',seq,radio,arm,T);
            ...
            fprintf('DONE GAUSSIAN EVIDENCE %04d %s %s OSPA %.6f runtime %.2fs\n',seq,radio,arm,mean(runs{a}.ospa,'all'),run.runtimeSeconds);
        result=struct('protocol','coherent-gaussian-evidence-v1','implementation','gaussian-evidence-v1', ...
...
fprintf('COMPLETED GAUSSIAN EVIDENCE indices %d--%d cohort=%s\n',firstIndex,lastIndex,mode);
end
...
run=struct('arm',arm,'estimates',{cell(2,T)},'rawEstimates',{cell(2,T)},'labels',{cell(2,T)}, ...
    ...
    'packetGaussianRecords',zeros(0,19),'iterationRecords',zeros(0,60),'maximumBernoulliCount',0,'runtimeSeconds',0,'totalWireBytes',0);
end
