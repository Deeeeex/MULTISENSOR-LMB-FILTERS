function runV2v4realReplay(firstSequence,lastSequence,smoke)
...
inputManifest=jsondecode(fileread(fullfile(out,'v2v4real_input_manifest.json')));
sourceManifest=jsondecode(fileread(fullfile(out,'replay_source_sha256.json')));
for seq=firstSequence:lastSequence
    ...
    for condition={'reliable','intermittent'}
        ...
        runs=cell(1,6); runs{1}=local;
        arms={'lineage','qualified_exist','mil_support'};
        for a=1:3
            fprintf('START V2V %04d %s %s %d frames\n',seq,radio,arms{a},T);
            runs{a+1}=runDensity(model,measurements,positions,delivered,arms{a});
        end
        runs{5}=runTc(local,positions,delivered,5);runs{6}=runTc(local,positions,delivered,10);
        ...
        result=struct('protocol','v2v4real-2d-real-detection-replay-v1', ...
            'implementation','consistent-domain-and-absence-v2', ...
            'inputSha256',inputManifest.sequences(seq+1).input_sha256, ...
