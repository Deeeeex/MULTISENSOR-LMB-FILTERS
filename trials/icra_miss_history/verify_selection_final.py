...
def main():
    ...
    diagnostics={name:sha(OUT/name) for name in ['FALSE_SUPPORT_DIAGNOSTIC.json','FALSE_EXTRA_POLARITY_DIAGNOSTIC.json','FALSE_POLARITY_VERIFICATION.json']}
    assert report['diagnostic_sha256']==diagnostics
    diagnostic=json.loads((OUT/'FALSE_SUPPORT_DIAGNOSTIC.json').read_text())
    assert diagnostic['passed'] and len(diagnostic['rows'])==700
    assert diagnostic['selection_sha256']==sha(OUT/'SCREEN_SELECTION.json')
    assert diagnostic['source_sha256']==sha(OUT/'diagnose_false_support.py')
    polarity=json.loads((OUT/'FALSE_POLARITY_VERIFICATION.json').read_text())
    assert polarity['passed'] and polarity['exact_saved_result_reproduced'] and polarity['rows']==352
    assert polarity['source_sha256']==sha(OUT/'verify_false_polarity.py')
    for group in [diagnostic['inputs'],polarity['inputs']]:
        for name,value in group.items():assert sha(ROOT/name)==value,name
    answer=dict(report_sha256=sha(OUT/'RESULTS_CN.md'),report_build_sha256=sha(OUT/'REPORT_BUILD.json'),diagnostic_sha256=diagnostics,passed=True,full=args.full,rows=len(rows),summaries=len(result['summaries']),comparisons=len(result['comparisons']),
    ...
    if args.full:
        path=OUT/'FINAL_VERIFICATION.json';assert not path.exists();path.write_text(json.dumps(answer,indent=2)+'\n')
