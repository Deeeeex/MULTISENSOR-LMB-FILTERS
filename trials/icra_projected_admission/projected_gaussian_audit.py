...
from projected_ratio import project
...
def check_gaussians(run, records, original, present, active, joint, beta, logits, delta, gates, negative):
    ...
    if run['arm'] != 'marked_gaussian_evidence_no_curvature':
        ...
        kept[~allowed] = 0
    original_kept = kept.copy()
    original_precision = j0 + np.einsum('ns,nsij->nij', original_kept, dj[indices])
    for i in np.flatnonzero(called):
        try:
            np.linalg.cholesky(original_precision[i])
            if 1 / np.linalg.cond(original_precision[i], 1) < 1e-12:
                original_kept[i] = 0
        except np.linalg.LinAlgError:
            original_kept[i] = 0
    dj, dh, dc, original_good, ranks = project(prior_mean, prior_cov, post_mean, post_cov)
    kept = raw * (original_good[indices] | (ranks[indices] > 0))
    ...
    existence_kept = original_kept.copy()
    if run['arm'].endswith('projected_all'):
        existence_kept = raw
    elif run['arm'].endswith('projected_consensus'):
        existence_kept += (raw-original_kept)*((delta > 0) & (kept > 0))
        current = records[:, 21:23] == records[:, 0, None]
        agreed = joint & ((~active) | (current & (delta < 0))).all(1)
        existence_kept[agreed] = raw[agreed]
    expected_r = expit((beta*logits).sum(1)+(existence_kept*delta).sum(1)+log_i)
    for column in [6, 9]:
