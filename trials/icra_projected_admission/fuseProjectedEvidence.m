function [objects,stats]=fuseProjectedEvidence(inputs,weights,model,details,cfg,arm,t)
% Conservative inherited posterior with selectively admitted current evidence.
assert(ismember(arm,{'gaussian_evidence_projected_space','gaussian_evidence_projected_all', ...
    'gaussian_evidence_projected_consensus'}));
...
for k=1:size(keys,2)
    ...
    originalKept=zeros(size(weights));
    ...
    if any(rawKappa>0)
        [~,~,~,originalKept]=applyGaussianEvidence(pieces,alpha,rawKappa,true);
        [mu,P,logIntegral,kept,allowed,aggregateFallback]=applyProjectedEvidence(pieces,alpha,rawKappa);
        if any(kept>0)
    ...
    existenceKappa=originalKept;
    if strcmp(arm,'gaussian_evidence_projected_all')
        existenceKappa=rawKappa;
    elseif strcmp(arm,'gaussian_evidence_projected_consensus')
        existenceKappa=originalKept+(rawKappa-originalKept).*(increments>0 & kept>0);
