function runKnownCensorReplay(configPath,unitIndex)
...
assert(strcmp(which('Hungarian'),fullfile(runtimePath,'Hungarian.m')));
checkRangeDetection(baseQuality);checkKnownCensor();
...
for condition=reshape(unit.conditions,1,[])
    ...
    for a=1:numel(unit.arms)
        ...
        result=struct('protocol','icra-known-censor-v1','stage',config.stage, ...
