...
def main():
    ...
    for u,native in zip(cfg['units'],runtime):
        ...
        for arm in u['arms']:
            ...
            if args.stage=='known_censor_controls':
                ...
                audit=json.loads((OUT/'audit_known_censor_controls.json').read_text());assert sha(reference)==audit['inputs'][str(reference.relative_to(ROOT))]
    ...
    destination.write_text(json.dumps(dict(passed=True,stage=args.stage,rows=rows,diagnostics=diagnostics,parity=parity,
        inputs=hashes,audited_robot_frames=2880,source_sha256={str(p.relative_to(ROOT)):sha(p) for p in OUT.glob('*.py')}),indent=2,allow_nan=False)+'\n')
