...
def main():
    log=ROOT/'RUN/ICRA_KNOWN_CENSOR/fixture.log';receipt=OUT/'FIXTURE.json'
    ...
    command=['/Applications/MATLAB_R2024a.app/bin/matlab','-singleCompThread','-batch',
        "addpath('common','lmb','multisensorLmb','trials/icra_known_censor');checkKnownCensor();"]
    with log.open('w') as stream:code=subprocess.run(command,cwd=ROOT,stdout=stream,stderr=subprocess.STDOUT).returncode
    content=log.read_text();passed=code==0 and 'KNOWN CENSOR CHECK PASSED' in content
