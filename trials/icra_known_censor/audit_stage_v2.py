...
def exact_prefix(run,control):
    events=np.asarray(run['knownCensorRecords'],float).reshape(-1,25);changed=events[events[:,4]!=events[:,5]]
    ...
    for name,omit in [('iterationRecords',{6,9}),('fusionOutputRecords',{4})]:
        ...
        assert a==b,name;checked.append(name+'_except_actual_r_at_first_event')
    ...
    for name in ['rawPayloadBytes','deliveredRawBytes','wireBytes','controlBytes','attemptedMessages','deliveredMessages']:
        assert run[name][:first]==control[name][:first],name;checked.append(name)
...
def main():
    ...
    destination=OUT/('audit_v2_'+args.stage+'.json');assert not destination.exists()
    ...
    for u,native in zip(cfg['units'],runtime):
        ...
        log=ROOT/'RUN/ICRA_KNOWN_CENSOR'/args.stage/(condition+'.log');content=log.read_text()
        for marker in ['KNOWN CENSOR CHECK PASSED','RANGE DETECTION CHECK PASSED',f'COMPLETED REVIEW {args.stage} {seq}']:assert marker in content
        ...
        for arm in u['arms']:
            path=OUT/'results'/args.stage/f'{seq}_{condition}_{arm}.json.gz';data=read(path);run=data['runs'];base=arm.removesuffix(SUFFIX)
            ...
            assert run['rangeDetectionModel']==fit and run['rangeDetectionMode']=='nominal' and run['knownCensorEnabled']==arm.endswith(SUFFIX)
            ...
            eventpath=OUT/'results'/args.stage/f'{seq}_{condition}_{arm}_reimport_v2.csv.gz';assert not eventpath.exists()
            ...
            diagnostics.append(dict(condition=condition,arm=arm,quality=quality,density=density,direct=direct,positive_mark_rows=marks,
            ...
            if args.stage=='known_censor_controls':
                ref=u['references'][base];reference=ROOT/ref['path'];assert sha(reference)==ref['sha256'];original=read(reference)['runs']
                ...
                reference=OUT/'results/known_censor_controls'/f'{seq}_{condition}_{base}.json.gz'
                audit=json.loads((OUT/'audit_v2_known_censor_controls.json').read_text());assert sha(reference)==audit['inputs'][str(reference.relative_to(ROOT))]
            ...
            print('AUDITED KNOWN CENSOR',args.stage,condition,arm,flush=True)
    for prefix,values in [('scores_v2_',rows),('target_frames_v2_',timeline),('population_frames_v2_',population_rows)]:
    ...
    print('KNOWN CENSOR NATIVE AUDIT PASSED',args.stage,2880,'robot frames',flush=True)
