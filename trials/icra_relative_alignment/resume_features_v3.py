"""Resume input preparation with an audited bin-index checker; archive failures."""
...
def main():
    frozen();repair=json.loads((OUT/'GRID_VERIFIER_REPAIR.json').read_text());assert repair['passed']
    for name,digest in repair['artifacts'].items():assert sha(ROOT/name)==digest,name
    receipt=OUT/'FEATURE_EXECUTION.json';old_bytes=receipt.read_bytes();previous=json.loads(old_bytes)
    ...
    command=[sys.executable,'-u',str((OUT/'prepare_features_index_retry.py').relative_to(ROOT))]
    ...
    current=dict(returncode=code,completed=code==0,command=command,freeze_sha256=sha(OUT/'FREEZE.json'),
        ...
        unchanged_estimator_sha256=sha(OUT/'alignment_math.py'),effective_voxel_checker_sha256=sha(OUT/'independent_voxel_index.py'),
        checker_repair_sha256=sha(OUT/'GRID_VERIFIER_REPAIR.json'),retry_entry_sha256=sha(OUT/'prepare_features_index_retry.py'),
        note='Previous failed receipt archived byte-for-byte. Frozen producer grids preserved; independent boundary checker corrected as documented.')
    write_new(OUT/f'FEATURE_RETRY_{attempt}.json',current);assert receipt.read_bytes()==old_bytes
    temporary=receipt.with_suffix('.next.json');temporary.write_text(json.dumps(current,indent=2)+'\n');temporary.replace(receipt)
    print('RAW INPUT CHECKER RETRY PROCESS EXITED',attempt,code,flush=True)
    assert code==0
