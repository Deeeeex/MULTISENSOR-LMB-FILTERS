function runGaussianZeroReplay(firstIndex,lastIndex,mode)
...
checkStableMarkedEvidence();checkGaussianZeroCodec();
...
for sequenceIndex=firstIndex:lastIndex
    ...
    for condition={'reliable','intermittent'}
        ...
        runs=cell(size(arms));
        for a=1:numel(arms)
            arm=arms{a};fprintf('START GAUSSIAN ZERO CODEC %04d %s %s %d frames\n',seq,radio,arm,T);
            ...
            fprintf('DONE GAUSSIAN ZERO CODEC %04d %s %s OSPA %.6f runtime %.2fs\n',seq,radio,arm,mean(runs{a}.ospa,'all'),run.runtimeSeconds);
        result=struct('protocol','coherent-gaussian-zero-codec-v1','implementation','gaussian-zero-codec-v1', ...
...
fprintf('COMPLETED GAUSSIAN ZERO CODEC indices %d--%d cohort=%s\n',firstIndex,lastIndex,mode);
end
...
run=struct('arm',arm,'estimates',{cell(2,T)},'rawEstimates',{cell(2,T)},'labels',{cell(2,T)}, ...
    ...
    'packetGaussianRecords',zeros(0,19),'packetGaussianTags',zeros(0,5),'iterationRecords',zeros(0,60),'maximumBernoulliCount',0,'runtimeSeconds',0,'totalWireBytes',0);
end
        ...
        for n=1:2
            if startsWith(rule,'gaussian_evidence')
                ...
                for j=1:numel(decoded{n})
                    o=decoded{n}(j);
                    run.packetGaussianTags(end+1,:)=[t,n,o.birthTime,o.birthLocation,double(tags(j))]; %#ok<AGROW>
