function diagnoseZeroCodecFixture()
...
fprintf('PLAIN_EQUAL=%d NAN_EQUAL=%d\n',isequal(received,old),isequaln(received,old));
for name=fieldnames(received)'
 if ~isequal(received.(name{1}),old.(name{1})),fprintf('DIFFERENT_FIELD %s NAN_EQUAL %d\n',name{1},isequaln(received.(name{1}),old.(name{1})));end
end
