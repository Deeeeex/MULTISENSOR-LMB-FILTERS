...
def main():
    ...
    if target.exists():
        assert target.read_bytes() == (OUT / 'initial_preflight/source_sha256.json').read_bytes()
        assert not list(OUT.glob('results_*/*.json.gz'))
    ...
    files += [OUT / 'PREFLIGHT_FIX.md'] + list((OUT / 'initial_preflight').iterdir())
