function runConservativeReplayFast(firstSequence,lastSequence,smoke,arms)
...
resultDir=fullfile(out,'results_v2v_conservative_fast');if ~isfolder(resultDir),mkdir(resultDir);end
manifest=jsondecode(fileread(fullfile(original,'v2v4real_input_manifest.json')));
sourceManifest=jsondecode(fileread(fullfile(out,'source_sha256_cr_fast.json')));
for seq=firstSequence:lastSequence
    ...
    for condition={'reliable','intermittent'}
        ...
        result=struct('protocol','conservative-existence-recency-v1','implementation','cr-v1-accelerated', ...
