function runMethodReplayFast(firstSequence,lastSequence,smoke,arms)
...
resultDir=fullfile(out,'results_v2v_fast');if ~isfolder(resultDir),mkdir(resultDir);end
manifest=jsondecode(fileread(fullfile(original,'v2v4real_input_manifest.json')));
sourceManifest=jsondecode(fileread(fullfile(out,'source_sha256_ir_fast.json')));
for seq=firstSequence:lastSequence
    ...
    for condition={'reliable','intermittent'}
        ...
        result=struct('protocol','current-innovation-recency-v1','implementation','ir-v1-accelerated', ...
...
for t=1:T
    ...
    if strcmp(arm,'local')
        ...
        for n=1:2,[decoded{n},packet]=innovationLmbPacket(local{n},model,n,t);sizes(n)=numel(packet);end
        for n=1:2
            other=3-n;
            ...
            [posterior{n},stats]=fuseInnovationRecency(inputs,[.5,.5],model,details,cfg,arm,t);
