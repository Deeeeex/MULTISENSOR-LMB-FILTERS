function [objects,stats]=fuseConfirmedRecency(inputs,weights,model,details,cfg,arm,t)
% Admit an upward age correction only with current local confirmation.
assert(strcmp(arm,'confirmation_recency'));
...
for k=1:size(keys,2)
    pieces=cell(size(inputs));factors=ones(size(weights));stamps=zeros(size(weights));confirmed=false;currentConfirmation=false(size(weights));
    for s=1:numel(inputs)
        mask=[inputs{s}.birthTime]==keys(1,k) & [inputs{s}.birthLocation]==keys(2,k);
        pieces{s}=inputs{s}(mask);
        if isempty(pieces{s}),continue;end
        o=pieces{s};currentConfirmation(s)=o.positiveConfirmation;stamps(s)=o.lastDirectOpportunity;factors(s)=.25;
    ...
    if isfield(cfg,'captureIterationRecords') && cfg.captureIterationRecords
        ...
        stats.records(end+1,:)=[t,details.sourceIndices(1),keys(:,k)',one.mu{1}(1:2)', ...
            ...
            rec.inputExistence,double(currentConfirmation),stamps,shift,admitted,shift-admitted]; %#ok<AGROW>
    end
