function runTransferReplay(firstSequence,lastSequence,smoke)
if nargin<1, firstSequence=0; end
if nargin<2, lastSequence=6; end
...
original=fullfile(root,'trials','icra_external_fusion');addpath(original);
...
resultDir=fullfile(out,'results_transfer'); if ~isfolder(resultDir), mkdir(resultDir); end
inputManifest=jsondecode(fileread(fullfile(out,'transfer_input_manifest.json')));
sourceManifest=jsondecode(fileread(fullfile(out,'source_sha256_transfer.json')));
for sequenceIndex=firstSequence:lastSequence
    seq=inputManifest.selected_sequences(sequenceIndex+1);
    data=load(fullfile(out,'data_transfer',sprintf('v2v4real_%04d.mat',seq)));
    ...
    % Function has no ground-truth input. Local is reusable across radio arms.
    local=runDensity(model,measurements,positions,false(2,2,T),'local');
    for condition={'reliable','intermittent'}
        radio=condition{1}; delivered=repmat(logical([0,1;1,0]),1,1,T);
        if strcmp(radio,'intermittent')
            rng(8301+seq,'twister'); draws=rand(2,2,T);
            delivered=delivered & draws>=.1;
            delivered(:,:,floor(.4*T)+1:floor(.6*T))=false;
        end
        runs=cell(1,8); runs{1}=local;
        arms={'lineage','qualified_exist','conservative_recency','confirmation_recency','mil_support'};
        for a=1:5
            fprintf('START V2V %04d %s %s %d frames\n',seq,radio,arms{a},T);
            runs{a+1}=runDensity(model,measurements,positions,delivered,arms{a});
        end
        runs{7}=runTc(local,positions,delivered,5);runs{8}=runTc(local,positions,delivered,10);
        % Scoring is the first point at which the tracker outputs see truth.
        for a=1:numel(runs)
            runs{a}=scoreOutputs(runs{a},data.truth(1:T));
            fprintf('DONE V2V %04d %s %s OSPA %.5f count %.5f runtime %.2fs\n', ...
                seq,radio,runs{a}.arm,mean(runs{a}.ospa,'all'),mean(runs{a}.countError,'all'),runs{a}.runtimeSeconds);
        end
        result=struct('protocol','reserved-v2v4real-algorithm-transfer-v1', ...
            'implementation','cr-cgr-transfer-v1', ...
            'inputSha256',inputManifest.sequences(sequenceIndex+1).input_sha256, ...
            'sourceSha256',sourceManifest, ...
            'sequence',sprintf('%04d',seq), ...
            'condition',radio,'smoke',smoke,'time',double(data.time(1:T)), ...
            'positions',positions,'truth',{data.truth(1:T)},'truthIds',{data.truthIds(1:T)}, ...
            'delivered',delivered,'runs',[runs{:}]);
...
fprintf('COMPLETED V2V sequences %04d--%04d smoke=%d\n',firstSequence,lastSequence,smoke);
end
...
template.lastDirectOpportunity=0;template.positiveConfirmation=false;
...
run=struct('arm',arm,'estimates',{cell(2,T)},'rawEstimates',{cell(2,T)},'labels',{cell(2,T)}, ...
    ...
    'maximumBernoulliCount',0,'runtimeSeconds',0,'totalWireBytes',0);
end
...
for t=1:T
    ...
    for n=1:2
        ...
        if strcmp(arm,'confirmation_recency')
