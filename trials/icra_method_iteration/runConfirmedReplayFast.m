function runConfirmedReplayFast(firstSequence,lastSequence,smoke,arms)
...
if nargin<4,arms={'confirmation_recency'};end
...
resultDir=fullfile(out,'results_v2v_confirmed_fast');if ~isfolder(resultDir),mkdir(resultDir);end
manifest=jsondecode(fileread(fullfile(original,'v2v4real_input_manifest.json')));
sourceManifest=jsondecode(fileread(fullfile(out,'source_sha256_cgr_fast.json')));
for seq=firstSequence:lastSequence
    ...
    for condition={'reliable','intermittent'}
        ...
        result=struct('protocol','confirmation-gated-recency-v1','implementation','cgr-v1-accelerated', ...
