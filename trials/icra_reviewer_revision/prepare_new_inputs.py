...
def main():
    freeze_path = OUT / 'NEW_DATA_FREEZE.json'; freeze = json.loads(freeze_path.read_text())
    detections_path = OUT / 'NEW_DETECTION_MANIFEST.json'; detection_manifest = json.loads(detections_path.read_text())
    ...
    pose_folder = OUT / 'pose_inputs/new_validation'; pose_folder.mkdir(parents=True, exist_ok=True)
    ...
    for sequence, detection_record in zip(freeze['sequences'], detection_manifest['sequences']):
        ...
        data_path = folder / f'v2v4real_{name}.mat'; save_mat(data_path, data)
        ...
        records.append(dict(sequence=name, scene=sequence['scene'], original_recording=sequence['original_recording'],
                            original_recording_present_in_old_data=sequence['original_recording_present_in_old_data'],
    ...
    sources = [Path(__file__), OUT / 'official_input_adapter.py', OUT / 'prepare_pose_inputs.py',
    ...
    result = dict(passed=True, created_utc=datetime.now(timezone.utc).isoformat(), sequences=records,
                  ...
                  purpose='Frozen new method-evaluation input; annotation fields enter only scoring after each complete trajectory.')
