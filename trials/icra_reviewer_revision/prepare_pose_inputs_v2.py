...
def save_mat(path, values):
    if path.exists():
        ...
        assert all(np.array_equal(existing[key], value) for key, value in values.items()), path
        return
...
def main():
    ...
    for sequence in sorted(selected):
        ...
        stems = sorted(p.stem for p in (scene / '0').glob('*.yaml'))
        assert len(stems) == T and np.all(np.diff([int(s) for s in stems]) == 1)
        ...
        row = dict(sequence=f'{sequence:04d}', scene=scene.name, frames=T,
                   raw_frame_first=stems[0], raw_frame_last=stems[-1],
