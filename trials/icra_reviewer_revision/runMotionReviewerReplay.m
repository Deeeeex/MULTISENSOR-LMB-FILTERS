function runMotionReviewerReplay(configPath,unitIndex)
...
for condition={'reliable','intermittent'}
    ...
    for a=1:numel(config.arms)
        ...
        result=struct('protocol','icra-reviewer-revision-v2','stage',config.stage, ...
            ...
            'pd',config.pd,'coordinateAdapter',config.coordinate_adapter,'egoPrevToCurrent',motion,'time',double(data.time),'positions',positions, ...
