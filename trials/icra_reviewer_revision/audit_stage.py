...
ROOT=OUT.parents[1]
for directory in ['icra_gaussian_evidence','icra_marked_control','icra_fusion_holdout']:
...
def main():
    ...
    result=dict(passed=True,stage=args.stage,cohort=cfg['cohort'],sequences=len(cfg['units']),
                ...
                auditor_sha256={str(p.relative_to(ROOT)):sha(p) for p in [Path(__file__),OUT/'review_probability_audit.py',OUT/'review_gaussian_audit.py']})
