...
def main():
    ...
    for unit in cfg['units']:
        assert hashlib.sha256((ROOT/unit['data_path']).read_bytes()).hexdigest()==unit['input_sha256']
        if 'pose_path' in unit:
            assert hashlib.sha256((ROOT/unit['pose_path']).read_bytes()).hexdigest()==unit['pose_sha256']
    logdir=ROOT/'RUN/ICRA_REVIEWER_REVISION'/args.stage
    ...
    def worker(i,unit):
        ...
        command=['/Applications/MATLAB_R2024a.app/bin/matlab','-singleCompThread','-batch',
                 f"addpath('trials/icra_reviewer_revision');runMotionReviewerReplay('{cfgpath.relative_to(ROOT)}',{i+1});"]
