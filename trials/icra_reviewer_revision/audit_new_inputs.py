...
def main():
    ...
    detections = json.loads((OUT / 'NEW_DETECTION_MANIFEST.json').read_text())
    for item in [freeze['protected_sha256'], freeze['archive_directory_hashes'], freeze['raw_file_sha256'], manifest['source_sha256']]:
    ...
    for sequence, frozen, detector in zip(manifest['sequences'], freeze['sequences'], detections['sequences']):
        ...
        for t, stem in enumerate(frozen['paired_stems']):
            raw = [read_yaml(scene / str(n) / f'{stem}.yaml') for n in range(2)]
            transforms = [np.linalg.inv(raw[0]['lidar_pose']) @ source['lidar_pose'] for source in raw]
            positions = np.vstack([np.zeros(2), transforms[1][:2, 3]])
            assert np.array_equal(mat['positions'][:, :, t].T, positions)
            assert np.array_equal(pose['egoLidarToWorld'][:, :, t], raw[0]['lidar_pose'])
            for n in range(2):
            ...
            boxes, labels = labels_for_pair(raw, transforms); keep = domain(boxes[:, :2].astype(float), positions)
    ...
    result = dict(passed=True, sequences=rows, frames=sum(r['frames'] for r in rows),
                  ...
                  adapter_sha256=sha(OUT / 'official_input_adapter.py'), calibration_probability_clamps=all_calibration_clamps,
                  checks='All source hashes; exact saved poses, domain-filtered measurements, raw/calibrated scores, likelihood ratios, author-geometry label union and annotation-only velocities.')
