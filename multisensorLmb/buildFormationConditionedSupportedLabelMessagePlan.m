...
if strcmp(payloadMode, 'signed-complete-label-exceptions')
    if nargin >= 7 || isempty(timeIdx) || ...
        ...
        error('FormationSupportedLabelPlan:MissingExceptionSchedule', ...
            'The signed complete-label mode requires a frozen time plan.');
    end
...
plan.isOnlineDeployable = ...
    ~strcmp(payloadMode, 'signed-complete-label-exceptions');
...
function keep = selectExplicitLabels(senderObjects, labels)
if isempty(labels)
    labels = zeros(2, 0);
end
...
if nnz(keep) ~= size(labels, 2)
    error('FormationSupportedLabelPlan:ExceptionLabelMissing', ...
        'A frozen V108 label is absent from the current sender payload.');
end
end
