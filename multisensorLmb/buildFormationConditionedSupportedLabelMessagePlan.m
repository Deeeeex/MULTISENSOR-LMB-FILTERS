...
binaryAdmissionMode = strcmp( ...
    ...
    strcmp(payloadMode, 'lineage-relay-full-working-abstention');
lineageRelayMode = strcmp( ...
    payloadMode, 'lineage-relay-full-working-abstention');
...
plan.usesFullSenderPosteriors = lineageRelayMode || ...
    (fullPosteriorTrustMode && any(formationTrustFactors > 1e-12));
    ...
    for localSenderIdx = 1:numel(senders)
        ...
        if fullPosteriorTrustMode
            trustIdx = find(scheduledFormationIds == ...
                groupIds(receiverIdx), 1);
            if lineageRelayMode
                % V135 keeps the reference-lineage posterior on the wire
                % even when the working estimator abstains from consuming it.
                % This is a replacement payload, not an auxiliary channel.
                edgePayloadMode = 'full-posterior';
            elseif formationTrustFactors(trustIdx) <= 1e-12
