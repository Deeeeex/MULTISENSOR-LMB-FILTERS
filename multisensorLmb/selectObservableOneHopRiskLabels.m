...
if isempty(cachedPath) || ~strcmp(cachedPath, path)
    ...
    if ~isfield(loaded, 'policy') || ~isstruct(loaded.policy) || ...
            ...
            ~ismember(loaded.policy.contractVersion, { ...
            ...
            ~ismember(loaded.policy.maximumSequentialEdits, [1, 2]) || ...
            exist(loaded.policy.firstModelPath, 'file') ~= 2
        error('LearnedOneHopSafeLabelV169:InvalidSequentialPolicy', ...
            'The registered learned one-hop policy contract is invalid.');
    end
    if loaded.policy.maximumSequentialEdits == 2 && ...
            (~strcmp(loaded.policy.contractVersion, ...
                'cardinality-pressure-relief-policy-v175-v1') || ...
             ~isfield(loaded.policy.thresholds, ...
                'minimumExpectedCardinalityFraction') || ...
             ~isfield(loaded.policy.thresholds, ...
                'minimumMapProbability') || ...
             exist(loaded.policy.secondModelPath, 'file') ~= 2)
        error('LearnedOneHopSafeLabelV169:InvalidSequentialPolicy', ...
            'The registered two-stage learned policy is incomplete.');
    end
    if loaded.policy.maximumSequentialEdits == 1 && ...
            (~strcmp(loaded.policy.contractVersion, ...
                'rollout-aware-first-action-policy-v179-v1') || ...
             ~isempty(loaded.policy.secondModelPath))
        error('LearnedOneHopSafeLabelV169:InvalidSequentialPolicy', ...
            'The registered rollout-aware one-stage policy is incomplete.');
    end
    cachedPath = path;
