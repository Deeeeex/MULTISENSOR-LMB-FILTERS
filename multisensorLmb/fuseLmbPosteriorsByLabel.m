...
captureLabelKlaDiagnostics = logical(getField( ...
    triggerConfig, 'captureLabelKlaDiagnosticsEnabled', false));
untouchedPriorExclusionEnabled = logical(getField( ...
    triggerConfig, 'untouchedPriorExclusionEnabled', false));
fusionDiagnostics.labelKlaRecords = repmat( ...
    emptyLabelKlaDiagnosticRecord(), 1, 0);
...
for labelIdx = 1:size(labels, 2)
    ...
    if untouchedPriorExclusionEnabled
        [present, existencePresent, lineageInformed, excludedCount] = ...
            restrictUntouchedLmbPriorsV284( ...
                localObjects, present, existencePresent, ...
                labelExistenceWeights, existenceOverrides);
        fusionDiagnostics.lineageExcludedSourceCount = ...
            fusionDiagnostics.lineageExcludedSourceCount + excludedCount;
    end
    activeSpatialWeights = labelSpatialWeights .* present;
    activeExistenceWeights = ...
        labelExistenceWeights .* existencePresent;
    ...
    if captureLabelKlaDiagnostics
        fusionDiagnostics.labelKlaRecords(end + 1) = ...
            buildLabelKlaDiagnosticRecord( ...
...
function [spatialWeights, existenceWeights] = ...
        resolveLabelSpecificWeights( ...
            label, spatialWeights, existenceWeights, fusionDetails)
% Apply a routing decision inside the one ordinary label-wise KLA call.
%
% The appended semantic source carries zero topology weight globally, so it
% cannot perturb unrelated labels.  For its selected label only, routing may
% either allocate a fixed proportional share, transfer one ordinary input
% edge's complete weight to the source, or transfer a bounded fraction of
% that edge.  All modes stay inside the single ordinary KLA call and avoid a
% second post-fusion update.

if ~isstruct(fusionDetails) || ...
        ~isfield(fusionDetails, 'labelSpecificWeightOverrides')
    return;
end
overrides = fusionDetails.labelSpecificWeightOverrides;
if isempty(overrides)
    return;
end
if ~isstruct(overrides)
    error('LmbKla:InvalidLabelSpecificWeightOverride', ...
        'Label-specific fusion-weight overrides must be structures.');
end
matched = false;
for overrideIdx = 1:numel(overrides)
    override = overrides(overrideIdx);
    required = {'label', 'sourceInputIndex', 'mode'};
    if any(~isfield(override, required)) || ...
            ~isequal(size(override.label), [2, 1]) || ...
            ~isscalar(override.sourceInputIndex) || ...
            ~isfinite(override.sourceInputIndex) || ...
            override.sourceInputIndex ~= round(override.sourceInputIndex) || ...
            override.sourceInputIndex < 1 || ...
            override.sourceInputIndex > numel(spatialWeights) || ...
            ~ischar(override.mode)
        error('LmbKla:InvalidLabelSpecificWeightOverride', ...
            'A label-specific fusion-weight override is malformed.');
    end
    if ~isequal(override.label, label)
        continue;
    end
    if matched
        error('LmbKla:DuplicateLabelSpecificWeightOverride', ...
            'Only one fusion-weight override is allowed per label.');
    end
    mode = lower(strrep(override.mode, '_', '-'));
    if strcmp(mode, 'proportional-share')
        if ~isfield(override, 'sourceWeight') || ...
                ~isscalar(override.sourceWeight) || ...
                ~isfinite(override.sourceWeight) || ...
                override.sourceWeight <= 0 || override.sourceWeight >= 1
            error('LmbKla:InvalidLabelSpecificWeightOverride', ...
                'A proportional source-share override is malformed.');
        end
        spatialWeights = assignProportionalSourceShare( ...
            spatialWeights, override.sourceInputIndex, ...
            override.sourceWeight);
        existenceWeights = assignProportionalSourceShare( ...
            existenceWeights, override.sourceInputIndex, ...
            override.sourceWeight);
    elseif strcmp(mode, 'dominant-nonself-transfer')
        if ~isfield(override, 'replacedInputIndex') || ...
                ~isscalar(override.replacedInputIndex) || ...
                ~isfinite(override.replacedInputIndex) || ...
                override.replacedInputIndex ~= ...
                    round(override.replacedInputIndex) || ...
                override.replacedInputIndex < 1 || ...
                override.replacedInputIndex > numel(spatialWeights) || ...
                override.replacedInputIndex == override.sourceInputIndex
            error('LmbKla:InvalidLabelSpecificWeightOverride', ...
                'A dominant-edge transfer override is malformed.');
        end
        spatialWeights = transferInputWeight( ...
            spatialWeights, override.replacedInputIndex, ...
            override.sourceInputIndex);
        existenceWeights = transferInputWeight( ...
            existenceWeights, override.replacedInputIndex, ...
            override.sourceInputIndex);
    elseif strcmp(mode, 'bounded-dominant-nonself-transfer')
        if ~isfield(override, 'replacedInputIndex') || ...
                ~isscalar(override.replacedInputIndex) || ...
                ~isfinite(override.replacedInputIndex) || ...
                override.replacedInputIndex ~= ...
                ...
                override.replacedInputIndex > numel(spatialWeights) || ...
                override.replacedInputIndex == override.sourceInputIndex || ...
                ~isfield(override, 'transferFraction') || ...
                ~isscalar(override.transferFraction) || ...
                ~isfinite(override.transferFraction) || ...
                override.transferFraction <= 0 || ...
                override.transferFraction > 1
            error('LmbKla:InvalidLabelSpecificWeightOverride', ...
                'A bounded dominant-edge transfer override is malformed.');
        end
        spatialWeights = transferInputWeightFraction( ...
            spatialWeights, override.replacedInputIndex, ...
            override.sourceInputIndex, override.transferFraction);
        existenceWeights = transferInputWeightFraction( ...
            existenceWeights, override.replacedInputIndex, ...
            override.sourceInputIndex, override.transferFraction);
    else
        error('LmbKla:UnknownLabelSpecificWeightMode', ...
            'Unknown label-specific fusion-weight mode: %s.', mode);
    end
    matched = true;
end
end

function weights = transferInputWeightFraction( ...
        weights, replacedInputIndex, sourceInputIndex, transferFraction)
% Preserve total label-wise KLA mass while moving a bounded edge fraction.

weights = reshape(weights, 1, []);
transferredWeight = ...
    transferFraction * weights(replacedInputIndex);
if transferredWeight <= 0
    error('LmbKla:DegenerateLabelSpecificWeightOverride', ...
        'The bounded label-input transfer must carry positive KLA weight.');
end
weights(replacedInputIndex) = ...
    weights(replacedInputIndex) - transferredWeight;
weights(sourceInputIndex) = ...
    weights(sourceInputIndex) + transferredWeight;
end

function weights = assignProportionalSourceShare( ...
        weights, sourceInputIndex, sourceWeight)
weights = reshape(weights, 1, []);
weights(sourceInputIndex) = 0;
ordinaryMass = sum(weights);
if ordinaryMass <= 0
    error('LmbKla:DegenerateLabelSpecificWeightOverride', ...
        'The routed label must retain at least one ordinary KLA input.');
end
weights = weights * ((1 - sourceWeight) / ordinaryMass);
weights(sourceInputIndex) = sourceWeight;
end

function weights = transferInputWeight( ...
        weights, replacedInputIndex, sourceInputIndex)
% Preserve total label-wise KLA mass while rewiring one effective edge.

weights = reshape(weights, 1, []);
transferredWeight = weights(replacedInputIndex);
if transferredWeight <= 0
    error('LmbKla:DegenerateLabelSpecificWeightOverride', ...
        'The replaced label input must carry positive KLA weight.');
end
weights(replacedInputIndex) = 0;
weights(sourceInputIndex) = ...
    weights(sourceInputIndex) + transferredWeight;
end
...
for restrictedInputIdx = find(restricted)
    whitelistMatrix = whitelists{restrictedInputIdx};
    if isempty(whitelistMatrix)
        whitelistMatrix = zeros(2, 0);
    end
    if ~isnumeric(whitelistMatrix) || size(whitelistMatrix, 1) ~= 2 || ...
            any(~isfinite(whitelistMatrix(:))) || ...
            any(whitelistMatrix(:) ~= round(whitelistMatrix(:))) || ...
            size(unique(whitelistMatrix', 'rows'), 1) ~= ...
                size(whitelistMatrix, 2)
        error('LmbKla:InvalidLabelWhitelist', ...
            'Every fusion-input label whitelist must be unique and 2-by-K.');
    end
    presentObject = localObjects{restrictedInputIdx};
    if present(restrictedInputIdx)
        presentObjectLabel = [presentObject.birthTime; ...
            presentObject.birthLocation];
        if ~any(all(bsxfun( ...
                @eq, whitelistMatrix, presentObjectLabel), 1))
            error('LmbKla:PayloadOutsideLabelWhitelist', ...
                'A restricted payload contains a non-whitelisted label.');
        end
    else
        abstained(restrictedInputIdx) = true;
    end
end
diagnostics.explicitLabelAbstentionSourceCount = ...
...
function record = buildLabelKlaDiagnosticRecord( ...
...
for sourceIdx = find(existencePresent)
    if sourceIdx <= numel(existenceOverrides) && ...
            isfinite(existenceOverrides(sourceIdx))
        inputExistence(sourceIdx) = clampProbability( ...
            existenceOverrides(sourceIdx));
    elseif ~isempty(localObjects{sourceIdx})
        inputExistence(sourceIdx) = clampProbability( ...
...
for sourceIdx = find(existencePresent)
    weightedInputLogOdds = weightedInputLogOdds + ...
        activeExistenceWeights(sourceIdx) * ...
        probabilityLogOdds(inputExistence(sourceIdx));
end
fusedLogOdds = probabilityLogOdds(fusedObject.r);
record.label = reshape(label, 2, 1);
record.sourceCount = sourceCount;
record.spatialParticipating = reshape(logical(spatialPresent), 1, []);
record.existenceParticipating = ...
    reshape(logical(existencePresent), 1, []);
record.activeSpatialWeights = reshape(activeSpatialWeights, 1, []);
record.activeExistenceWeights = reshape(activeExistenceWeights, 1, []);
...
record.fusedExistence = fusedObject.r;
record.fusedLogOdds = fusedLogOdds;
record.normalizerIdentityResidual = ...
    fusedLogOdds - weightedInputLogOdds - real(logEta);
record.fusedComponentCount = fusedObject.numberOfGmComponents;
end

function record = emptyLabelKlaDiagnosticRecord()
record = struct( ...
    'label', zeros(2, 1), ...
    'sourceCount', 0, ...
    'spatialParticipating', false(1, 0), ...
    'existenceParticipating', false(1, 0), ...
    'activeSpatialWeights', zeros(1, 0), ...
    'activeExistenceWeights', zeros(1, 0), ...
