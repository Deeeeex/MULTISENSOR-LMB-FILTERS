...
% A registered explicit omitted-label set masks only labels deliberately held
% back by the sender; natural absence continues to use the selected reference
% missing-label rule.
...
for labelIdx = 1:size(labels, 2)
    ...
    [present, existencePresent, existenceOverrides, ...
        ...
        label, localObjects, present, spatialWeights, existenceWeights, ...
...
function [participating, existenceParticipating, ...
    ...
    resolveMissingLabelParticipation( ...
    label, localObjects, present, spatialWeights, existenceWeights, model, ...
    fusionDetails, triggerConfig)
...
[explicitLabelAbstention, whitelistDiagnostics] = ...
    resolveExplicitLabelAbstention( ...
        fusionDetails, label, localObjects, present, ...
        positiveExistenceSource);
...
hasOmissionFlags = isfield( ...
    fusionDetails, 'explicitLabelOmissionRegistered');
hasOmissionSets = isfield(fusionDetails, 'explicitOmittedLabels');
hasLegacyFlags = isfield(fusionDetails, 'labelWhitelistRestricted');
hasLegacyWhitelists = isfield(fusionDetails, 'labelWhitelist');
if ~hasOmissionFlags && ~hasOmissionSets && ...
        ~hasLegacyFlags && ~hasLegacyWhitelists
    return;
end
if xor(hasOmissionFlags, hasOmissionSets)
    error('LmbKla:IncompleteExplicitLabelOmissionContext', ...
        ['Explicit label omission requires registration flags and ', ...
         'omitted-label sets together.']);
end
if xor(hasLegacyFlags, hasLegacyWhitelists)
    error('LmbKla:IncompleteLabelWhitelistContext', ...
        'Label-wise abstention requires flags and whitelists together.');
end

omissionRegistered = false(1, sourceCount);
omittedLabelSets = repmat({zeros(2, 0)}, 1, sourceCount);
if hasOmissionFlags
    omissionRegistered = reshape( ...
        logical(fusionDetails.explicitLabelOmissionRegistered), 1, []);
    omittedLabelSets = fusionDetails.explicitOmittedLabels;
    if numel(omissionRegistered) ~= sourceCount || ...
            ~iscell(omittedLabelSets) || ...
            numel(omittedLabelSets) ~= sourceCount
        error('LmbKla:InvalidExplicitLabelOmissionContext', ...
            ['Explicit label-omission metadata must match the ', ...
             'fusion inputs.']);
    end
    for inputIdx = 1:sourceCount
        omittedLabelSets{inputIdx} = validateLabelMatrix( ...
            omittedLabelSets{inputIdx}, ...
            'LmbKla:InvalidExplicitOmittedLabels', ...
            ['Every explicit omitted-label set must be unique and ', ...
             '2-by-K.']);
        if ~omissionRegistered(inputIdx) && ...
                ~isempty(omittedLabelSets{inputIdx})
            error('LmbKla:UnregisteredExplicitOmittedLabels', ...
                ['A nonempty omitted-label set requires a registered ', ...
                 'control envelope.']);
        end
    end
end

legacyRestricted = false(1, sourceCount);
legacyWhitelists = repmat({zeros(2, 0)}, 1, sourceCount);
if hasLegacyFlags
    legacyRestricted = reshape( ...
        logical(fusionDetails.labelWhitelistRestricted), 1, []);
    legacyWhitelists = fusionDetails.labelWhitelist;
end
if numel(legacyRestricted) ~= sourceCount || ...
        ~iscell(legacyWhitelists) || ...
        numel(legacyWhitelists) ~= sourceCount
    error('LmbKla:InvalidLabelWhitelistContext', ...
        'Label-wise abstention metadata must match the fusion inputs.');
end
if any(omissionRegistered & legacyRestricted)
    error('LmbKla:AmbiguousLabelAbstentionContext', ...
        ['One fusion input cannot use both the explicit omission ', ...
         'contract and the legacy restricted whitelist.']);
end

for inputIdx = find(omissionRegistered)
    omittedLabels = omittedLabelSets{inputIdx};
    currentLabelWasOmitted = any(all(bsxfun( ...
        @eq, omittedLabels, label), 1));
    if present(inputIdx) && currentLabelWasOmitted
        error('LmbKla:PayloadAlsoMarkedOmitted', ...
            ['A participating payload label cannot also be marked ', ...
             'as explicitly omitted.']);
    end
    if ~present(inputIdx) && currentLabelWasOmitted
        abstained(inputIdx) = true;
    end
end

for inputIdx = find(legacyRestricted)
    whitelistMatrix = validateLabelMatrix( ...
        legacyWhitelists{inputIdx}, ...
        'LmbKla:InvalidLabelWhitelist', ...
        ['Every fusion-input label whitelist must be unique and ', ...
         '2-by-K.']);
    presentObject = localObjects{inputIdx};
    if present(inputIdx)
...
function labels = validateLabelMatrix(labels, errorId, errorMessage)
if isempty(labels)
    labels = zeros(2, 0);
end
