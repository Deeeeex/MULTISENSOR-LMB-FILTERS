    ...
    for receiverIdx = 1:numberOfSensors
        ...
        [objectsBySensor{receiverIdx}, stateEstimatesBySensor{receiverIdx}] = ...
            finalizeNodeStep( ...
                ...
                model, currentTime);
        if triggerConfig.capturePosteriorSnapshots
            diagnostics.posteriorSnapshots{receiverIdx, currentTime} = ...
                snapshotLmbPosterior( ...
                    objectsBySensor{receiverIdx}, model);
        end
    end
...
capturePosteriorSnapshots = getField( ...
    config, 'capturePosteriorSnapshots', false);
if ~(islogical(capturePosteriorSnapshots) || ...
        isnumeric(capturePosteriorSnapshots)) || ...
        ~isreal(capturePosteriorSnapshots) || ...
        ~isscalar(capturePosteriorSnapshots) || ...
        ~isfinite(capturePosteriorSnapshots)
    error(['runEventTriggeredDistributedLmbFilter:' ...
        'InvalidSnapshotCaptureFlag'], ...
        'capturePosteriorSnapshots must be a finite logical scalar.');
end
config.capturePosteriorSnapshots = logical(capturePosteriorSnapshots);
...
if triggerConfig.capturePosteriorSnapshots
    diagnostics.posteriorSnapshots = cell( ...
        numberOfSensors, simulationLength);
end
end
