...
plan.contractVersion = ...
    'context-aware-receiver-safe-oracle-message-plan-v1';
...
for labelIdx = 1:size(labels, 2)
    ...
    if isempty(receiverObject) && ...
            ~any(cellfun(@(object) ~isempty(object), fixedForLabel))
        [optionSets{labelIdx}, optionMetadata{labelIdx}] = ...
            buildReceiverAndContextMissingOption( ...
                selectableForLabel, evidenceForLabel, ...
                model.xDimension);
        continue;
    end
...
function [options, metadata] = buildReceiverAndContextMissingOption( ...
...
option = makeMissingOption(senderCount);
option.variablePayloadBytes = collectionSubsetLabelBytes( ...
    senderObjects, mandatoryMask, stateDimension);
option.activeEdgeMask = mandatoryMask;
option.senderSubset = mandatoryMask;
option.selectedSenderCount = sum(mandatoryMask);
options = option;
metadata = struct( ...
    'contractVersion', ...
        'receiver-and-fixed-context-missing-positive-label-rule-v1', ...
    'mandatoryPositiveSenderMask', mandatoryMask, ...
    'fixedContextIncluded', true, ...
    'credibleNegativePropagatedWithoutContextLabel', false);
end
...
function option = makeMissingOption(senderCount)
option = struct( ...
    'distortion', 0, ...
    'variablePayloadBytes', 0, ...
